#!/bin/bash
# ace-tools.probe.sh - Runtime Validation Probe for ace-tools
#
# Comprehensive probe script to validate ace-tools service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/ace-tools.probe.sh
...
# Test 2.$((i + 1)): /v1/tools
if [[ "/v1/tools" == "/"* ]]; then
    test_json_endpoint "/v1/tools" "Endpoint 2: /v1/tools" || \
    test_http_endpoint "/v1/tools" "Endpoint 2: /v1/tools" "200" || \
    log_warning "Endpoint 2 (/v1/tools) not available"
else
    log_info "Command check: /v1/tools"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [[ "/v1/status" == "/"* ]]; then
    test_json_endpoint "/v1/status" "Endpoint 3: /v1/status" || \
    test_http_endpoint "/v1/status" "Endpoint 3: /v1/status" "200" || \
    log_warning "Endpoint 3 (/v1/status) not available"
else
...
if [[ "/v1/capabilities" == "/"* ]]; then
    test_json_endpoint "/v1/capabilities" "Endpoint 4: /v1/capabilities" || \
    test_http_endpoint "/v1/capabilities" "Endpoint 4: /v1/capabilities" "200" || \
    log_warning "Endpoint 4 (/v1/capabilities) not available"
else
...
# Test 2.$((i + 1)): /v1/metrics
if [[ "/v1/metrics" == "/"* ]]; then
    test_json_endpoint "/v1/metrics" "Endpoint 5: /v1/metrics" || \
    test_http_endpoint "/v1/metrics" "Endpoint 5: /v1/metrics" "200" || \
    log_warning "Endpoint 5 (/v1/metrics) not available"
else
    log_info "Command check: /v1/metrics"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ ACE-TOOLS PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ ACE-TOOLS PROBE FAILED${NC}"
    echo ""
    exit 1
fi
