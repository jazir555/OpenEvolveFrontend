#!/bin/bash
# api-aggregator.probe.sh - Runtime Validation Probe for api-aggregator
#
# Comprehensive probe script to validate api-aggregator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/api-aggregator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 API-AGGREGATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
if [[ "/aggregate" == "/"* ]]; then
    test_json_endpoint "/aggregate" "Endpoint 1: /aggregate" || \
    test_http_endpoint "/aggregate" "Endpoint 1: /aggregate" "200" || \
    log_warning "Endpoint 1 (/aggregate) not available"
else
...
# Test 2.$((i + 1)): /combine
if [[ "/combine" == "/"* ]]; then
    test_json_endpoint "/combine" "Endpoint 2: /combine" || \
    test_http_endpoint "/combine" "Endpoint 2: /combine" "200" || \
    log_warning "Endpoint 2 (/combine) not available"
else
    log_info "Command check: /combine"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /cache
if [[ "/cache" == "/"* ]]; then
    test_json_endpoint "/cache" "Endpoint 3: /cache" || \
    test_http_endpoint "/cache" "Endpoint 3: /cache" "200" || \
    log_warning "Endpoint 3 (/cache) not available"
else
    log_info "Command check: /cache"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /parallel
if [[ "/parallel" == "/"* ]]; then
    test_json_endpoint "/parallel" "Endpoint 4: /parallel" || \
    test_http_endpoint "/parallel" "Endpoint 4: /parallel" "200" || \
    log_warning "Endpoint 4 (/parallel) not available"
else
    log_info "Command check: /parallel"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ API-AGGREGATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ API-AGGREGATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
