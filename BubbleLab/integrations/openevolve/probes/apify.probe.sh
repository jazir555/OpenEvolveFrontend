#!/bin/bash
# apify.probe.sh - Runtime Validation Probe for apify
#
# Comprehensive probe script to validate apify service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/apify.probe.sh
...
# Test 2.$((i + 1)): /v2/actors
if [[ "/v2/actors" == "/"* ]]; then
    test_json_endpoint "/v2/actors" "Endpoint 1: /v2/actors" || \
    test_http_endpoint "/v2/actors" "Endpoint 1: /v2/actors" "200" || \
    log_warning "Endpoint 1 (/v2/actors) not available"
else
    log_info "Command check: /v2/actors"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v2/datasets
if [[ "/v2/datasets" == "/"* ]]; then
    test_json_endpoint "/v2/datasets" "Endpoint 2: /v2/datasets" || \
    test_http_endpoint "/v2/datasets" "Endpoint 2: /v2/datasets" "200" || \
    log_warning "Endpoint 2 (/v2/datasets) not available"
else
    log_info "Command check: /v2/datasets"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v2/users/me
if [[ "/v2/users/me" == "/"* ]]; then
    test_json_endpoint "/v2/users/me" "Endpoint 3: /v2/users/me" || \
    test_http_endpoint "/v2/users/me" "Endpoint 3: /v2/users/me" "200" || \
    log_warning "Endpoint 3 (/v2/users/me) not available"
else
    log_info "Command check: /v2/users/me"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v2/key-value-stores
if [[ "/v2/key-value-stores" == "/"* ]]; then
    test_json_endpoint "/v2/key-value-stores" "Endpoint 4: /v2/key-value-stores" || \
    test_http_endpoint "/v2/key-value-stores" "Endpoint 4: /v2/key-value-stores" "200" || \
    log_warning "Endpoint 4 (/v2/key-value-stores) not available"
else
    log_info "Command check: /v2/key-value-stores"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v2/request-queues
if [[ "/v2/request-queues" == "/"* ]]; then
    test_json_endpoint "/v2/request-queues" "Endpoint 5: /v2/request-queues" || \
    test_http_endpoint "/v2/request-queues" "Endpoint 5: /v2/request-queues" "200" || \
    log_warning "Endpoint 5 (/v2/request-queues) not available"
else
    log_info "Command check: /v2/request-queues"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ APIFY PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ APIFY PROBE FAILED${NC}"
    echo ""
    exit 1
fi
