#!/bin/bash
# backup-restore.probe.sh - Runtime Validation Probe for backup-restore
#
# Comprehensive probe script to validate backup-restore service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/backup-restore.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 BACKUP-RESTORE SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /backup
if [[ "/backup" == "/"* ]]; then
    test_json_endpoint "/backup" "Endpoint 1: /backup" || \
    test_http_endpoint "/backup" "Endpoint 1: /backup" "200" || \
    log_warning "Endpoint 1 (/backup) not available"
else
    log_info "Command check: /backup"
...
# Test 2.$((i + 1)): /restore
if [[ "/restore" == "/"* ]]; then
    test_json_endpoint "/restore" "Endpoint 2: /restore" || \
    test_http_endpoint "/restore" "Endpoint 2: /restore" "200" || \
    log_warning "Endpoint 2 (/restore) not available"
else
    log_info "Command check: /restore"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /verify
if [[ "/verify" == "/"* ]]; then
    test_json_endpoint "/verify" "Endpoint 4: /verify" || \
    test_http_endpoint "/verify" "Endpoint 4: /verify" "200" || \
    log_warning "Endpoint 4 (/verify) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ BACKUP-RESTORE PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ BACKUP-RESTORE PROBE FAILED${NC}"
    echo ""
    exit 1
fi
