#!/bin/bash
# data-enrichment.probe.sh - Runtime Validation Probe for data-enrichment
#
# Comprehensive probe script to validate data-enrichment service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/data-enrichment.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 DATA-ENRICHMENT SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /enrich
if [[ "/enrich" == "/"* ]]; then
    test_json_endpoint "/enrich" "Endpoint 1: /enrich" || \
    test_http_endpoint "/enrich" "Endpoint 1: /enrich" "200" || \
    log_warning "Endpoint 1 (/enrich) not available"
else
    log_info "Command check: /enrich"
...
# Test 2.$((i + 1)): /batch
if [[ "/batch" == "/"* ]]; then
    test_json_endpoint "/batch" "Endpoint 2: /batch" || \
    test_http_endpoint "/batch" "Endpoint 2: /batch" "200" || \
    log_warning "Endpoint 2 (/batch) not available"
else
...
# Test 2.$((i + 1)): /validate
if [[ "/validate" == "/"* ]]; then
    test_json_endpoint "/validate" "Endpoint 3: /validate" || \
    test_http_endpoint "/validate" "Endpoint 3: /validate" "200" || \
    log_warning "Endpoint 3 (/validate) not available"
else
...
# Test 2.$((i + 1)): /sources
if [[ "/sources" == "/"* ]]; then
    test_json_endpoint "/sources" "Endpoint 4: /sources" || \
    test_http_endpoint "/sources" "Endpoint 4: /sources" "200" || \
    log_warning "Endpoint 4 (/sources) not available"
else
...
# Test 2.$((i + 1)): /cache
if [[ "/cache" == "/"* ]]; then
    test_json_endpoint "/cache" "Endpoint 5: /cache" || \
    test_http_endpoint "/cache" "Endpoint 5: /cache" "200" || \
    log_warning "Endpoint 5 (/cache) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ DATA-ENRICHMENT PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ DATA-ENRICHMENT PROBE FAILED${NC}"
    echo ""
    exit 1
fi
