#!/bin/bash
# data-transformer.probe.sh - Runtime Validation Probe for data-transformer
#
# Comprehensive probe script to validate data-transformer service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/data-transformer.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 DATA-TRANSFORMER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
if [[ "/transform" == "/"* ]]; then
    test_json_endpoint "/transform" "Endpoint 1: /transform" || \
    test_http_endpoint "/transform" "Endpoint 1: /transform" "200" || \
    log_warning "Endpoint 1 (/transform) not available"
else
...
# Test 2.$((i + 1)): /map
if [[ "/map" == "/"* ]]; then
    test_json_endpoint "/map" "Endpoint 2: /map" || \
    test_http_endpoint "/map" "Endpoint 2: /map" "200" || \
    log_warning "Endpoint 2 (/map) not available"
else
    log_info "Command check: /map"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /filter
if [[ "/filter" == "/"* ]]; then
    test_json_endpoint "/filter" "Endpoint 3: /filter" || \
    test_http_endpoint "/filter" "Endpoint 3: /filter" "200" || \
    log_warning "Endpoint 3 (/filter) not available"
else
    log_info "Command check: /filter"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /pivot
if [[ "/pivot" == "/"* ]]; then
    test_json_endpoint "/pivot" "Endpoint 5: /pivot" || \
    test_http_endpoint "/pivot" "Endpoint 5: /pivot" "200" || \
    log_warning "Endpoint 5 (/pivot) not available"
else
    log_info "Command check: /pivot"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ DATA-TRANSFORMER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ DATA-TRANSFORMER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
