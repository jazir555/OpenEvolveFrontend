#!/bin/bash
# email-validator.probe.sh - Runtime Validation Probe for email-validator
#
# Comprehensive probe script to validate email-validator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/email-validator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 EMAIL-VALIDATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /verify
if [[ "/verify" == "/"* ]]; then
    test_json_endpoint "/verify" "Endpoint 2: /verify" || \
    test_http_endpoint "/verify" "Endpoint 2: /verify" "200" || \
    log_warning "Endpoint 2 (/verify) not available"
else
    log_info "Command check: /verify"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /batch
if [[ "/batch" == "/"* ]]; then
    test_json_endpoint "/batch" "Endpoint 4: /batch" || \
    test_http_endpoint "/batch" "Endpoint 4: /batch" "200" || \
    log_warning "Endpoint 4 (/batch) not available"
else
    log_info "Command check: /batch"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /check
if [[ "/check" == "/"* ]]; then
    test_json_endpoint "/check" "Endpoint 5: /check" || \
    test_http_endpoint "/check" "Endpoint 5: /check" "200" || \
    log_warning "Endpoint 5 (/check) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ EMAIL-VALIDATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ EMAIL-VALIDATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
