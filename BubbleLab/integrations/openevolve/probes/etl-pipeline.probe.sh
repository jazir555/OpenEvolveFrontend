#!/bin/bash
# etl-pipeline.probe.sh - Runtime Validation Probe for etl-pipeline
#
# Comprehensive probe script to validate etl-pipeline service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/etl-pipeline.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 ETL-PIPELINE SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /extract
if [[ "/extract" == "/"* ]]; then
    test_json_endpoint "/extract" "Endpoint 1: /extract" || \
    test_http_endpoint "/extract" "Endpoint 1: /extract" "200" || \
    log_warning "Endpoint 1 (/extract) not available"
else
    log_info "Command check: /extract"
...
if [[ "/transform" == "/"* ]]; then
    test_json_endpoint "/transform" "Endpoint 2: /transform" || \
    test_http_endpoint "/transform" "Endpoint 2: /transform" "200" || \
    log_warning "Endpoint 2 (/transform) not available"
else
...
# Test 2.$((i + 1)): /load
if [[ "/load" == "/"* ]]; then
    test_json_endpoint "/load" "Endpoint 3: /load" || \
    test_http_endpoint "/load" "Endpoint 3: /load" "200" || \
    log_warning "Endpoint 3 (/load) not available"
else
    log_info "Command check: /load"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /run
if [[ "/run" == "/"* ]]; then
    test_json_endpoint "/run" "Endpoint 4: /run" || \
    test_http_endpoint "/run" "Endpoint 4: /run" "200" || \
    log_warning "Endpoint 4 (/run) not available"
else
    log_info "Command check: /run"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /schedule
if [[ "/schedule" == "/"* ]]; then
    test_json_endpoint "/schedule" "Endpoint 5: /schedule" || \
    test_http_endpoint "/schedule" "Endpoint 5: /schedule" "200" || \
    log_warning "Endpoint 5 (/schedule) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ ETL-PIPELINE PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ ETL-PIPELINE PROBE FAILED${NC}"
    echo ""
    exit 1
fi
