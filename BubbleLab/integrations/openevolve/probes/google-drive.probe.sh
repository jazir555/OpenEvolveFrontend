#!/bin/bash
# google-drive.probe.sh - Runtime Validation Probe for google-drive
#
# Comprehensive probe script to validate google-drive service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/google-drive.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 GOOGLE-DRIVE SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /drive/v3/about
if [[ "/drive/v3/about" == "/"* ]]; then
    test_json_endpoint "/drive/v3/about" "Endpoint 1: /drive/v3/about" || \
    test_http_endpoint "/drive/v3/about" "Endpoint 1: /drive/v3/about" "200" || \
    log_warning "Endpoint 1 (/drive/v3/about) not available"
else
    log_info "Command check: /drive/v3/about"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /drive/v3/files
if [[ "/drive/v3/files" == "/"* ]]; then
    test_json_endpoint "/drive/v3/files" "Endpoint 2: /drive/v3/files" || \
    test_http_endpoint "/drive/v3/files" "Endpoint 2: /drive/v3/files" "200" || \
    log_warning "Endpoint 2 (/drive/v3/files) not available"
else
    log_info "Command check: /drive/v3/files"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /drive/v3/changes
if [[ "/drive/v3/changes" == "/"* ]]; then
    test_json_endpoint "/drive/v3/changes" "Endpoint 3: /drive/v3/changes" || \
    test_http_endpoint "/drive/v3/changes" "Endpoint 3: /drive/v3/changes" "200" || \
    log_warning "Endpoint 3 (/drive/v3/changes) not available"
else
    log_info "Command check: /drive/v3/changes"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /drive/v3/teamdrives
if [[ "/drive/v3/teamdrives" == "/"* ]]; then
    test_json_endpoint "/drive/v3/teamdrives" "Endpoint 4: /drive/v3/teamdrives" || \
    test_http_endpoint "/drive/v3/teamdrives" "Endpoint 4: /drive/v3/teamdrives" "200" || \
    log_warning "Endpoint 4 (/drive/v3/teamdrives) not available"
else
    log_info "Command check: /drive/v3/teamdrives"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ GOOGLE-DRIVE PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ GOOGLE-DRIVE PROBE FAILED${NC}"
    echo ""
    exit 1
fi
