#!/bin/bash
# hephaestus.probe.sh - Runtime Validation Probe for hephaestus
#
# Comprehensive probe script to validate hephaestus service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/hephaestus.probe.sh
...
# Test 2.$((i + 1)): /v1/capabilities
if [[ "/v1/capabilities" == "/"* ]]; then
    test_json_endpoint "/v1/capabilities" "Endpoint 2: /v1/capabilities" || \
    test_http_endpoint "/v1/capabilities" "Endpoint 2: /v1/capabilities" "200" || \
    log_warning "Endpoint 2 (/v1/capabilities) not available"
else
    log_info "Command check: /v1/capabilities"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [[ "/v1/workflows" == "/"* ]]; then
    test_json_endpoint "/v1/workflows" "Endpoint 3: /v1/workflows" || \
    test_http_endpoint "/v1/workflows" "Endpoint 3: /v1/workflows" "200" || \
    log_warning "Endpoint 3 (/v1/workflows) not available"
else
...
# Test 2.$((i + 1)): /v1/version
if [[ "/v1/version" == "/"* ]]; then
    test_json_endpoint "/v1/version" "Endpoint 5: /v1/version" || \
    test_http_endpoint "/v1/version" "Endpoint 5: /v1/version" "200" || \
    log_warning "Endpoint 5 (/v1/version) not available"
else
    log_info "Command check: /v1/version"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ HEPHAESTUS PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ HEPHAESTUS PROBE FAILED${NC}"
    echo ""
    exit 1
fi
