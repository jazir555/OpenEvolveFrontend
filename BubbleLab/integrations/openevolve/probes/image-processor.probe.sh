#!/bin/bash
# image-processor.probe.sh - Runtime Validation Probe for image-processor
#
# Comprehensive probe script to validate image-processor service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/image-processor.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 IMAGE-PROCESSOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /resize
if [[ "/resize" == "/"* ]]; then
    test_json_endpoint "/resize" "Endpoint 1: /resize" || \
    test_http_endpoint "/resize" "Endpoint 1: /resize" "200" || \
    log_warning "Endpoint 1 (/resize) not available"
else
    log_info "Command check: /resize"
...
# Test 2.$((i + 1)): /crop
if [[ "/crop" == "/"* ]]; then
    test_json_endpoint "/crop" "Endpoint 2: /crop" || \
    test_http_endpoint "/crop" "Endpoint 2: /crop" "200" || \
    log_warning "Endpoint 2 (/crop) not available"
else
    log_info "Command check: /crop"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /rotate
if [[ "/rotate" == "/"* ]]; then
    test_json_endpoint "/rotate" "Endpoint 3: /rotate" || \
    test_http_endpoint "/rotate" "Endpoint 3: /rotate" "200" || \
    log_warning "Endpoint 3 (/rotate) not available"
else
    log_info "Command check: /rotate"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /filter
if [[ "/filter" == "/"* ]]; then
    test_json_endpoint "/filter" "Endpoint 4: /filter" || \
    test_http_endpoint "/filter" "Endpoint 4: /filter" "200" || \
    log_warning "Endpoint 4 (/filter) not available"
else
...
# Test 2.$((i + 1)): /optimize
if [[ "/optimize" == "/"* ]]; then
    test_json_endpoint "/optimize" "Endpoint 5: /optimize" || \
    test_http_endpoint "/optimize" "Endpoint 5: /optimize" "200" || \
    log_warning "Endpoint 5 (/optimize) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ IMAGE-PROCESSOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ IMAGE-PROCESSOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
