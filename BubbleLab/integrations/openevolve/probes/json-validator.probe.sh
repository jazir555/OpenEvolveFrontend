#!/bin/bash
# json-validator.probe.sh - Runtime Validation Probe for json-validator
#
# Comprehensive probe script to validate json-validator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/json-validator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 JSON-VALIDATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /format
if [[ "/format" == "/"* ]]; then
    test_json_endpoint "/format" "Endpoint 2: /format" || \
    test_http_endpoint "/format" "Endpoint 2: /format" "200" || \
    log_warning "Endpoint 2 (/format) not available"
else
    log_info "Command check: /format"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /minify
if [[ "/minify" == "/"* ]]; then
    test_json_endpoint "/minify" "Endpoint 3: /minify" || \
    test_http_endpoint "/minify" "Endpoint 3: /minify" "200" || \
    log_warning "Endpoint 3 (/minify) not available"
else
    log_info "Command check: /minify"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [[ "/transform" == "/"* ]]; then
    test_json_endpoint "/transform" "Endpoint 4: /transform" || \
    test_http_endpoint "/transform" "Endpoint 4: /transform" "200" || \
    log_warning "Endpoint 4 (/transform) not available"
else
...
# Test 2.$((i + 1)): /compare
if [[ "/compare" == "/"* ]]; then
    test_json_endpoint "/compare" "Endpoint 5: /compare" || \
    test_http_endpoint "/compare" "Endpoint 5: /compare" "200" || \
    log_warning "Endpoint 5 (/compare) not available"
else
    log_info "Command check: /compare"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ JSON-VALIDATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ JSON-VALIDATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
