#!/bin/bash
# metrics-collector.probe.sh - Runtime Validation Probe for metrics-collector
#
# Comprehensive probe script to validate metrics-collector service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/metrics-collector.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 METRICS-COLLECTOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /metrics
if [[ "/metrics" == "/"* ]]; then
    test_json_endpoint "/metrics" "Endpoint 1: /metrics" || \
    test_http_endpoint "/metrics" "Endpoint 1: /metrics" "200" || \
    log_warning "Endpoint 1 (/metrics) not available"
else
    log_info "Command check: /metrics"
...
# Test 2.$((i + 1)): /collect
if [[ "/collect" == "/"* ]]; then
    test_json_endpoint "/collect" "Endpoint 2: /collect" || \
    test_http_endpoint "/collect" "Endpoint 2: /collect" "200" || \
    log_warning "Endpoint 2 (/collect) not available"
else
    log_info "Command check: /collect"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /query
if [[ "/query" == "/"* ]]; then
    test_json_endpoint "/query" "Endpoint 3: /query" || \
    test_http_endpoint "/query" "Endpoint 3: /query" "200" || \
    log_warning "Endpoint 3 (/query) not available"
else
    log_info "Command check: /query"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /export
if [[ "/export" == "/"* ]]; then
    test_json_endpoint "/export" "Endpoint 4: /export" || \
    test_http_endpoint "/export" "Endpoint 4: /export" "200" || \
    log_warning "Endpoint 4 (/export) not available"
else
    log_info "Command check: /export"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [[ "/aggregate" == "/"* ]]; then
    test_json_endpoint "/aggregate" "Endpoint 5: /aggregate" || \
    test_http_endpoint "/aggregate" "Endpoint 5: /aggregate" "200" || \
    log_warning "Endpoint 5 (/aggregate) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ METRICS-COLLECTOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ METRICS-COLLECTOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
