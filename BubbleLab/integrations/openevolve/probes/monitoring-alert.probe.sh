#!/bin/bash
# monitoring-alert.probe.sh - Runtime Validation Probe for monitoring-alert
#
# Comprehensive probe script to validate monitoring-alert service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/monitoring-alert.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 MONITORING-ALERT SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /monitor
if [[ "/monitor" == "/"* ]]; then
    test_json_endpoint "/monitor" "Endpoint 1: /monitor" || \
    test_http_endpoint "/monitor" "Endpoint 1: /monitor" "200" || \
    log_warning "Endpoint 1 (/monitor) not available"
else
    log_info "Command check: /monitor"
...
# Test 2.$((i + 1)): /alert
if [[ "/alert" == "/"* ]]; then
    test_json_endpoint "/alert" "Endpoint 2: /alert" || \
    test_http_endpoint "/alert" "Endpoint 2: /alert" "200" || \
    log_warning "Endpoint 2 (/alert) not available"
else
    log_info "Command check: /alert"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /threshold
if [[ "/threshold" == "/"* ]]; then
    test_json_endpoint "/threshold" "Endpoint 3: /threshold" || \
    test_http_endpoint "/threshold" "Endpoint 3: /threshold" "200" || \
    log_warning "Endpoint 3 (/threshold) not available"
else
    log_info "Command check: /threshold"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /history
if [[ "/history" == "/"* ]]; then
    test_json_endpoint "/history" "Endpoint 4: /history" || \
    test_http_endpoint "/history" "Endpoint 4: /history" "200" || \
    log_warning "Endpoint 4 (/history) not available"
else
...
# Test 2.$((i + 1)): /rules
if [[ "/rules" == "/"* ]]; then
    test_json_endpoint "/rules" "Endpoint 5: /rules" || \
    test_http_endpoint "/rules" "Endpoint 5: /rules" "200" || \
    log_warning "Endpoint 5 (/rules) not available"
else
    log_info "Command check: /rules"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ MONITORING-ALERT PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ MONITORING-ALERT PROBE FAILED${NC}"
    echo ""
    exit 1
fi
