#!/bin/bash
# multi-step-approval.probe.sh - Runtime Validation Probe for multi-step-approval
#
# Comprehensive probe script to validate multi-step-approval service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/multi-step-approval.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 MULTI-STEP-APPROVAL SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /create
if [[ "/create" == "/"* ]]; then
    test_json_endpoint "/create" "Endpoint 1: /create" || \
    test_http_endpoint "/create" "Endpoint 1: /create" "200" || \
    log_warning "Endpoint 1 (/create) not available"
else
    log_info "Command check: /create"
...
# Test 2.$((i + 1)): /approve
if [[ "/approve" == "/"* ]]; then
    test_json_endpoint "/approve" "Endpoint 2: /approve" || \
    test_http_endpoint "/approve" "Endpoint 2: /approve" "200" || \
    log_warning "Endpoint 2 (/approve) not available"
else
    log_info "Command check: /approve"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /reject
if [[ "/reject" == "/"* ]]; then
    test_json_endpoint "/reject" "Endpoint 3: /reject" || \
    test_http_endpoint "/reject" "Endpoint 3: /reject" "200" || \
    log_warning "Endpoint 3 (/reject) not available"
else
    log_info "Command check: /reject"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /status
if [[ "/status" == "/"* ]]; then
    test_json_endpoint "/status" "Endpoint 4: /status" || \
    test_http_endpoint "/status" "Endpoint 4: /status" "200" || \
    log_warning "Endpoint 4 (/status) not available"
else
    log_info "Command check: /status"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ MULTI-STEP-APPROVAL PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ MULTI-STEP-APPROVAL PROBE FAILED${NC}"
    echo ""
    exit 1
fi
