#!/bin/bash
# notion.probe.sh - Runtime Validation Probe for notion
#
# Comprehensive probe script to validate notion service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/notion.probe.sh
...
# Test 2.$((i + 1)): /v1/users/me
if [[ "/v1/users/me" == "/"* ]]; then
    test_json_endpoint "/v1/users/me" "Endpoint 1: /v1/users/me" || \
    test_http_endpoint "/v1/users/me" "Endpoint 1: /v1/users/me" "200" || \
    log_warning "Endpoint 1 (/v1/users/me) not available"
else
    log_info "Command check: /v1/users/me"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/search
if [[ "/v1/search" == "/"* ]]; then
    test_json_endpoint "/v1/search" "Endpoint 2: /v1/search" || \
    test_http_endpoint "/v1/search" "Endpoint 2: /v1/search" "200" || \
    log_warning "Endpoint 2 (/v1/search) not available"
else
    log_info "Command check: /v1/search"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/databases
if [[ "/v1/databases" == "/"* ]]; then
    test_json_endpoint "/v1/databases" "Endpoint 3: /v1/databases" || \
    test_http_endpoint "/v1/databases" "Endpoint 3: /v1/databases" "200" || \
    log_warning "Endpoint 3 (/v1/databases) not available"
else
    log_info "Command check: /v1/databases"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /v1/pages
if [[ "/v1/pages" == "/"* ]]; then
    test_json_endpoint "/v1/pages" "Endpoint 4: /v1/pages" || \
    test_http_endpoint "/v1/pages" "Endpoint 4: /v1/pages" "200" || \
    log_warning "Endpoint 4 (/v1/pages) not available"
else
    log_info "Command check: /v1/pages"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/blocks
if [[ "/v1/blocks" == "/"* ]]; then
    test_json_endpoint "/v1/blocks" "Endpoint 5: /v1/blocks" || \
    test_http_endpoint "/v1/blocks" "Endpoint 5: /v1/blocks" "200" || \
    log_warning "Endpoint 5 (/v1/blocks) not available"
else
    log_info "Command check: /v1/blocks"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ NOTION PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ NOTION PROBE FAILED${NC}"
    echo ""
    exit 1
fi
