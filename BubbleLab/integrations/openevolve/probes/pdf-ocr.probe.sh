#!/bin/bash
# pdf-ocr.probe.sh - Runtime Validation Probe for pdf-ocr
#
# Comprehensive probe script to validate pdf-ocr service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/pdf-ocr.probe.sh
...
# Test 2.$((i + 1)): /process
if [[ "/process" == "/"* ]]; then
    test_json_endpoint "/process" "Endpoint 1: /process" || \
    test_http_endpoint "/process" "Endpoint 1: /process" "200" || \
    log_warning "Endpoint 1 (/process) not available"
else
    log_info "Command check: /process"
...
# Test 2.$((i + 1)): /search
if [[ "/search" == "/"* ]]; then
    test_json_endpoint "/search" "Endpoint 3: /search" || \
    test_http_endpoint "/search" "Endpoint 3: /search" "200" || \
    log_warning "Endpoint 3 (/search) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ PDF-OCR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ PDF-OCR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
