#!/bin/bash
# slack.probe.sh - Runtime Validation Probe for slack
#
# Comprehensive probe script to validate slack service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/slack.probe.sh
...
# Test 2.$((i + 1)): /api/auth.test
if [[ "/api/auth.test" == "/"* ]]; then
    test_json_endpoint "/api/auth.test" "Endpoint 1: /api/auth.test" || \
    test_http_endpoint "/api/auth.test" "Endpoint 1: /api/auth.test" "200" || \
    log_warning "Endpoint 1 (/api/auth.test) not available"
else
    log_info "Command check: /api/auth.test"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /api/conversations.list
if [[ "/api/conversations.list" == "/"* ]]; then
    test_json_endpoint "/api/conversations.list" "Endpoint 2: /api/conversations.list" || \
    test_http_endpoint "/api/conversations.list" "Endpoint 2: /api/conversations.list" "200" || \
    log_warning "Endpoint 2 (/api/conversations.list) not available"
else
    log_info "Command check: /api/conversations.list"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /api/team.info
if [[ "/api/team.info" == "/"* ]]; then
    test_json_endpoint "/api/team.info" "Endpoint 3: /api/team.info" || \
    test_http_endpoint "/api/team.info" "Endpoint 3: /api/team.info" "200" || \
    log_warning "Endpoint 3 (/api/team.info) not available"
else
    log_info "Command check: /api/team.info"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /api/users.list
if [[ "/api/users.list" == "/"* ]]; then
    test_json_endpoint "/api/users.list" "Endpoint 4: /api/users.list" || \
    test_http_endpoint "/api/users.list" "Endpoint 4: /api/users.list" "200" || \
    log_warning "Endpoint 4 (/api/users.list) not available"
else
    log_info "Command check: /api/users.list"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /api/channels.list
if [[ "/api/channels.list" == "/"* ]]; then
    test_json_endpoint "/api/channels.list" "Endpoint 5: /api/channels.list" || \
    test_http_endpoint "/api/channels.list" "Endpoint 5: /api/channels.list" "200" || \
    log_warning "Endpoint 5 (/api/channels.list) not available"
else
    log_info "Command check: /api/channels.list"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ SLACK PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ SLACK PROBE FAILED${NC}"
    echo ""
    exit 1
fi
