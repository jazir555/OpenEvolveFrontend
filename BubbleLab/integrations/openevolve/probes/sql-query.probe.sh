#!/bin/bash
# sql-query.probe.sh - Runtime Validation Probe for sql-query
#
# Comprehensive probe script to validate sql-query service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/sql-query.probe.sh
...
# Test 2.$((i + 1)): /query
if [[ "/query" == "/"* ]]; then
    test_json_endpoint "/query" "Endpoint 1: /query" || \
    test_http_endpoint "/query" "Endpoint 1: /query" "200" || \
    log_warning "Endpoint 1 (/query) not available"
else
...
# Test 2.$((i + 1)): /explain
if [[ "/explain" == "/"* ]]; then
    test_json_endpoint "/explain" "Endpoint 3: /explain" || \
    test_http_endpoint "/explain" "Endpoint 3: /explain" "200" || \
    log_warning "Endpoint 3 (/explain) not available"
else
    log_info "Command check: /explain"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /schema
if [[ "/schema" == "/"* ]]; then
    test_json_endpoint "/schema" "Endpoint 4: /schema" || \
    test_http_endpoint "/schema" "Endpoint 4: /schema" "200" || \
    log_warning "Endpoint 4 (/schema) not available"
else
...
# Test 2.$((i + 1)): /tables
if [[ "/tables" == "/"* ]]; then
    test_json_endpoint "/tables" "Endpoint 5: /tables" || \
    test_http_endpoint "/tables" "Endpoint 5: /tables" "200" || \
    log_warning "Endpoint 5 (/tables) not available"
else
    log_info "Command check: /tables"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ SQL-QUERY PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ SQL-QUERY PROBE FAILED${NC}"
    echo ""
    exit 1
fi
