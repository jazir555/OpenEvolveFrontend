#!/bin/bash
# web-scrape.probe.sh - Runtime Validation Probe for web-scrape
#
# Comprehensive probe script to validate web-scrape service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/web-scrape.probe.sh
...
# Test 2.$((i + 1)): /scrape
if [[ "/scrape" == "/"* ]]; then
    test_json_endpoint "/scrape" "Endpoint 1: /scrape" || \
    test_http_endpoint "/scrape" "Endpoint 1: /scrape" "200" || \
    log_warning "Endpoint 1 (/scrape) not available"
else
    log_info "Command check: /scrape"
...
# Test 2.$((i + 1)): /extract
if [[ "/extract" == "/"* ]]; then
    test_json_endpoint "/extract" "Endpoint 2: /extract" || \
    test_http_endpoint "/extract" "Endpoint 2: /extract" "200" || \
    log_warning "Endpoint 2 (/extract) not available"
else
...
# Test 2.$((i + 1)): /parse
if [[ "/parse" == "/"* ]]; then
    test_json_endpoint "/parse" "Endpoint 3: /parse" || \
    test_http_endpoint "/parse" "Endpoint 3: /parse" "200" || \
    log_warning "Endpoint 3 (/parse) not available"
else
...
# Test 2.$((i + 1)): /validate
if [[ "/validate" == "/"* ]]; then
    test_json_endpoint "/validate" "Endpoint 4: /validate" || \
    test_http_endpoint "/validate" "Endpoint 4: /validate" "200" || \
    log_warning "Endpoint 4 (/validate) not available"
else
...
# Test 2.$((i + 1)): /sitemap
if [[ "/sitemap" == "/"* ]]; then
    test_json_endpoint "/sitemap" "Endpoint 5: /sitemap" || \
    test_http_endpoint "/sitemap" "Endpoint 5: /sitemap" "200" || \
    log_warning "Endpoint 5 (/sitemap) not available"
else
    log_info "Command check: /sitemap"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ WEB-SCRAPE PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ WEB-SCRAPE PROBE FAILED${NC}"
    echo ""
    exit 1
fi
