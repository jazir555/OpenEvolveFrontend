#!/bin/bash
# web-search.probe.sh - Runtime Validation Probe for web-search
#
# Comprehensive probe script to validate web-search service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/web-search.probe.sh
...
# Test 2.$((i + 1)): /news
if [[ "/news" == "/"* ]]; then
    test_json_endpoint "/news" "Endpoint 2: /news" || \
    test_http_endpoint "/news" "Endpoint 2: /news" "200" || \
    log_warning "Endpoint 2 (/news) not available"
else
    log_info "Command check: /news"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /images
if [[ "/images" == "/"* ]]; then
    test_json_endpoint "/images" "Endpoint 3: /images" || \
    test_http_endpoint "/images" "Endpoint 3: /images" "200" || \
    log_warning "Endpoint 3 (/images) not available"
else
    log_info "Command check: /images"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /videos
if [[ "/videos" == "/"* ]]; then
    test_json_endpoint "/videos" "Endpoint 4: /videos" || \
    test_http_endpoint "/videos" "Endpoint 4: /videos" "200" || \
    log_warning "Endpoint 4 (/videos) not available"
else
    log_info "Command check: /videos"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /maps
if [[ "/maps" == "/"* ]]; then
    test_json_endpoint "/maps" "Endpoint 5: /maps" || \
    test_http_endpoint "/maps" "Endpoint 5: /maps" "200" || \
    log_warning "Endpoint 5 (/maps) not available"
else
    log_info "Command check: /maps"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ WEB-SEARCH PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ WEB-SEARCH PROBE FAILED${NC}"
    echo ""
    exit 1
fi
