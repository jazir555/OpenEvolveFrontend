#!/bin/bash
# webhook-repeater.probe.sh - Runtime Validation Probe for webhook-repeater
#
# Comprehensive probe script to validate webhook-repeater service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/webhook-repeater.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 WEBHOOK-REPEATER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /repeat
if [[ "/repeat" == "/"* ]]; then
    test_json_endpoint "/repeat" "Endpoint 1: /repeat" || \
    test_http_endpoint "/repeat" "Endpoint 1: /repeat" "200" || \
    log_warning "Endpoint 1 (/repeat) not available"
else
    log_info "Command check: /repeat"
...
# Test 2.$((i + 1)): /schedule
if [[ "/schedule" == "/"* ]]; then
    test_json_endpoint "/schedule" "Endpoint 2: /schedule" || \
    test_http_endpoint "/schedule" "Endpoint 2: /schedule" "200" || \
    log_warning "Endpoint 2 (/schedule) not available"
else
    log_info "Command check: /schedule"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /transform
if [[ "/transform" == "/"* ]]; then
    test_json_endpoint "/transform" "Endpoint 3: /transform" || \
    test_http_endpoint "/transform" "Endpoint 3: /transform" "200" || \
    log_warning "Endpoint 3 (/transform) not available"
else
    log_info "Command check: /transform"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /aggregate
if [[ "/aggregate" == "/"* ]]; then
    test_json_endpoint "/aggregate" "Endpoint 4: /aggregate" || \
    test_http_endpoint "/aggregate" "Endpoint 4: /aggregate" "200" || \
    log_warning "Endpoint 4 (/aggregate) not available"
else
    log_info "Command check: /aggregate"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /history
if [[ "/history" == "/"* ]]; then
    test_json_endpoint "/history" "Endpoint 5: /history" || \
    test_http_endpoint "/history" "Endpoint 5: /history" "200" || \
    log_warning "Endpoint 5 (/history) not available"
else
    log_info "Command check: /history"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ WEBHOOK-REPEATER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ WEBHOOK-REPEATER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
