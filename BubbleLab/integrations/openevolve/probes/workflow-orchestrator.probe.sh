#!/bin/bash
# workflow-orchestrator.probe.sh - Runtime Validation Probe for workflow-orchestrator
#
# Comprehensive probe script to validate workflow-orchestrator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/workflow-orchestrator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 WORKFLOW-ORCHESTRATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /health
if [[ "/health" == "/"* ]]; then
    test_json_endpoint "/health" "Endpoint 1: /health" || \
    test_http_endpoint "/health" "Endpoint 1: /health" "200" || \
    log_warning "Endpoint 1 (/health) not available"
else
    log_info "Command check: /health"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/workflows
if [[ "/v1/workflows" == "/"* ]]; then
    test_json_endpoint "/v1/workflows" "Endpoint 2: /v1/workflows" || \
    test_http_endpoint "/v1/workflows" "Endpoint 2: /v1/workflows" "200" || \
    log_warning "Endpoint 2 (/v1/workflows) not available"
else
    log_info "Command check: /v1/workflows"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/executions
if [[ "/v1/executions" == "/"* ]]; then
    test_json_endpoint "/v1/executions" "Endpoint 3: /v1/executions" || \
    test_http_endpoint "/v1/executions" "Endpoint 3: /v1/executions" "200" || \
    log_warning "Endpoint 3 (/v1/executions) not available"
else
    log_info "Command check: /v1/executions"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/status
if [[ "/v1/status" == "/"* ]]; then
    test_json_endpoint "/v1/status" "Endpoint 4: /v1/status" || \
    test_http_endpoint "/v1/status" "Endpoint 4: /v1/status" "200" || \
    log_warning "Endpoint 4 (/v1/status) not available"
else
    log_info "Command check: /v1/status"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/schedules
if [[ "/v1/schedules" == "/"* ]]; then
    test_json_endpoint "/v1/schedules" "Endpoint 5: /v1/schedules" || \
    test_http_endpoint "/v1/schedules" "Endpoint 5: /v1/schedules" "200" || \
    log_warning "Endpoint 5 (/v1/schedules) not available"
else
    log_info "Command check: /v1/schedules"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ WORKFLOW-ORCHESTRATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ WORKFLOW-ORCHESTRATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
