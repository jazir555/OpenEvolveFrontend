#!/bin/bash
# xml-parser.probe.sh - Runtime Validation Probe for xml-parser
#
# Comprehensive probe script to validate xml-parser service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/xml-parser.probe.sh
...
# Test 2.$((i + 1)): /query
if [[ "/query" == "/"* ]]; then
    test_json_endpoint "/query" "Endpoint 4: /query" || \
    test_http_endpoint "/query" "Endpoint 4: /query" "200" || \
    log_warning "Endpoint 4 (/query) not available"
else
...
# Test 2.$((i + 1)): /format
if [[ "/format" == "/"* ]]; then
    test_json_endpoint "/format" "Endpoint 5: /format" || \
    test_http_endpoint "/format" "Endpoint 5: /format" "200" || \
    log_warning "Endpoint 5 (/format) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ XML-PARSER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ XML-PARSER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
