/**
 * DataTransformerTool Test Suite
 *
 * Comprehensive tests for data-transformer tool bubble
 *
 * Test Coverage:
...
import { DataTransformerTool } from '../tool-bubbles/data-transformer-tool';
...
describe('DataTransformerTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new DataTransformerTool({
        operation: 'test',
        data: '[]'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('DataTransformerTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(DataTransformerTool.service).toBe('openevolve');
      expect(DataTransformerTool.bubbleName).toBe('data-transformer');
      expect(DataTransformerTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new DataTransformerTool({
        operation: 'test1',
        data: '[]'
      } as any);

      const bubble2 = new DataTransformerTool({
        operation: 'test2',
        data: '[]'
      } as any);
