/**
 * EmailValidatorTool Test Suite
 *
 * Comprehensive tests for email-validator tool bubble
 *
 * Test Coverage:
...
import { EmailValidatorTool } from '../tool-bubbles/email-validator-tool';
...
describe('EmailValidatorTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new EmailValidatorTool({
        operation: 'test',
        email: 'test@test.com'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('EmailValidatorTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(EmailValidatorTool.service).toBe('openevolve');
      expect(EmailValidatorTool.bubbleName).toBe('email-validator');
      expect(EmailValidatorTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new EmailValidatorTool({
        operation: 'test1',
        email: 'test@test.com'
      } as any);

      const bubble2 = new EmailValidatorTool({
        operation: 'test2',
        email: 'test@test.com'
      } as any);
