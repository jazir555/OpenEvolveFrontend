/**
 * ImageProcessorTool Test Suite
 *
 * Comprehensive tests for image-processor tool bubble
 *
 * Test Coverage:
...
import { ImageProcessorTool } from '../tool-bubbles/image-processor-tool';
...
describe('ImageProcessorTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new ImageProcessorTool({
        operation: 'test',
        image: '/tmp/test.jpg'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('ImageProcessorTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(ImageProcessorTool.service).toBe('openevolve');
      expect(ImageProcessorTool.bubbleName).toBe('image-processor');
      expect(ImageProcessorTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new ImageProcessorTool({
        operation: 'test1',
        image: '/tmp/test.jpg'
      } as any);

      const bubble2 = new ImageProcessorTool({
        operation: 'test2',
        image: '/tmp/test.jpg'
      } as any);
