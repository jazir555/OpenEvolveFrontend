/**
 * PDFOCRWorkflow Test Suite
 *
 * Comprehensive tests for pdf-ocr workflow bubble
 *
 * Test Coverage:
...
import { PDFOCRWorkflow } from '../workflow-bubbles/pdf-ocr-workflow';
...
describe('PDFOCRWorkflow', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new PDFOCRWorkflow({
        operation: 'test',
        pdf: '/tmp/test.pdf'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('PDFOCRWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(PDFOCRWorkflow.service).toBe('openevolve');
      expect(PDFOCRWorkflow.bubbleName).toBe('pdf-ocr');
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new PDFOCRWorkflow({
        operation: 'test1',
        pdf: '/tmp/test.pdf'
      } as any);

      const bubble2 = new PDFOCRWorkflow({
        operation: 'test2',
        pdf: '/tmp/test.pdf'
      } as any);
