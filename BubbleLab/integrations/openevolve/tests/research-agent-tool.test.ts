/**
 * ResearchAgentTool Test Suite
 *
 * Comprehensive tests for research-agent tool bubble
 *
 * Test Coverage:
...
import { ResearchAgentTool } from '../tool-bubbles/research-agent-tool';
...
describe('ResearchAgentTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new ResearchAgentTool({
        operation: 'test',
        topic: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('ResearchAgentTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(ResearchAgentTool.service).toBe('openevolve');
      expect(ResearchAgentTool.bubbleName).toBe('research-agent');
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new ResearchAgentTool({
        operation: 'test1',
        topic: 'test'
      } as any);

      const bubble2 = new ResearchAgentTool({
        operation: 'test2',
        topic: 'test'
      } as any);
