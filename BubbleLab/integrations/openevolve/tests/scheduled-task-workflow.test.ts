/**
 * ScheduledTaskWorkflow Test Suite
 *
 * Comprehensive tests for scheduled-task workflow bubble
 *
 * Test Coverage:
...
import { ScheduledTaskWorkflow } from '../workflow-bubbles/scheduled-task-workflow';

describe('ScheduledTaskWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new ScheduledTaskWorkflow({
        operation: 'test',
        task: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('ScheduledTaskWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(ScheduledTaskWorkflow.service).toBe('openevolve');
      expect(ScheduledTaskWorkflow.bubbleName).toBe('scheduled-task');
      expect(ScheduledTaskWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new ScheduledTaskWorkflow({
        operation: 'test1',
        task: 'test'
      } as any);

      const bubble2 = new ScheduledTaskWorkflow({
        operation: 'test2',
        task: 'test'
      } as any);
