/**
 * SQLQueryTool Test Suite
 *
 * Comprehensive tests for sql-query tool bubble
 *
 * Test Coverage:
...
import { SQLQueryTool } from '../tool-bubbles/sql-query-tool';
...
describe('SQLQueryTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new SQLQueryTool({
        operation: 'test',
        query: 'SELECT 1'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('SQLQueryTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(SQLQueryTool.service).toBe('openevolve');
      expect(SQLQueryTool.bubbleName).toBe('sql-query');
      expect(SQLQueryTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new SQLQueryTool({
        operation: 'test1',
        query: 'SELECT 1'
      } as any);

      const bubble2 = new SQLQueryTool({
        operation: 'test2',
        query: 'SELECT 1'
      } as any);
