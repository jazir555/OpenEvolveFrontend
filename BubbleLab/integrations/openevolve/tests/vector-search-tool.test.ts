/**
 * VectorSearchTool Test Suite
 *
 * Comprehensive tests for vector-search tool bubble
 *
 * Test Coverage:
...
import { VectorSearchTool } from '../tool-bubbles/vector-search-tool';
...
describe('VectorSearchTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new VectorSearchTool({
        operation: 'test',
        vector: '[0.1]'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('VectorSearchTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(VectorSearchTool.service).toBe('openevolve');
      expect(VectorSearchTool.bubbleName).toBe('vector-search');
      expect(VectorSearchTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new VectorSearchTool({
        operation: 'test1',
        vector: '[0.1]'
      } as any);

      const bubble2 = new VectorSearchTool({
        operation: 'test2',
        vector: '[0.1]'
      } as any);
