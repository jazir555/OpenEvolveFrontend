...
export function validateUrl(url, allowedProtocols = ['http', 'https']) {
    ...
    try {
        ...
        if (!allowedProtocols.includes(parsedUrl.protocol.replace(':', ''))) {
            throw new ValidationError(`URL protocol must be one of: ${allowedProtocols.join(', ')}`, 'url');
        }
        return true;
    }
    catch (error) {
...
export function validateNumberRange(value, min, max, fieldName = 'value') {
    ...
    if (value < min || value > max) {
        throw new ValidationError(`${fieldName} must be between ${min} and ${max} (inclusive)`, fieldName);
    }
    return true;
}
/**
 ...
 */
export function validateArrayLength(array, minLength = 0, maxLength, fieldName = 'array') {
    if (!Array.isArray(array)) {
        throw new ValidationError(`${fieldName} must be an array`, fieldName);
    }
    if (array.length < minLength) {
        throw new ValidationError(`${fieldName} must contain at least ${minLength} item(s)`, fieldName);
    }
    if (array.length > maxLength) {
        throw new ValidationError(`${fieldName} cannot contain more than ${maxLength} item(s)`, fieldName);
    }
    return true;
}
/**
 ...
 */
export function validateRequiredProperties(obj, requiredProps, fieldName = 'object') {
    ...
    if (missingProps.length > 0) {
        throw new ValidationError(`${fieldName} is missing required properties: ${missingProps.join(', ')}`, fieldName);
    }
    return true;
}
/**
 ...
 */
export function validateFilePath(filePath, allowAbsolutePaths = false) {
 ...
 * @returns Zod schema
 */
export function createNonEmptyStringSchema(fieldName = 'value') {
    return z.string()
 ...
 * @returns Zod schema
 */
export function createUrlSchema(allowedProtocols = ['http', 'https']) {
    return z.string()
