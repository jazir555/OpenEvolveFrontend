...
import { ResilienceWrapper, DEFAULT_RESILIENCE_CONFIG, isTransientError, } from '../../__mocks__/resilience.js';
// ============================================================================
// SECURITY UTILITIES
// ============================================================================
/**
 ...
 */
function validateUrl(url, allowPrivateRanges = false) {
    try {
...
export class ApifyBubble extends ServiceBubble {
    ...
    static schema = ApifyBubbleParamsSchema;
    static resultSchema = ApifyBubbleResultSchema;
    static shortDescription = 'Web scraping and automation platform';
  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            const result = await this.resilience.execute(`apify-${operation}-${JSON.stringify(this.params)}`, async () => {
                switch (operation) {
                    case 'runActor':
                        return await this.runActor(this.params);
                    case 'getActor':
                        return await this.getActor(this.params);
                    case 'listActors':
                        return await this.listActors(this.params);
                    case 'buildActor':
                        return await this.buildActor(this.params);
                    case 'getRun':
                        return await this.getRun(this.params);
                    case 'waitForRun':
                        return await this.waitForRun(this.params);
                    case 'stopRun':
                        return await this.stopRun(this.params);
                    case 'listRuns':
                        return await this.listRuns(this.params);
                    case 'getDataset':
                        return await this.getDataset(this.params);
                    case 'getDatasetItems':
                        return await this.getDatasetItems(this.params);
                    case 'downloadDataset':
                        return await this.downloadDataset(this.params);
                    case 'webScrape':
                        return await this.webScrape(this.params);
                    case 'crawlWebsite':
                        return await this.crawlWebsite(this.params);
                    default:
    ...
    async buildActor(params) {
        ...
        try {
            ...
            const body = {};
            if (buildTag)
                body.buildTag = buildTag;
            if (version)
                body.version = version;
            if (waitForFinish)
                body.waitForFinish = 300; // Wait up to 5 minutes
    ...
    // HELPER METHODS
    // ========================================================================
    async waitForCompletion(runId, timeoutSecs) {
        const startTime = Date.now();
...
//# sourceMappingURL=apify-bubble.js.map