...
const ApifyParamsSchema = z.object({
    actorId: z
        .string()
        .optional()
        .describe('The Apify actor to run. Examples: "apify/instagram-scraper", "apify/reddit-scraper", etc. Required when running an actor, not needed for discovery mode.'),
    search: z
        .string()
        .optional()
        .describe('Search query to discover available Apify actors. When provided, this triggers discovery mode to search for actors matching the query and return their schemas and information.'),
    limit: z
        .number()
        .min(1)
        .max(100)
        .optional()
        .default(20)
        .describe('Maximum number of actors to return in discovery mode (default: 20, max: 100)'),
    input: z
        .record(z.unknown())
        .describe('Input parameters for the actor. Structure depends on the specific actor being used. Not used in discovery mode.'),
    waitForFinish: z
        .boolean()
        .optional()
        .default(true)
...
const ApifyResultSchema = z.object({
    ...
    items: z
        .array(z.unknown())
        .optional()
        .describe('Array of scraped items (if waitForFinish is true). Structure depends on the actor. For discovery mode, contains actor information with schemas.'),
    ...
    discoveredActors: z
        ...
        inputSchemaUrl: z
            .string()
            .describe('URL to the actor input schema page. Use the web scrape tool to scrape from this URL (e.g., https://apify.com/apify/google-search-scraper/input-schema) to get the input/output schema details.'),
        stars: z
            .number()
            .nullable()
            .optional()
        ...
        requiresRental: z
            .boolean()
            .optional()
            .describe('Whether this actor requires rental/private access (filtered out when true)'),
    }))
        .optional()
        .describe('Discovered actors with description and input schema URL (only present in discovery mode)'),
});
/**
...
export class ApifyBubble extends ServiceBubble {
    ...
    static shortDescription = 'Discover and run specialized Apify actors for complex web scraping tasks not covered by standard tools';
  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            const { actorId, search, limit, input, waitForFinish, timeout } = this.params;
            // Discovery mode: search for actors when search parameter is provided
            if (search) {
                return (await this.discoverActors(apiToken, search, limit || 20));
            }
            ...
            // Start the actor run
            const runResponse = await this.startActorRun(apiToken, actorId, input, limit, waitForFinish, timeout);
    ...
    async startActorRun(apiToken, actorId, input, limit, waitForFinish, timeout) {
     ...
     */
    async discoverActors(apiToken, query, limit) {
        try {
            ...
            if (!searchResponse.ok) {
                throw new Error(`Failed to search actors: ${searchResponse.status} - ${await searchResponse.text()}`);
            }
            const searchData = (await searchResponse.json());
            const actors = searchData.data?.items || [];
            // Build actor information with input schema URL, skipping rental/private actors
            const discoveredActors = (await Promise.all(actors.map(async (actor) => {
                try {
                    ...
                    try {
                        ...
                        if (detailResp.ok) {
                            ...
                            if (detailData.data?.pricingInfos?.some((p) => p.pricingModel === 'FLAT_PRICE_PER_MONTH')) {
                                requiresRental = true;
                            }
                        }
                    }
