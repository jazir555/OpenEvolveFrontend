...
export class GoogleDriveBubble extends ServiceBubble {
    ...
    static type = 'service';
    static schema = GoogleDriveBubbleParamsSchema;
    static resultSchema = GoogleDriveBubbleResultSchema;
    static shortDescription = 'Cloud file storage and synchronization service';
  ...
  `;
    ...
    logger = createLogger('GoogleDriveBubble');
    ...
    validateFileSize(content) {
        ...
        if (size > MAX_FILE_SIZE) {
            throw new Error(`File size (${Math.round(size / 1024 / 1024)}MB) exceeds maximum allowed size (${MAX_FILE_SIZE / 1024 / 1024 / 1024}GB)`);
        }
    }
    getCredentialType() {
    ...
    async uploadFile() {
        ...
        const mediaPart = 'Content-Type: ' + (params.mimeType || 'application/octet-stream') + '\r\n\r\n';
    ...
    async downloadFile() {
        ...
        if (metadata.mimeType.startsWith('application/vnd.google-apps')) {
            ...
            const exportResponse = await fetch(`${this.baseUrl}/files/${params.fileId}/export?mimeType=${encodeURIComponent(exportMimeType)}`, {
                headers: {
    ...
    async getFileInfo() {
        const params = this.params;
        const result = await this.makeRequest('GET', `/files/${params.fileId}?fields=id,name,mimeType,createdTime,modifiedTime,size,owners,parents,permissions,webContentLink,webViewLink`);
    ...
    async updateFile() {
        ...
        const response = await fetch(`${this.uploadUrl}/files/${params.fileId}?uploadType=media`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    async getPermissions() {
        const params = this.params;
        const result = await this.makeRequest('GET', `/files/${params.fileId}/permissions?fields=id,type,role,emailAddress,displayName,photoLink,expirationTime,deleted`);
    ...
    async revokeAccess() {
        const params = this.params;
        await this.makeRequest('DELETE', `/files/${params.fileId}/permissions/${params.permissionId}`);
    ...
    async updateMetadata() {
        ...
        const result = await this.makeRequest('PATCH', `/files/${params.fileId}`, metadata);
...
//# sourceMappingURL=google-drive-bubble.js.map