...
const GoogleDriveParamsSchema = z.discriminatedUnion('operation', [
    ...
    z.object({
        operation: z
            .literal('move_file')
            .describe('Move a file or folder to a different location in Google Drive'),
        file_id: z
            .string()
            .min(1, 'File ID is required')
        ...
        remove_parent_folder_id: z
            .string()
            .optional()
            .describe('ID of the parent folder to remove (removes from all parents if not provided)'),
        credentials: z
            .record(z.nativeEnum(CredentialType), z.string())
            .optional()
            .describe('Object mapping credential types to values (injected at runtime)'),
...
const GoogleDriveResultSchema = z.discriminatedUnion('operation', [
    ...
    z.object({
        ...
        file: DriveFileSchema.optional().describe('Updated file metadata after move'),
        error: z.string().describe('Error message if operation failed'),
    }),
]);
/**
...
export class GoogleDriveBubble extends ServiceBubble {
  ...
  `;
    ...
    async testCredential() {
        ...
        if (!credential) {
            throw new AuthenticationError('Google Drive credentials are required');
        }
        try {
    ...
    async makeGoogleApiRequest(endpoint, method = 'GET', body, headers = {}, responseType = 'auto') {
        ...
        if (!response.ok) {
            ...
            throw new ExternalServiceError('google-drive', `API error: ${response.status} ${response.statusText}`, String(response.status), { error: errorText });
        }
    ...
    async moveFile(params) {
        ...
        const currentFileInfo = await this.makeGoogleApiRequest(`/files/${file_id}?fields=parents`);
        const currentParents = currentFileInfo.parents || [];
        ...
        queryParams.set('fields', 'id,name,mimeType,size,createdTime,modifiedTime,webViewLink,webContentLink,parents,shared,owners');
        // Make the update request
        const response = await this.makeGoogleApiRequest(`/files/${file_id}?${queryParams.toString()}`, 'PATCH', {});
        return {
            operation: 'move_file',
            success: true,
            file: response,
