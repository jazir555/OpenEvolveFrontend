...
export class SendGridBubble extends ServiceBubble {
    static service = 'sendgrid';
    static authType = 'apikey';
    static bubbleName = 'sendgrid';
    static type = 'service';
    static schema = SendGridBubbleParamsSchema;
    static resultSchema = SendGridBubbleResultSchema;
    static shortDescription = 'Email delivery and marketing automation platform';
  ...
  `;
    ...
    async sendBulkEmails() {
        const params = this.params;
        const responses = await sgMail.send(params.messages);
    ...
    async getContact() {
        const params = this.params;
        const apiKey = this.getApiKey();
        const response = await fetch(`https://api.sendgrid.com/v3/marketing/contacts/emails/${params.email}`, {
            headers: {
                'Authorization': `Bearer ${apiKey}`,
            },
        });
...
//# sourceMappingURL=sendgrid-bubble.js.map