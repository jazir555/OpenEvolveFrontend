...
class StripeClient {
     ...
     * @throws ExternalServiceError if the API request fails
     */
    async postJson(endpoint, params) {
        const url = `${this.baseUrl}/${endpoint}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
...
export class StripeBubble extends ServiceBubble {
    ...
    static schema = StripeBubbleParamsSchema;
    static resultSchema = StripeBubbleResultSchema;
    static shortDescription = 'Complete Stripe integration for payments and billing';
  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            const result = await this.resilience.execute(`stripe-${operation}-${Date.now()}`, async () => {
                switch (operation) {
                    case 'createPaymentIntent':
                        return await this.createPaymentIntent(this.params);
                    case 'confirmPayment':
                        return await this.confirmPayment(this.params);
                    case 'refundPayment':
                        return await this.refundPayment(this.params);
                    case 'createCustomer':
                        return await this.createCustomer(this.params);
                    case 'getCustomer':
                        return await this.getCustomer(this.params);
                    case 'updateCustomer':
                        return await this.updateCustomer(this.params);
                    case 'createSubscription':
                        return await this.createSubscription(this.params);
                    case 'cancelSubscription':
                        return await this.cancelSubscription(this.params);
                    case 'updateSubscription':
                        return await this.updateSubscription(this.params);
                    case 'createInvoice':
                        return await this.createInvoice(this.params);
                    case 'getInvoice':
                        return await this.getInvoice(this.params);
                    case 'listInvoices':
                        return await this.listInvoices(this.params);
                    case 'createProduct':
                        return await this.createProduct(this.params);
                    case 'createPrice':
                        return await this.createPrice(this.params);
                    case 'handleWebhook':
                        return await this.handleWebhook(this.params);
                    default:
    ...
    async handleWebhook(params) {
        ...
        try {
            ...
            const expectedSignature = crypto
                ...
                .digest('hex');
            const signatureValid = signatures.some(sig => crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expectedSignature)));
            if (!signatureValid) {
                return {
                    id: '',
                    type: '',
...
//# sourceMappingURL=stripe-bubble.js.map