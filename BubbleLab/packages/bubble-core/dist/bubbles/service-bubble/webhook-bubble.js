...
class WebhookStorage {
    ...
    getHandlersForEvent(eventType) {
        return Array.from(this.handlers.values()).filter(h => h.active && h.eventType === eventType);
    }
    ...
    timeRangeToMs(timeRange) {
        ...
        return units[timeRange] || units.day;
    }
}
// ============================================================================
// MAIN BUBBLE CLASS
// ============================================================================
export class WebhookBubble extends ServiceBubble {
    static type = 'service';
    static service = 'webhook';
    static authType = 'none';
    static bubbleName = 'webhook';
    static schema = WebhookBubbleParamsSchema;
    static resultSchema = WebhookBubbleResultSchema;
    static shortDescription = 'Complete webhook management and processing service';
  ...
  `;
    static alias = 'webhook';
    static storage = new WebhookStorage();
    resilience;
    ...
    async performAction(context) {
        ...
        try {
            const result = await this.resilience.execute(`webhook-${operation}-${Date.now()}`, async () => {
                switch (operation) {
                    case 'receiveWebhook':
                        return await this.receiveWebhook(this.params);
                    case 'verifySignature':
                        return await this.verifySignature(this.params);
                    case 'parsePayload':
                        return await this.parsePayload(this.params);
                    case 'validateSignature':
                        return await this.validateSignature(this.params);
                    case 'dispatchEvent':
                        return await this.dispatchEvent(this.params);
                    case 'registerHandler':
                        return await this.registerHandler(this.params);
                    case 'unregisterHandler':
                        return await this.unregisterHandler(this.params);
                    case 'retryFailedWebhook':
                        return await this.retryFailedWebhook(this.params);
                    case 'getRetryStatus':
                        return await this.getRetryStatus(this.params);
                    case 'listWebhooks':
                        return await this.listWebhooks(this.params);
                    case 'getWebhook':
                        return await this.getWebhook(this.params);
                    case 'replayWebhook':
                        return await this.replayWebhook(this.params);
                    case 'deleteWebhook':
                        return await this.deleteWebhook(this.params);
                    case 'getStats':
                        return await this.getStats(this.params);
                    default:
    ...
    async receiveWebhook(params) {
        const { path, headers, body, signature, secret, signatureAlgorithm, timestamp, maxAge, store, contentType, maxPayloadSize } = params;
        try {
            ...
            let validated = false;
            if (signature && secret) {
                const validationResult = await this.validateSignatureInternal(body, signature, secret, signatureAlgorithm || 'hmac-sha256');
    ...
    async dispatchEvent(params) {
        ...
        try {
            ...
            const targetResults = await Promise.all(targets.map(async (url) => {
                ...
                try {
                    const response = await this.resilience.execute(`dispatch-${eventId}-${url}`, async () => {
                        return fetch(url, {
                            method: 'POST',
                            headers: {
    ...
    async replayWebhook(params) {
        ...
        try {
            ...
            const targetResults = await Promise.all(dispatchTargets.map(async (url) => {
                try {
    ...
    // ========================================================================
    async unregisterHandler(params) {
    ...
    async parsePayloadInternal(provider, payload, headers) {
        switch (provider) {
            case 'github':
                return {
    ...
    async validateSignatureInternal(payload, signature, secret, algorithm) {
...
//# sourceMappingURL=webhook-bubble.js.map