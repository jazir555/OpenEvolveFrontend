...
export class DataTransformerTool extends ToolBubble {
     ...
     */
    static type = 'tool';
    static bubbleName = 'data-transformer-tool';
    static schema = DataTransformerToolParamsSchema;
    static resultSchema = DataTransformerToolResultSchema;
    static shortDescription = 'Transform, filter, sort, and aggregate data arrays';
  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            ...
            switch (this.params.operation) {
                case 'map':
                    outputData = this.applyMapOperations(outputData, fieldsAdded, fieldsModified, fieldsRemoved);
                    break;
                case 'filter':
     ...
     */
    applyMapOperations(data, fieldsAdded, fieldsModified, fieldsRemoved) {
    ...
    evaluateExpression(expression, context) {
        ...
        if (!/^[\d\s+\-*/().%]+$/.test(sanitized)) {
            throw new Error(`Invalid expression: "${expression}". Only mathematical operations are allowed.`);
        }
        try {
    ...
    applyGroupBy(data) {
        ...
        groups.forEach((groupRecords) => {
            ...
            this.params.aggregations.forEach((agg) => {
                ...
                switch (operation) {
                    case 'sum':
                        resultValue = values.reduce((sum, v) => sum + Number(v), 0);
                        break;
                    case 'avg':
                        resultValue =
    ...
    applyPivot(data) {
        ...
        const groupByFields = Object.keys(data[0]).filter((f) => f !== pivotField && f !== valueField);
        const groups = new Map();
        data.forEach((record) => {
        ...
        groups.forEach((groupRecords) => {
            ...
            groupByFields.forEach((field, index) => {
                const groupKey = Array.from(groups.keys()).find((k) => groups.get(k) === groupRecords);
                pivoted[field] = groupKey?.split('|')[index];
            });
            // Pivot the data
            Array.from(pivotValues).forEach((pivotValue) => {
                const matchingRecords = groupRecords.filter((r) => String(r[pivotField]) === pivotValue);
                let aggregatedValue;
                switch (aggFunc) {
                    case 'sum':
                        aggregatedValue = matchingRecords.reduce((sum, r) => sum + Number(r[valueField] || 0), 0);
                        break;
                    case 'avg':
                        aggregatedValue =
                            matchingRecords.reduce((sum, r) => sum + Number(r[valueField] || 0), 0) / matchingRecords.length;
                        break;
                    case 'min':
                        aggregatedValue = Math.min(...matchingRecords.map((r) => Number(r[valueField] || 0)));
                        break;
                    case 'max':
                        aggregatedValue = Math.max(...matchingRecords.map((r) => Number(r[valueField] || 0)));
                        break;
                    case 'count':
    ...
    applyUnpivot(data) {
        ...
        const valueField = this.params.valueField;
        const result = [];
        const identifierFields = Object.keys(data[0]).filter((f) => f !== pivotField && f !== valueField);
        data.forEach((record) => {
            const identifier = Object.fromEntries(identifierFields.map((f) => [f, record[f]]));
            Object.entries(record).forEach(([key, value]) => {
     ...
     */
    applyCustomTransformation(data) {
        ...
        if (!allowCustom) {
            throw new Error('Custom transformations are disabled for security reasons. ' +
                'To enable, set environment variable ALLOW_CUSTOM_TRANSFORMATIONS=true. ' +
        ...
        for (const pattern of dangerousPatterns) {
            if (pattern.test(this.params.customScript)) {
                throw new Error(`Custom script contains dangerous pattern: ${pattern.source}. ` +
        ...
        try {
            ...
            const transformFn = new Function('data', '"use strict"; ' +
            ...
            console.warn(`[DataTransformerTool] Custom transformation executed. ` +
...
//# sourceMappingURL=data-transformer-tool.js.map