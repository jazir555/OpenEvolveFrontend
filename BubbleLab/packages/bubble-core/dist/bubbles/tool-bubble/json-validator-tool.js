...
export class JSONValidatorTool extends ToolBubble {
     ...
     */
    static type = 'tool';
    static bubbleName = 'json-validator-tool';
    static schema = JSONValidatorToolParamsSchema;
    static resultSchema = JSONValidatorToolResultSchema;
    static shortDescription = 'Validate JSON syntax, schema, and data quality';
  ...
  `;
    ...
    queryPath(data, path) {
        ...
        let current = data;
        const results = [];
        const traverse = (obj, parts, index) => {
                    ...
                    if (typeof value === 'string' && Array.isArray(target)) {
                        for (let i = 0; i < target.length; i++) {
                            if (typeof target[i] === 'string') {
                                ...
                                target[i] = target[i].replace(new RegExp(search, 'g'), replace);
                            }
                        }
                    }
                        ...
                        if (parent) {
                            const key = this.getLeafKey(path);
                            const [search, replace] = value.split('|');
                            parent[key] = target.replace(new RegExp(search, 'g'), replace);
                        }
                    }
                    break;
                case 'add':
     ...
     */
    validateAgainstSchema(data, schema, errors, path = 'root') {
        ...
        if (schema.properties && typeof data === 'object' && data !== null) {
            ...
            Object.entries(properties).forEach(([propName, propSchema]) => {
                if (propName in data) {
                    this.validateAgainstSchema(data[propName], propSchema, errors, `${path}.${propName}`);
                }
            });
        }
        // Check array items
     ...
     * Check for required fields
     */
    checkRequiredFields(data, requiredFields, errors) {
     ...
     */
    checkDataTypes(data, dataTypes, warnings) {
        if (typeof data !== 'object' || data === null) {
            return;
        }
     ...
     */
    applyCustomRules(data, rules, errors) {
            ...
            switch (rule) {
                ...
                case 'length':
                    if (Array.isArray(value)) {
                        const [min, max] = value;
                        const length = typeof current === 'string'
                            ? current.length
...
//# sourceMappingURL=json-validator-tool.js.map