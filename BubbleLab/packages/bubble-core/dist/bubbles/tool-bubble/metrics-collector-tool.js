...
class SystemMetricsCollector {
     ...
     */
    static collectDiskMetrics(path = '/') {
        try {
...
export class MetricsCollectorTool extends ToolBubble {
    static bubbleName = 'metrics-collector-tool';
    static schema = MetricsCollectorToolParamsSchema;
    static resultSchema = MetricsCollectorToolResultSchema;
    static shortDescription = 'Collect and analyze metrics';
  ...
  `;
    ...
    async collectMetrics(params, startTime) {
        const allMetrics = [];
    ...
    async collectFromPrometheus(source) {
        if (!source.endpoint) {
    ...
    async queryMetrics(params) {
        ...
        if (params.query?.labels) {
            filtered = filtered.filter((m) => {
                if (!m.labels)
                    return false;
                return Object.entries(params.query.labels).every(([key, value]) => m.labels[key] === value);
            });
        }
    ...
    exportToPrometheusFormat(metrics) {
        const lines = [];
    ...
    exportToGraphiteFormat(metrics) {
        const lines = [];
        metrics.forEach((metric) => {
    ...
    exportToCsvFormat(metrics) {
        const headers = ['name', 'value', 'timestamp', 'labels', 'unit', 'type'];
        const lines = [headers.join(',')];
        metrics.forEach((metric) => {
            const labels = JSON.stringify(metric.labels || {});
            lines.push([metric.name, metric.value, metric.timestamp, labels, metric.unit || '', metric.type].join(','));
        });
        return lines.join('\n');
    }
    ...
    async compareMetrics(params) {
        ...
        const previousMetrics = allMetrics.filter((m) => new Date(m.timestamp) < compareStart && new Date(m.timestamp) >= new Date(compareStart.getTime() - (now.getTime() - compareStart.getTime())));
    ...
    calculateAggregateValues(metrics) {
        const agg = {};
        metrics.forEach((metric) => {
            if (!agg[metric.name]) {
                agg[metric.name] = 0;
    ...
    applyForecastMethod(values, method) {
        if (values.length === 0)
            return 0;
        switch (method) {
            case 'linear':
...
//# sourceMappingURL=metrics-collector-tool.js.map