...
export class PDFGeneratorTool extends ToolBubble {
     ...
     */
    static type = 'tool';
    static bubbleName = 'pdf-generator-tool';
    static schema = PDFGeneratorToolParamsSchema;
    static resultSchema = PDFGeneratorToolResultSchema;
    static shortDescription = 'Generate PDF documents from HTML, Markdown, or text';
  ...
  `;
     ...
     * Process content based on type
     */
    async processContent(content, contentType, doc) {
        switch (contentType) {
            case 'markdown':
     ...
     */
    async processMarkdown(markdown, doc) {
        const elements = [];
     ...
     */
    async addContentToPDF(doc, elements) {
        ...
        if (this.params.footer?.showPageNumbers) {
            ...
            for (let i = 0; i < pages.count; i++) {
                doc.switchToPage(i);
                doc.fontSize(10).text(`Page ${i + 1} of ${pages.count}`, 0, doc.page.height - 50, { align: 'center' });
            }
        }
    }
    /**
     ...
     */
    async addImageToPDF(doc, imageSource) {
        try {
            let imageBuffer;
...
//# sourceMappingURL=pdf-generator-tool.js.map