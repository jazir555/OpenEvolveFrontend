...
export class URLValidatorTool extends ToolBubble {
     ...
     */
    static type = 'tool';
    static bubbleName = 'url-validator-tool';
    static schema = URLValidatorToolParamsSchema;
    static resultSchema = URLValidatorToolResultSchema;
    static shortDescription = 'Validate URLs with syntax, accessibility, and security checks';
  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            ...
            // Validate each URL
            const results = await Promise.all(urlsToValidate.map((url) => this.validateURL(url)));
    ...
    async validateURL(url) {
        ...
        try {
            ...
            // Check protocol
            if (this.params.allowedProtocols && !this.params.allowedProtocols.includes(protocol)) {
                isValid = false;
...
//# sourceMappingURL=url-validator-tool.js.map