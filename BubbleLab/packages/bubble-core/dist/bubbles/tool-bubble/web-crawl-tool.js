  ...
  `;
    ...
    async performAction(context) {
        ...
        try {
            ...
            const summarizationPromises = crawlResult.pages.map(async (page, index) => {
                if (!page.content || page.content.length <= 50000) {
                    return; // Skip short pages
                }
                console.debug(`[WebCrawlTool] Summarizing large page (${page.content.length} chars): ${page.url}`);
                try {
...
${page.content}`,
                    ...
                    console.error(`[WebCrawlTool] Error summarizing ${page.url}:`, error);
     ...
     */
    async executeCrawl(startTime) {
        const { url, format, onlyMainContent, maxPages, crawlDepth, includePaths, excludePaths, credentials, } = this.params;
        ...
        const firecrawl = new FirecrawlBubble(firecrawlParams, this.context, 'web_crawl_tool_firecrawl');
