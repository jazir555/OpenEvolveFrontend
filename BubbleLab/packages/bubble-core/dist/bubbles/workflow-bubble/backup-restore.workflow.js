...
export class BackupRestoreWorkflow extends WorkflowBubble {
    ...
    static bubbleName = 'backup-restore-workflow';
    static schema = BackupRestoreParamsSchema;
    static resultSchema = BackupRestoreResultSchema;
    static shortDescription = 'Automated backup and restore with rollback support';
  ...
  `;
    ...
    async validateBackup() {
        ...
        try {
            ...
            const issues = [];
            if (!sizeMatches) {
                issues.push(`Size mismatch: expected ${metadata.size}, got ${backupData.length}`);
            }
            // Validate metadata
     ...
     * Decompress data
     */
    async decompressData(data, compression) {
     ...
     * Store metadata
     */
    async storeMetadata(backupId, metadata) {
     ...
     * Perform rollback
     */
    async performRollback(backupId) {
...
//# sourceMappingURL=backup-restore.workflow.js.map