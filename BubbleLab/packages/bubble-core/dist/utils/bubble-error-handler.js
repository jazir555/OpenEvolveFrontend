 ...
 */
import { createErrorResponse, createSuccessResponse, generateCorrelationId, withTimeout, retryWithBackoff, CircuitBreaker, defaultCircuitBreakerConfig, } from './error-handler.js';
/**
 * Default configuration
 */
export const defaultBubbleErrorHandlerConfig = {
    timeout: 30000,
    retryAttempts: 3,
    retryDelay: 1000,
...
export class BubbleErrorHandler {
    ...
    async executeWithErrorHandling(operation, operationName, context) {
        ...
        try {
            ...
            console.error(`[${correlationId}] ${operationName} failed after ${duration}ms:`, error);
...
export function getDataOrThrow(response) {
    ...
    throw new Error(`Operation failed: ${response.error.message} (${response.error.code})`);
}
/**
 * Extract error from an error response, throwing if it's a success
 */
export function getErrorOrThrow(response) {
    if (isErrorResponse(response)) {
        return response;
    }
    throw new Error('Operation succeeded, cannot extract error');
}
//# sourceMappingURL=bubble-error-handler.js.map