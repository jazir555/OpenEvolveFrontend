 ...
 */
function extractTopLevelJson(input) {
    const s = input;
    let start = -1;
 ...
 */
function escapeProblematicQuotesAndControlChars(input) {
    const out = [];
    const scope = [];
    let insideString = false;
    let escapeNext = false;
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (!insideString) {
            ...
            if (ch === '"') {
                ...
                else if (scope[scope.length - 1] === 'object' &&
 ...
 */
function quoteUnquotedKeysOutsideStrings(input) {
    const out = [];
    let insideString = false;
    let escapeNext = false;
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (ch === '"') {
            ...
            buffer = buffer.replace(/([{,]\s*)([A-Za-z_$][A-Za-z0-9_$]*)(\s*):/g, '$1"$2"$3:');
            flushBuffer();
            insideString = true;
            out.push('"');
            continue;
 ...
 */
export function postProcessJSON(jsonString) {
...
export function parseJsonWithFallbacks(finalResponse) {
    ...
    for (let i = 0; i < attempts.length; i++) {
        try {
            ...
            console.warn(`[JSON Parser] Parsing attempt ${i + 1} failed:`, lastError.message);
        }
    }
    // All attempts failed
    ...
    const lastAttempt = extractAndCleanJSON(finalResponse) || postProcessJSON(finalResponse);
    return {
...
//# sourceMappingURL=json-parsing.js.map