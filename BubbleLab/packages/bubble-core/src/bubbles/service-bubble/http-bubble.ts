 ...
 *
 * Advanced HTTP client bubble with enterprise features:
 ...
 * - Timeout handling
 * - Support for query parameters, headers, and multiple body types
...
export class HttpBubble extends ServiceBubble<HttpBubbleParams, HttpBubbleResult> {
  static readonly service = 'nodex-core';
  static readonly authType = 'apikey' as const;
  static readonly bubbleName: BubbleName = 'http';
  static readonly type = 'service' as const;
  static readonly schema = HttpBubbleParamsSchema;
  static readonly resultSchema = HttpBubbleResultSchema;
  ...
  protected async performAction(context?: BubbleContext): Promise<HttpBubbleResult> {
    void context;
  ...
  private async executeWithRetry(): Promise<HttpBubbleResult> {
    const maxAttempts = this.params.retryEnabled ? this.params.maxRetries + 1 : 1;
    let lastError: Error | null = null;
    let lastResult: HttpBubbleResult | null = null;
  ...
  private async executeRequest(attempt: number): Promise<HttpBubbleResult> {
    const startTime = Date.now();
  ...
  private addAuthHeaders(headers: Record<string, string>, credential: string): void {
  ...
  private serializeBody(): BodyInit | null {
    if (!this.params.body) {
      return null;
    }
  ...
  private shouldRetry(result: HttpBubbleResult, attempt: number, maxAttempts: number): boolean {
  ...
  private static getCircuitBreakerState(url: string): CircuitBreakerState {
  ...
  private static setCircuitBreakerState(url: string, state: CircuitBreakerState): void {
    const normalizedUrl = this.normalizeUrl(url);
  ...
  private handleCircuitBreakerFailure(): void {
    ...
    if (state.failureCount >= this.params.circuitBreakerThreshold) {
      ...
      console.warn(
        `[HttpBubble] Circuit breaker opened for ${this.params.url} (failures: ${state.failureCount})`
      );
    }
  ...
  private createCircuitBreakerResult(): HttpBubbleResult {
    const state = HttpBubble.getCircuitBreakerState(this.params.url);
  ...
  private createErrorResult(error: Error, attempt: number, responseTime: number): HttpBubbleResult {
    return {
      success: false,
      data: undefined,
      status: 0,
