...
class EncodingDetector {
   ...
   */
  static detect(filePath: string): 'utf8' | 'ascii' | 'base64' | 'hex' {
    try {
      ...
      for (let i = 0; i < buffer.length; i++) {
        ...
        if (byte > 127) {
          // Multi-byte sequence
          let expectedBytes = 0;
          if (byte >= 0xC0 && byte <= 0xDF) expectedBytes = 1;
          else if (byte >= 0xE0 && byte <= 0xEF) expectedBytes = 2;
          else if (byte >= 0xF0 && byte <= 0xF7) expectedBytes = 3;
...
class QuotaManager {
  ...
  constructor(defaultQuota: number = 1024 * 1024 * 1024) { // 1GB default
    this.quotas = new Map();
   ...
   */
  setQuota(path: string, quota: number): void {
   ...
   */
  checkQuota(path: string, additionalSize: number): boolean {
   ...
   */
  recordOperation(path: string, size: number, operation: 'write' | 'delete'): void {
    const currentUsage = this.usage.get(path) || 0;
...
class VirusScanner {
  ...
  private scannerCommand?: string;
  private timeout: number;

  constructor(enabled: boolean = false, scannerCommand?: string, timeout: number = 30000) {
   ...
   */
  async scanFile(filePath: string): Promise<{ clean: boolean; reason?: string }> {
...
export class FileProcessorTool extends ToolBubble<
  FileProcessorToolParams,
  FileProcessorToolResult
> {
  /**
   * REQUIRED STATIC METADATA
   */
  static readonly type = 'tool' as const;
  static readonly bubbleName: BubbleName = 'file-processor-tool';
  static readonly schema = FileProcessorToolParamsSchema;
  static readonly resultSchema = FileProcessorToolResultSchema;
  static readonly shortDescription =
    'Read, write, and manipulate files with security validation';
  ...
  private static fileWatcher = new FileWatcher(100); // Max 100 concurrent watchers

  // Quota manager for storage tracking
  private static quotaManager = new QuotaManager(1024 * 1024 * 1024); // 1GB default quota

  // Virus scanner for security
  private static virusScanner = new VirusScanner(
    process.env.ENABLE_VIRUS_SCANNING === 'true',
    process.env.VIRUS_SCANNER_COMMAND,
    30000 // 30 second timeout
  );

  // Size limiter for preventing DoS attacks
  private static sizeLimiter = new SizeLimiter(10 * 1024 * 1024); // 10MB default

  // Rate limiter for preventing abuse
  private static rateLimiter = new RateLimiter(100, 60000); // 100 requests per minute

  constructor(
    params: FileProcessorToolParamsInput,
  ...
  async performAction(
    context?: BubbleContext
  ): Promise<FileProcessorToolResult> {
    ...
    try {
      ...
      let result: FileProcessorToolResult;
   ...
   * Read file
   */
  private async readFile(): Promise<FileProcessorToolResult> {
    ...
    let detectedEncoding = encoding as BufferEncoding;
    if (encoding === 'auto') {
   ...
   */
  private async writeFile(): Promise<FileProcessorToolResult> {
    ...
    if (!sizeCheck.withinLimit) {
      throw new Error(
        `Content too large: ${sizeCheck.size} bytes (max ${sizeCheck.maxSize} bytes)`
      );
    }
    ...
    if (!FileProcessorTool.quotaManager.checkQuota(filePath, contentSize)) {
      ...
      throw new Error(
        `Storage quota exceeded. Quota: ${quota} bytes, Used: ${usage} bytes, Required: ${contentSize} bytes`
      );
    }
    ...
    writeFileSync(tempFilePath, content, { encoding: encoding as BufferEncoding });
   ...
   * Check if file exists
   */
  private async checkExists(): Promise<FileProcessorToolResult> {
   ...
   */
  private async listFiles(): Promise<FileProcessorToolResult> {
   ...
   * Get file metadata
   */
  private async getMetadata(): Promise<FileProcessorToolResult> {
    const { filePath } = this.params;
   ...
   * Copy file
   */
  private async copyFile(): Promise<FileProcessorToolResult> {
   ...
   * Create directory
   */
  private async makeDirectory(): Promise<FileProcessorToolResult> {
   ...
   */
  private async watchDirectory(): Promise<FileProcessorToolResult> {
    ...
    const changes: Array<{ eventType: string; filename: string; timestamp: string }> = [];
   ...
   */
  private async executeBatch(): Promise<FileProcessorToolResult> {
