...
class TimezoneParser {
   ...
   */
  static parseWithTimezone(timestamp: string, format?: string): Date {
   ...
   */
  static toUTC(timestamp: string): string {
...
class FieldExtractor {
   ...
   */
  static extractStatus(line: string): number | null {
   ...
   */
  static extractResponseTime(line: string): number | null {
   ...
   * Extract user agent
   */
  static extractUserAgent(line: string): string | null {
    const uaPattern = /"([^"]*"[^"]*)"/;
   ...
   * Extract HTTP method
   */
  static extractMethod(line: string): string | null {
   ...
   * Extract URL path
   */
  static extractPath(line: string): string | null {
   ...
   */
  static extractKeyValuePairs(line: string): Record<string, string> {
 ...
 */
export class LogParserTool extends ToolBubble<LogParserToolParams, LogParserToolResult> {
  static readonly bubbleName: BubbleName = 'log-parser-tool';
  static readonly schema = LogParserToolParamsSchema;
  static readonly resultSchema = LogParserToolResultSchema;
  ...
  constructor(
    params: LogParserToolParamsInput = {
      operation: 'parse',
      logData: '',
    },
  ...
  async performAction(): Promise<LogParserToolResult> {
    ...
    try {
      ...
      if (format === 'auto') {
        ...
        format = detectedFormat as 'json' | 'csv' | 'apache' | 'nginx' | 'syslog' | 'custom';
        if (!format) {
      ...
      // Prepare result
      const result: LogParserToolResult = {
      ...
      switch (validatedParams.operation) {
        ...
        case 'analyze':
          result.anomalies = await this.detectAnomalies(limitedEntries);
          result.errors = this.detectErrors(limitedEntries);
          result.aggregations = this.aggregateLogs(
            limitedEntries,
            validatedParams
          );
          break;
  ...
  private detectFormat(logData: string): string {
  ...
  private parseLogs(logData: string, format: string): z.infer<typeof LogEntrySchema>[] {
    const lines = logData.split('\n');
    ...
    lines.forEach((line, index) => {
      ...
      try {
        let entry: z.infer<typeof LogEntrySchema>;
  ...
  private parseJSONLine(line: string, index: number): z.infer<typeof LogEntrySchema> {
  ...
  private parseApacheLine(line: string, index: number): z.infer<typeof LogEntrySchema> {
    const match = line.match(this.APACHE_PATTERN);
    if (!match) {
      throw new Error('Invalid Apache log format');
    }

    const [, ip, timestamp, method, path, protocol, status, bytes, referrer, userAgent] =
      match;
  ...
  private parseNginxLine(line: string, index: number): z.infer<typeof LogEntrySchema> {
  ...
  private parseSyslogLine(line: string, index: number): z.infer<typeof LogEntrySchema> {
  ...
  private parseCustomLine(line: string, index: number): z.infer<typeof LogEntrySchema> {
  ...
  private extractLevel(message: string): string {
  ...
  private filterLogs(
    entries: z.infer<typeof LogEntrySchema>[],
    params: LogParserToolParams
    ...
    if (params.filterLevel && params.filterLevel.length > 0) {
      filtered = filtered.filter((entry) =>
        entry.level ? params.filterLevel!.includes(entry.level as any) : false
      );
    }
    ...
    if (params.filterSource && params.filterSource.length > 0) {
      filtered = filtered.filter(
        (entry) => entry.source && params.filterSource!.includes(entry.source)
      );
    }
  ...
  private calculateStatistics(
    entries: z.infer<typeof LogEntrySchema>[]
  ): z.infer<typeof LogStatisticsSchema> {
    ...
    entries.forEach((entry) => {
      ...
      if (entry.level) {
        ...
        if (entry.level === 'ERROR' || entry.level === 'FATAL') errorsCount++;
        if (entry.level === 'WARN') warningsCount++;
      }
  ...
  private detectErrors(entries: z.infer<typeof LogEntrySchema>[]): z.infer<typeof LogEntrySchema>[] {
  ...
  private async detectAnomalies(
    entries: z.infer<typeof LogEntrySchema>[]
  ): Promise<Array<{ entry: z.infer<typeof LogEntrySchema>; reason: string; confidence: number }>> {
    const anomalies: Array<{
      entry: z.infer<typeof LogEntrySchema>;
      reason: string;
      confidence: number;
    }> = [];
    ...
    entries.forEach((entry) => {
      ...
      const sameErrors = entries.filter(
        (e) => e.message === entry.message && e.lineNumber !== entry.lineNumber
      );
...
Respond with JSON array of anomalies with format: [{"lineNumber": number, "reason": string, "confidence": number}]`;
        ...
        if (result.success && result.data?.response) {
          ...
          aiAnomalies.forEach((anomaly: any) => {
  ...
  private async enrichWithGeo(
    entries: z.infer<typeof LogEntrySchema>[]
  ): Promise<z.infer<typeof LogEntrySchema>[]> {
    const enrichedEntries = [...entries];

    for (const entry of enrichedEntries) {
      const ip = entry.metadata?.ip as string | undefined;
  ...
  private createErrorResult(errorMessage: string): LogParserToolResult {
    return {
