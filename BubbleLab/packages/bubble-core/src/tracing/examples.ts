...
import { traceAsync, createSpan, addSpanAttributes, recordException } from './tracer.js';
import { BubbleTracer, withBubbleTracing } from './bubble-tracer.js';
...
import type { TracingConfig, SpanAttributes } from './types.js';
...
async function bubbleSpecificTracing() {
  ...
  try {
    ...
    const result = await tracer.traceBubbleAction(
      ...
      async (span) => {
        ...
        if (span) {
          ...
          span.setAttribute('db.duration', (result as any).duration || 0);
        }
    ...
    if (instantiationSpan) {
      instantiationSpan.recordException(error as Error);
      instantiationSpan.end();
    }
    throw error;
  }
...
async function contextPropagationExample() {
  ...
  async function handleServiceBRequest(request: Request) {
    ...
    async (span) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          if (span) {
            span.setAttribute('db.error.code', (error as any).code || 'UNKNOWN');
...
function sendNotification(alert: unknown): void {
...
async function processDataWithAI(data: unknown): Promise<unknown> {
  return { processed: data };
}

async function storeResults(data: unknown): Promise<unknown> {
  return { stored: true };
}
...
export {
  basicSetupExample,
  traceBubbleOperation,
  bubbleSpecificTracing,
