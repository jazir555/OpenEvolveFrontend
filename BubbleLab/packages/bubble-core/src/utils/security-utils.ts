 ...
 */
export function sanitizeSQLQuery(query: string): { isSafe: boolean; sanitized?: string; reason?: string } {
...
export function sanitizeFilePath(
  filePath: string,
  allowedPaths: string[] = []
): { isSafe: boolean; sanitized?: string; reason?: string } {
 ...
 */
export function validateURL(url: string): { isValid: boolean; sanitized?: string; reason?: string } {
...
export class SizeLimiter {
   ...
   */
  checkSize(input: string): { withinLimit: boolean; size: number; maxSize: number } {
 ...
 * Command injection prevention
 */
export function sanitizeCommand(command: string): { isSafe: boolean; reason?: string } {
 ...
 */
export function sanitizeJSON(data: unknown): { isValid: boolean; sanitized?: unknown; reason?: string } {
  try {
    // Check for circular references
    const seen = new WeakSet();
    const detectCircular = (obj: unknown): boolean => {
 ...
 */
export function sanitizeRegex(pattern: string): { isSafe: boolean; reason?: string } {
