...
export class BubbleParser {
    ...
    extractPayloadDefaultsFromHandle(handleNode) {
    ...
    getFirstParamIdentifierName(firstParam) {
        if (firstParam.type === 'Identifier')
    ...
    evaluateDefaultExpressionToJSON(expr) {
        switch (expr.type) {
            case 'Literal':
    ...
    visitNode(node, nodes, classNameLookup, scopeManager) {
        ...
        if (node.type === 'ArrowFunctionExpression' &&
            ...
            const bubbleNode = this.extractBubbleFromExpression(node.body, classNameLookup);
            if (bubbleNode) {
                const synthetic = `_anonymous_${bubbleNode.className}_${Object.keys(nodes).length}`;
                bubbleNode.variableName = synthetic;
                const syntheticId = -1 * (Object.keys(nodes).length + 1);
        ...
        if (node.type === 'ReturnStatement' && node.argument) {
            const bubbleNode = this.extractBubbleFromExpression(node.argument, classNameLookup);
            if (bubbleNode) {
                const synthetic = `_anonymous_${bubbleNode.className}_${Object.keys(nodes).length}`;
                bubbleNode.variableName = synthetic;
                // Extract comment for this bubble node
            ...
            if (firstArg.type === 'ObjectExpression') {
                for (const prop of firstArg.properties) {
                    if (prop.type === 'Property') {
                        ...
                        const spreadName = spreadArg.type === 'Identifier' ? spreadArg.name : 'spread';
                        parameters.push({
                            name: spreadName,
                            ...value,
                            location,
