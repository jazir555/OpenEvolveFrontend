import { CredentialType, BubbleParameterType, BUBBLE_CREDENTIAL_OPTIONS, } from '@bubblelab/shared-schemas';
import { LoggerInjector } from './LoggerInjector';
import { replaceBubbleInstantiation } from '../utils/parameter-formatter';
const INVOCATION_GRAPH_START_MARKER = '// __BUBBLE_INVOCATION_DEPENDENCY_MAP_START__';
const INVOCATION_GRAPH_END_MARKER = '// __BUBBLE_INVOCATION_DEPENDENCY_MAP_END__';
export class BubbleInjector {
    ...
    findCredentials() {
        ...
        for (const [, bubble] of Object.entries(this.bubbleScript.getParsedBubblesRaw())) {
            const allCredentialTypes = new Set();
            // Get bubble-level credentials
            let credentialOptions = BUBBLE_CREDENTIAL_OPTIONS[bubble.bubbleName];
            ...
            if (bubble.bubbleName === 'ai-agent' && credentialOptions) {
                ...
                if (modelCredentialTypes !== null) {
                    ...
                    credentialOptions = credentialOptions.filter((credType) => modelCredentialTypes.includes(credType));
                }
     ...
     */
    extractModelCredentialType(bubble) {
        console.log('[BubbleInjector] Extracting model credential type for bubble:', bubble.bubbleName);
        if (bubble.bubbleName !== 'ai-agent') {
            return null;
        }
        // Find the model parameter
        const modelParam = bubble.parameters.find((param) => param.name === 'model');
        if (!modelParam) {
        ...
        if (modelParam.type === BubbleParameterType.OBJECT) {
            ...
            try {
                ...
                if (modelObj.backupModel &&
                    typeof modelObj.backupModel.model === 'string') {
                    backupModelString = modelObj.backupModel.model;
                }
            }
            catch (error) {
                console.error('[BubbleInjector] Failed to parse model parameter as JSON:', error);
        ...
        if (backupModelString) {
            const backupCredential = this.getCredentialTypeForProvider(backupModelString);
     ...
     */
    getCredentialTypeForProvider(modelString) {
     ...
     */
    extractToolCredentials(bubble) {
        if (bubble.bubbleName !== 'ai-agent') {
            return [];
        }
        const toolCredentials = new Set();
        ...
        try {
            ...
            for (const tool of toolsArray) {
                ...
                const toolBubbleName = tool.name;
            ...
            console.debug(`Failed to parse tools parameter for credential extraction: ${error}`);
        }
    ...
    injectCredentials(userCredentials = [], systemCredentials = {}) {
        try {
            ...
            for (const [_, bubble] of Object.entries(modifiedBubbles)) {
                const bubbleName = bubble.bubbleName;
                // Get the credential options for this bubble from the registry
                let bubbleCredentialOptions = BUBBLE_CREDENTIAL_OPTIONS[bubbleName] || [];
                ...
                if (bubble.bubbleName === 'ai-agent') {
                    ...
                    if (modelCredentialTypes !== null) {
                        ...
                        bubbleCredentialOptions = bubbleCredentialOptions.filter((credType) => modelCredentialTypes.includes(credType));
                    }
                    // If modelCredentialTypes is null, model is dynamic - include all credentials
                }
                // For AI agent bubbles, also collect tool-level credential requirements
                const toolCredentialOptions = bubble.bubbleName === 'ai-agent'
                ...
                for (const credentialType of allCredentialOptions) {
                    if (systemCredentials[credentialType]) {
                        credentialMapping[credentialType] = this.escapeString(systemCredentials[credentialType]);
                        injectedCredentials[`${bubble.variableId}.${credentialType}`] = {
                            isUserCredential: false,
                            credentialType: credentialType,
                            credentialValue: this.maskCredential(systemCredentials[credentialType]),
                        };
                    }
                }
                // Then inject user credentials (these override system credentials)
                const userCreds = userCredentials.filter((uc) => uc.bubbleVarId === bubble.variableId);
                for (const userCred of userCreds) {
                    const userCredType = userCred.credentialType;
                    if (allCredentialOptions.includes(userCredType)) {
                        credentialMapping[userCredType] = this.escapeString(userCred.secret);
            ...
            console.log('Final script:', this.bubbleScript.showScript('[BubbleInjector] Final script'));
            return {
     ...
     */
    injectCredentialsIntoBubble(bubble, credentialMapping) {
        // Check if bubble already has credentials parameter
        let credentialsParam = bubble.parameters.find((p) => p.name === 'credentials');
        if (!credentialsParam) {
    ...
    reapplyBubbleInstantiations() {
        const bubbles = Object.values(this.bubbleScript.getParsedBubblesRaw()).filter((bubble) => !bubble.invocationCallSiteKey);
        const lines = this.bubbleScript.currentBubbleScript.split('\n');
        // Sort bubbles by start line
        const sortedBubbles = [...bubbles].sort((a, b) => a.location.startLine - b.location.startLine);
        ...
        const nestedBubbleIds = new Set();
        const parentBubbleIds = new Set();
        for (const bubble of sortedBubbles) {
            for (const other of sortedBubbles) {
                if (other.variableId !== bubble.variableId &&
        ...
        const nestedBubbles = sortedBubbles.filter((b) => nestedBubbleIds.has(b.variableId));
        const nonNestedBubbles = sortedBubbles.filter((b) => !nestedBubbleIds.has(b.variableId));
        ...
        const nestedBubblesReversed = [...nestedBubbles].sort((a, b) => b.location.startLine - a.location.startLine);
     ...
     */
    refreshBubbleParametersFromSource(bubble, lines, adjustedBubbleEndLine) {
        // Calculate how many lines were deleted from within this bubble
        const linesDeletedInBubble = bubble.location.endLine - adjustedBubbleEndLine;
        ...
        for (const param of bubble.parameters) {
            if ((param.type === 'array' || param.type === 'object') &&
                ...
                const containsFunc = param.value.includes('func:') ||
                ...
                if (containsFunc && param.location) {
                    ...
                    const adjustedParamEndLine = param.location.endLine - linesDeletedInBubble;
    ...
    buildInvocationDependencyGraphLiteral() {
        const callSiteMap = {};
        for (const bubble of Object.values(this.bubbleScript.getParsedBubblesRaw())) {
        ...
        const literal = JSON.stringify(callSiteMap, null, 2).replace(/</g, '\\u003c');
        return literal || '{}';
    }
    injectInvocationDependencyGraphMap() {
        const literal = this.buildInvocationDependencyGraphLiteral();
        const lines = this.bubbleScript.currentBubbleScript.split('\n');
        const startIndex = lines.findIndex((line) => line.trim() === INVOCATION_GRAPH_START_MARKER);
        if (startIndex !== -1) {
            const endIndex = lines.findIndex((line, idx) => idx >= startIndex && line.trim() === INVOCATION_GRAPH_END_MARKER);
     ...
     */
    injectBubbleLoggingAndReinitializeBubbleParameters(loggingEnabled = true) {
        ...
        try {
            ...
            if (loggingEnabled) {
                ...
                this.bubbleScript.showScript('[BubbleInjector] After reapplyBubbleInstantiations');
            ...
            this.bubbleScript.parsingErrors.push(`Error injecting bubble logging and reinitialize bubble parameters: ${error instanceof Error ? error.message : String(error)}`);
            console.error('Error injecting bubble logging and reinitialize bubble parameters:', error);
        ...
        try {
            ...
            this.bubbleScript.parsingErrors.push(`Error injecting self capture: ${error instanceof Error ? error.message : String(error)}`);
    ...
    changeCredentials(bubbleId, credentials) {
        ...
        const credentialsParameter = parameters.find((p) => p.name === 'credentials');
        ...
        for (const credentialType of Object.keys(credentials)) {
            ...
            const credentialsObject = parameters.find((p) => p.name === 'credentials');
            // Add the credentials object parameter
            // Replace the credential parameter
            credentialsObject.value = credentials[credentialType];
        }
    }
}
//# sourceMappingURL=BubbleInjector.js.map