 ...
 */
export function sanitizeScript(code) {
    ...
    const throwError = "(() => { throw new Error('Access to process' + '.env is not allowed in bubble flows for security reasons.'); })()";
    ...
    const transformedCode = code.replace(/process\s*\[\s*['"]env['"]\s*\](\s*(\[['"`]?[^'"`\]]+['"`]?\]|\.\w+))*/g, throwError);
    ...
    let protectedCode = transformedCode.replace(stringAndCommentPattern, (match) => {
    ...
    protectedCode = protectedCode.replace(/process\.env\s*(\[['"`]?[^'"`\]]+['"`]?\]|\.\w+)/g, throwError);
    // Pattern 2: Standalone process.env (without property access)
    protectedCode = protectedCode.replace(/\bprocess\.env\b(?![.["'`\w])/g, throwError);
...
//# sourceMappingURL=sanitize-script.js.map