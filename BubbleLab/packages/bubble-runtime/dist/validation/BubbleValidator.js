...
class LanguageServiceTypechecker {
     ...
     */
    expandTypeString(type, typeChecker, sourceFile, visited = new Set()) {
            ...
            if (symbol) {
                ...
                if (declarations && declarations.length > 0) {
                    ...
                    if (ts.isTypeAliasDeclaration(declaration)) {
                        ...
                        if (aliasedType !== type) {
                            const expanded = this.expandTypeString(aliasedType, typeChecker, sourceFile, visited);
            ...
            if (properties.length > 0) {
                ...
                if (!baseTypeString.includes('{') &&
                    ...
                    for (const prop of properties.slice(0, 20)) {
                        ...
                        const propTypeString = this.expandTypeString(propType, typeChecker, sourceFile, new Set(visited));
                    ...
                    if (expandedParts.length > 0) {
                        const remaining = properties.length > 20
        ...
        if (type.isUnion()) {
            const unionTypes = type.types.map((t) => this.expandTypeString(t, typeChecker, sourceFile, new Set(visited)));
            return unionTypes.join(' | ');
        }
        // For intersection types, expand each member
        if (type.isIntersection()) {
            const intersectionTypes = type.types.map((t) => this.expandTypeString(t, typeChecker, sourceFile, new Set(visited)));
    ...
    extractVariableTypes(sourceFile, program) {
        ...
        const variableTypes = [];
        const expandTypeString = (type, visited = new Set()) => this.expandTypeString(type, typeChecker, sourceFile, visited);
        const visit = (node) => {
