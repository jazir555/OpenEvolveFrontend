 ...
 */
export async function validateBubbleFlow(code, requireLintErrors = true) {
    const syntaxErrors = [];
    const lintErrors = [];
    try {
        ...
        if (!validationResult.success) {
            if (validationResult.errors) {
                const scriptErrors = Object.entries(validationResult.errors).map(([lineNumber, errorMessage]) => `line ${lineNumber}: ${errorMessage}`);
        ...
        try {
            const sourceFile = ts.createSourceFile('bubbleflow.ts', code, ts.ScriptTarget.Latest, true);
            const lintRuleErrors = defaultLintRuleRegistry.validateAll(sourceFile);
            const lintErrorMessages = lintRuleErrors.map((err) => `line ${err.line}: ${err.message}`);
 ...
 */
export async function validateAndExtract(code, bubbleFactory, requireLintErrors = true) {
    ...
    try {
        ...
        return {
            ...
            requiredCredentials: Object.keys(requiredCredentials).length > 0
                ? requiredCredentials
                : undefined,
        };
    }
...
function validateTriggerEvent(bubbleScript) {
    const errors = [];
    ...
    if (triggerEvent?.type === 'schedule/cron') {
        if (!triggerEvent.cronSchedule) {
            errors.push("Missing cron schedule, please define it with the readonly cronSchedule property inside the BubbleFlow class. Ex. readonly cronSchedule = '0 0 * * *';");
        }
        if (!validateCronExpression(triggerEvent.cronSchedule).valid) {
            errors.push("Invalid cron schedule, please define it with the readonly cronSchedule property inside the BubbleFlow class. Ex. readonly cronSchedule = '0 0 * * *';");
        }
    }
    return errors;
}
