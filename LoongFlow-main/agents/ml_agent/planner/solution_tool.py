...
def build_ensemble_tool(context: Context) -> FunctionTool:
    ...
    async def select_solution_for_fusion(
        solution_ids: List[str], instruction: str
    ) -> dict[str, Any]:
        """
        ...
        if not isinstance(solution_ids, list):
            raise TypeError(
                f"Input must be a list of solution IDs, but got {type(solution_ids)}."
            )
        ...
        try:
            utils.write_assemble_model_info(
                context, json.dumps(assemble_info, ensure_ascii=False, indent=2)
            )
