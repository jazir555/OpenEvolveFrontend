...
import { BubbleButton, BubbleInput, BubbleSelect } from './bubblelab';
...
const OpenEvolveConfigPanelBase: React.FC<OpenEvolveConfigPanelProps> = ({
  ...
  useEffect(() => {
    const loadConfig = async () => {
      const result = await gracefulErrorHandler.executeWithErrorHandling(async () => {
        ...
        logError: true,
        context: {
          component: 'OpenEvolveConfigPanel',
          function: 'loadConfig',
      ...
      if (!result.success) {
        toast.error(`Failed to load configuration: ${result.error?.message || 'Unknown error'}`);
        setIsLoading(false);
      }
    };
    loadConfig();
  ...
  const handleConfigChange = async (newConfig: OpenEvolvePluginState) => {
    ...
    if (!result.success) {
      toast.error(`Failed to update configuration: ${result.error?.message || 'Unknown error'}`);
      setIsLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    if (!result.success) {
      toast.error(`Failed to reset configuration: ${result.error?.message || 'Unknown error'}`);
      setIsLoading(false);
    }
  };
    ...
    <div className="openevolve-config-panel bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="border-b border-gray-200 dark:border-gray-700">
        <nav className="-mb-px flex space-x-8 px-6" aria-label="Tabs">
          ...
          >
            <PuzzlePiece className="mr-2 w-4 h-4" /> Decomposition
          </BubbleButton>
          <BubbleButton
      ...
      <div className="px-6 py-4">
          ...
          <div className="space-y-6">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <Settings className="mr-2 w-4 h-4" /> General Settings
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    {OPENEVOLVE_PLUGIN_CONSTANTS.EXECUTION_METHODS.map((method: string) => (
                      <option key={method} value={method}>{
                        method.replace('_', ' ').replace(/\b\w/g, (letter: string) => letter.toUpperCase())
                      }</option>
                    ))}
                  </BubbleSelect>
                </div>
        ...
        {activeTab === 'evolution' && (
          <div className="space-y-6">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <Brain className="mr-2 w-4 h-4" /> Evolution Configuration
              </h3>
        ...
        {activeTab === 'adversarial' && (
          <div className="space-y-6">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <Shield className="mr-2 w-4 h-4" /> Adversarial Configuration
              </h3>
        ...
        {activeTab === 'decomposition' && (
          <div className="space-y-6">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <PuzzlePiece className="mr-2 w-4 h-4" /> Decomposition Configuration
              </h3>
        ...
        {activeTab === 'mdap_maker' && (
          <div className="space-y-6">
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <Network className="mr-2 w-4 h-4" /> MDAP/MAKER Configuration
              </h3>
