 ...
 * CrewAI Node
 *
 * Code generation bridge node for integration with CrewAI.
 ...
 * CrewAI task types
 */
export type CrewAITaskType = 'generate' | 'execute' | 'delegate' | 'optimize';
...
/**
 * CrewAI node configuration
 */
export interface CrewAINodeConfig {
  taskType?: CrewAITaskType;
 ...
 * CrewAI Node
 *
 * Bridges to CrewAI for code generation and execution.
 * Supports delegation, optimization, and cross-service integration.
 */
export class CrewAINode extends OpenEvolveBaseNode {
  static readonly DISPLAY_NAME = 'CrewAI Bridge';
  static readonly DESCRIPTION = 'Code generation and execution bridge with delegation and optimization capabilities';
  static readonly ICON = 'crewai';
  static readonly CATEGORY = 'integration';
  static readonly VERSION = '1.0.0';

  constructor(id: string, config: CrewAINodeConfig = {}) {
    super(id, {
      taskType: 'generate',
      language: 'python',
  ...
  async execute(inputs: NodeInputs, context: ExecutionContext): Promise<NodeResult> {
    try {
      ...
      const taskType = (inputs.taskType as CrewAITaskType) || (this.config.taskType as CrewAITaskType);
      ...
      const crewaiResult: CrewAIResult = {
      ...
      return this.createSuccessResult(crewaiResult);

    } catch (error) {
      return this.createErrorResult(
        error instanceof Error ? error.message : 'Unknown error during CrewAI execution'
      );
    }
  }
  ...
  private async generateCode(
    ...
    const response = await apiClient.post<any>('/crewai/generate', {
      description: input,
      language,
      include_tests: true,
      include_docs: true
  ...
  private async executeCode(
    ...
    const response = await apiClient.post<any>('/crewai/execute', {
      code,
      language,
  ...
  private async delegateTask(
    ...
    const response = await apiClient.post<any>('/crewai/delegate', {
      task: input,
  ...
  private async optimizeCode(
    ...
    const response = await apiClient.post<any>('/crewai/optimize', {
      code,
      language
    });
  ...
  private async monitorDelegation(taskId: string, context: ExecutionContext): Promise<any> {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        const status = await apiClient.get<any>(`/crewai/delegation/${taskId}`);
  ...
  async getAvailableServices(): Promise<NodeResult> {
    try {
      const response = await apiClient.get<any>('/crewai/services');
  ...
  async getQualityMetrics(code: string, language: CodeLanguage): Promise<NodeResult> {
    try {
      const response = await apiClient.post<any>('/crewai/quality', {
        code,
        language
      });
