            ...
            if hasattr(self, 'skill_manager') and self.skill_manager:
                if hasattr(self.skill_manager, 'llm'):
                    ...
                    if hasattr(llm_client, 'close'):
                        try:
                            llm_client.close()
                            logger.info("Closed skill_manager LLM client")
                        except Exception as e:
                            logger.warning(f"Failed to close skill_manager LLM client: {e}")
                    # Clear reference
                    self.skill_manager.llm = None
                self.skill_manager = None

            # Clear prompt manager
