...
try:
    ...
    logging.warning("ace_security_utils not available - using fallback validation")
...
# ============================================================================
# THREAD-SAFE MCP TOOL REGISTRY (Fixes Race Condition TS-1)
# ============================================================================

_MCP_TOOLS: Dict[str, Any] = {}
_MCP_TOOLS_LOCK = get_global_lock('mcp_tools_registry') if SECURITY_UTILS_AVAILABLE else threading.RLock()

def mcp_tool(name: str):
    """
    Thread-safe decorator to register MCP tools.

    CRITICAL FIX: Prevents race conditions during concurrent registration
    """
    @wraps(name)
    def decorator(func):
...
# ============================================================================
# MCP TOOL 1: Initialize ACE Agent (SECURITY HARDENED)
# ============================================================================
...
def initialize_ace_agent(
    ...
    try:
        ...
        elif not (0.0 <= dedup_threshold <= 1.0):
            raise ValueError(f"dedup_threshold must be 0-1")
    except ValueError as e:
    ...
    try:
        # Load or create skillbook with path validation (Fixes CVE-1: Path Traversal)
        if skillbook_path:
            try:
                if SECURITY_UTILS_AVAILABLE:
        ...
        return {
            ...
            "dedup_threshold": dedup_threshold,
            "message": f"ACE agent '{sanitize_for_logging(agent_id)}' initialized successfully",
            "components": {
                "skillbook": True,
                "agent": True,
                "reflector": True,
...
# (I'll include the complete file in the next message due to length)
```
