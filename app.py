...
st.markdown('<h2 style="text-align: center; color: #4a6fa5;">🧬 OpenEvolve Content Improver</h2>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center; font-size: 1.2rem;">AI-Powered Content Hardening with Multi-LLM Consensus</p>', unsafe_allow_html=True)
st.markdown("---")
...
with st.sidebar:
    ...
    if search_query:
        ...
        templates = [(selected_category, name, get_template_details(selected_category, name)) for name in template_names]
    ...
    for category, template_name, details in templates[:10]:  # Limit to first 10
        ...
        with col1:
            ...
            st.caption(details.get("description", "")[:100] + "..." if len(details.get("description", "")) > 100 else details.get("description", ""))
        with col2:
...
with tab1:
    ...
    st.text_area("Paste your draft content here:", height=300, key="protocol_text", disabled=st.session_state.adversarial_running)
...
def render_adversarial_testing_tab():
    ...
    with st.expander("⚡ Quick Start Wizard", expanded=True):
        ...
        if st.button("🚀 Quick Start Adversarial Testing", 
                    disabled=not (openrouter_key and st.session_state.red_team_models and st.session_state.blue_team_models and content.strip()),
                    type="primary",
...
def create_github_branch(token: str, repository: str, branch_name: str, base_branch: str = "main") -> Optional[str]:
    ...
    try:
        ...
        if create_response.status_code == 201:
            ...
            st.error(f"❌ Failed to create branch '{branch_name}': {create_response.status_code} - {create_response.text}")
            return None
...
def commit_to_github(token: str, repository: str, file_path: str, content: str, commit_message: str, branch: str = "main") -> bool:
