...
for name, info in sorted(public_methods.items()):
    status = '[WARN] STUB' if info['is_stub'] else ('[OK] REAL' if info['has_real_logic'] else '⚡ SIMPLE')
...
print(f'\n=== QUALITY ASSESSMENT ===')
print(f'[OK] Real implementations: {len(real_implementations)}/{len(all_methods)} ({len(real_implementations)*100//len(all_methods)}%)')
print(f'[WARN]  Stub methods: {len(stubs)}')
if stubs:
    print(f'    {stubs}')
print(f'[WARN]  TODO markers: {len(todos)}')
if todos:
    print(f'    {todos}')
print(f'[WARN]  Placeholder methods: {len(placeholders)}')
if placeholders:
...
"""
    ...
    for strategy in strategies:
        try:
            ...
            print(f'[FAIL] {strategy.value}: FAILED - {str(e)[:50]}')
    ...
    try:
        ...
        print(f'[OK] Reassembly: quality {reassembly.quality_score:.2f}')
    except (ValueError, TypeError, RuntimeError) as e:
        print(f'[FAIL] Reassembly: FAILED - {str(e)[:50]}')
