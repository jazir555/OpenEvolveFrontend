...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle, ref }) => {
  ...
  type ToolTo = '/leanaide' | '/oneke' | '/gket';
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col pt-2 px-2 items-stretch gap-2">
        ...
        {/* OpenEvolve section */}
        {isOpen && (
          <div className="mt-4 px-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500">
            OpenEvolve
          </div>
        )}
        {renderOpenEvolveLink('/openevolve/teams', 'Teams', Users)}
        {renderOpenEvolveLink('/openevolve/gauntlets', 'Gauntlets', ShieldCheck)}
        {renderOpenEvolveLink('/openevolve/workflows', 'Workflows', GitBranch)}
        {renderOpenEvolveLink(
          '/openevolve/decomposition',
          'Decomposition',
          GitFork
        )}
        ...
        {renderToolLink('/leanaide', 'LeanAide', Brain)}
        {renderToolLink('/oneke', 'OneKE', Network)}
        {renderToolLink('/gket', 'Knowledge Extraction', FileSearch)}
