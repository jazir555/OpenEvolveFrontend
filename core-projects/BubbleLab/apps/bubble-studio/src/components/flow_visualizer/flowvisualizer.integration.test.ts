...
`;
  ...
  it('should validate Slack integration test workflow', async () => {
    const code = `import {
  BubbleFlow,
  SlackBubble,
  type WebhookEvent,
...
`;
    ...
    const validationResult = await getValidationResponse(code);
    const stepGraph = await validateGraph(validationResult);
    // Ensure at least 1 step is extracted from the Slack workflow
    expect(stepGraph.steps.length).toBeGreaterThan(0);
    await validateAllNodesAreUsed(validationResult);
    await validateEachBubbleHasAtLeastOneClone(validationResult);
  });
...
export class TestFlow extends BubbleFlow<'webhook/http'> {
  // This method IS used - should be included
  ...
  // This method is NEVER called from handle - should be ignored
  private async unusedMethod(): Promise<string> {
    const agent = new AIAgentBubble({
      model: { model: 'google/gemini-2.5-flash' },
      systemPrompt: 'You are a helper.',
      message: 'This bubble should not appear in the workflow',
    });
    const result = await agent.action();
    return result.data?.response || '';
  }
  ...
  async handle(payload: TestPayload): Promise<Output> {
    // Only sendNotification is called, not unusedMethod
    await this.sendNotification(payload.input);
...
`;
    ...
    // Should have steps (from the used sendNotification method)
    expect(stepGraph.steps.length).toBeGreaterThan(0);

    // The AIAgentBubble from unusedMethod should NOT be in any step
    const allBubbleIds = stepGraph.steps.flatMap((step) => step.bubbleIds);
    const bubbles = validationResult.bubbles || {};

    // Find the AIAgentBubble (should exist in bubbles but not in steps)
    const aiAgentBubble = Object.values(bubbles).find(
      (b) => b.bubbleName === 'ai-agent'
    );

    // The unused method's bubble should not be cloned/used in the workflow
    if (aiAgentBubble) {
      expect(allBubbleIds).not.toContain(aiAgentBubble.variableId);
    }

    // The SlackBubble from sendNotification SHOULD be in steps
    const slackBubble = Object.values(bubbles).find(
