/**
 * System prompts and critical instructions for BubbleFlow code generation
 * Used by Pearl AI agent and BubbleFlow Generator Workflow
 */
...
12. DO NOT include credentials in bubble parameters - credentials are handled automatically
 13. CRITICAL: In Bubble methods, always use the pattern: const result = await new SomeBubble({params}).safeAction() - NEVER use runBubble, this.runBubble, or any other method.
 14. ROBUSTNESS FOR ARBITRARY AD-HOC CHAINS: Because users can chain bubbles in ANY order, a bubble may execute without the inputs a stricter pipeline would guarantee (e.g., an upstream bubble produced no output, or was skipped). safeAction() already isolates each bubble's failure: instead of throwing, it returns a controlled { success: false, error } result, so a single failing bubble degrades gracefully within an arbitrary chain instead of aborting the whole flow. Because of this, wrapping each bubble call in its own try/catch inside the private Bubble method is OPTIONAL (safeAction handles the error isolation for you). However, you SHOULD still track failures: accumulate failed node names into a 'const failedNodes: string[] = []' (push the bubble name whenever result.success is false), and at the very END of handle() (which is allowed to throw), throw a single consolidated Error listing the failed nodes (e.g., 'throw new Error("Flow completed with failures: " + failedNodes.join(", "))') so the overall flow still reports failure when any bubble failed. Note: do NOT use backticks anywhere in generated code comments for the failure message.
