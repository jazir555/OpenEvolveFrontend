...
export class CircuitBreaker {
    ...
    stateLock = false; // Simple mutex for state transitions
    constructor(config) {
        this.config = config;
        this.validateConfig();
    }
 ...
 */
export async function retryWithBackoff(operation, fn, config) {
    let lastError;
    for (let attempt = 0; attempt < config.maxRetries; attempt++) {
        try {
            ...
            console.log(`[Retry] Operation "${operation}" failed (attempt ${attempt + 1}/${config.maxRetries}). ` +
...
export class RequestDeduplicator {
    ...
    ttl; // Time to cache completed results
...
export class DeadLetterQueue {
     ...
     */
    async push(operation, input, error, retryCount = 0) {
        const entry = {
        ...
        console.error(`[DLQ] Added to dead letter queue: ${entry.id}. ` +
            `Operation: ${operation}, Error: ${error.message}`);
    }
    /**
     * Consume entries from DLQ
     */
    async consume(limit = 10) {
...
export class ResilienceWrapper {
     ...
     */
    async execute(key, operation, input) {
        const wrappedOperation = async () => {
            try {
                ...
                if (this.dlq && !isTransientError(error)) {
                    await this.dlq.push(this.operationName, input, error);
                }
                throw error;
            }
        };
        // Apply retry logic
        const retriedOperation = () => retryWithBackoff(this.operationName, wrappedOperation, this.retryConfig);
        // Apply deduplication
        if (this.deduplicator) {
            return this.deduplicator.execute(key, retriedOperation, { cacheResult: true });
        }
...
export class StructuredLogger {
    ...
    child(context) {
        ...
        childLogger.info = (data, error) => {
            this.log('info', { ...context, ...data }, error);
        };
        childLogger.warn = (data, error) => {
            this.log('warn', { ...context, ...data }, error);
        };
        childLogger.error = (data, error) => {
            this.log('error', { ...context, ...data }, error);
        };
        childLogger.debug = (data, error) => {
            if (process.env.LOG_LEVEL === 'debug') {
