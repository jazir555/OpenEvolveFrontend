#!/bin/bash
# ai-agent.probe.sh - Runtime Validation Probe for ai-agent
#
# Comprehensive probe script to validate ai-agent service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/ai-agent.probe.sh
...
# Test 2.$((i + 1)): /v1/models
if [[ "/v1/models" == "/"* ]]; then
    test_json_endpoint "/v1/models" "Endpoint 1: /v1/models" || \
    test_http_endpoint "/v1/models" "Endpoint 1: /v1/models" "200" || \
    log_warning "Endpoint 1 (/v1/models) not available"
else
    log_info "Command check: /v1/models"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/chat/completions
if [[ "/v1/chat/completions" == "/"* ]]; then
    test_json_endpoint "/v1/chat/completions" "Endpoint 2: /v1/chat/completions" || \
    test_http_endpoint "/v1/chat/completions" "Endpoint 2: /v1/chat/completions" "200" || \
    log_warning "Endpoint 2 (/v1/chat/completions) not available"
else
    log_info "Command check: /v1/chat/completions"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/embeddings
if [[ "/v1/embeddings" == "/"* ]]; then
    test_json_endpoint "/v1/embeddings" "Endpoint 3: /v1/embeddings" || \
    test_http_endpoint "/v1/embeddings" "Endpoint 3: /v1/embeddings" "200" || \
    log_warning "Endpoint 3 (/v1/embeddings) not available"
else
    log_info "Command check: /v1/embeddings"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /v1/files
if [[ "/v1/files" == "/"* ]]; then
    test_json_endpoint "/v1/files" "Endpoint 4: /v1/files" || \
    test_http_endpoint "/v1/files" "Endpoint 4: /v1/files" "200" || \
    log_warning "Endpoint 4 (/v1/files) not available"
else
    log_info "Command check: /v1/files"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v1/assistants
if [[ "/v1/assistants" == "/"* ]]; then
    test_json_endpoint "/v1/assistants" "Endpoint 5: /v1/assistants" || \
    test_http_endpoint "/v1/assistants" "Endpoint 5: /v1/assistants" "200" || \
    log_warning "Endpoint 5 (/v1/assistants) not available"
else
    log_info "Command check: /v1/assistants"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ AI-AGENT PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ AI-AGENT PROBE FAILED${NC}"
    echo ""
    exit 1
fi
