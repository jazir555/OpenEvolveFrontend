#!/bin/bash
# airtable.probe.sh - Runtime Validation Probe for airtable
#
# Comprehensive probe script to validate airtable service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/airtable.probe.sh
...
# Test 2.$((i + 1)): /v0/meta/bases
if [[ "/v0/meta/bases" == "/"* ]]; then
    test_json_endpoint "/v0/meta/bases" "Endpoint 1: /v0/meta/bases" || \
    test_http_endpoint "/v0/meta/bases" "Endpoint 1: /v0/meta/bases" "200" || \
    log_warning "Endpoint 1 (/v0/meta/bases) not available"
else
    log_info "Command check: /v0/meta/bases"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v0/meta/whoami
if [[ "/v0/meta/whoami" == "/"* ]]; then
    test_json_endpoint "/v0/meta/whoami" "Endpoint 2: /v0/meta/whoami" || \
    test_http_endpoint "/v0/meta/whoami" "Endpoint 2: /v0/meta/whoami" "200" || \
    log_warning "Endpoint 2 (/v0/meta/whoami) not available"
else
    log_info "Command check: /v0/meta/whoami"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v0/meta/organizations
if [[ "/v0/meta/organizations" == "/"* ]]; then
    test_json_endpoint "/v0/meta/organizations" "Endpoint 3: /v0/meta/organizations" || \
    test_http_endpoint "/v0/meta/organizations" "Endpoint 3: /v0/meta/organizations" "200" || \
    log_warning "Endpoint 3 (/v0/meta/organizations) not available"
else
    log_info "Command check: /v0/meta/organizations"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v0/meta/bases/{baseId}/tables
if [[ "/v0/meta/bases/{baseId}/tables" == "/"* ]]; then
    test_json_endpoint "/v0/meta/bases/{baseId}/tables" "Endpoint 4: /v0/meta/bases/{baseId}/tables" || \
    test_http_endpoint "/v0/meta/bases/{baseId}/tables" "Endpoint 4: /v0/meta/bases/{baseId}/tables" "200" || \
    log_warning "Endpoint 4 (/v0/meta/bases/{baseId}/tables) not available"
else
    log_info "Command check: /v0/meta/bases/{baseId}/tables"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ AIRTABLE PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ AIRTABLE PROBE FAILED${NC}"
    echo ""
    exit 1
fi
