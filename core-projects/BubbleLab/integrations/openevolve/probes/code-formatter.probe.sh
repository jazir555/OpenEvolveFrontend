#!/bin/bash
# code-formatter.probe.sh - Runtime Validation Probe for code-formatter
#
# Comprehensive probe script to validate code-formatter service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/code-formatter.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 CODE-FORMATTER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /format
if [[ "/format" == "/"* ]]; then
    test_json_endpoint "/format" "Endpoint 1: /format" || \
    test_http_endpoint "/format" "Endpoint 1: /format" "200" || \
    log_warning "Endpoint 1 (/format) not available"
else
...
# Test 2.$((i + 1)): /lint
if [[ "/lint" == "/"* ]]; then
    test_json_endpoint "/lint" "Endpoint 2: /lint" || \
    test_http_endpoint "/lint" "Endpoint 2: /lint" "200" || \
    log_warning "Endpoint 2 (/lint) not available"
else
    log_info "Command check: /lint"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /beautify
if [[ "/beautify" == "/"* ]]; then
    test_json_endpoint "/beautify" "Endpoint 3: /beautify" || \
    test_http_endpoint "/beautify" "Endpoint 3: /beautify" "200" || \
    log_warning "Endpoint 3 (/beautify) not available"
else
    log_info "Command check: /beautify"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /minify
if [[ "/minify" == "/"* ]]; then
    test_json_endpoint "/minify" "Endpoint 4: /minify" || \
    test_http_endpoint "/minify" "Endpoint 4: /minify" "200" || \
    log_warning "Endpoint 4 (/minify) not available"
else
...
# Test 2.$((i + 1)): /validate
if [[ "/validate" == "/"* ]]; then
    test_json_endpoint "/validate" "Endpoint 5: /validate" || \
    test_http_endpoint "/validate" "Endpoint 5: /validate" "200" || \
    log_warning "Endpoint 5 (/validate) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ CODE-FORMATTER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ CODE-FORMATTER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
