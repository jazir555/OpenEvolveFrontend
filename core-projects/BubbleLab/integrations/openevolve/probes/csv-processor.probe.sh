#!/bin/bash
# csv-processor.probe.sh - Runtime Validation Probe for csv-processor
#
# Comprehensive probe script to validate csv-processor service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/csv-processor.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 CSV-PROCESSOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /parse
if [[ "/parse" == "/"* ]]; then
    test_json_endpoint "/parse" "Endpoint 1: /parse" || \
    test_http_endpoint "/parse" "Endpoint 1: /parse" "200" || \
    log_warning "Endpoint 1 (/parse) not available"
else
    log_info "Command check: /parse"
...
# Test 2.$((i + 1)): /validate
if [[ "/validate" == "/"* ]]; then
    test_json_endpoint "/validate" "Endpoint 2: /validate" || \
    test_http_endpoint "/validate" "Endpoint 2: /validate" "200" || \
    log_warning "Endpoint 2 (/validate) not available"
else
...
# Test 2.$((i + 1)): /merge
if [[ "/merge" == "/"* ]]; then
    test_json_endpoint "/merge" "Endpoint 4: /merge" || \
    test_http_endpoint "/merge" "Endpoint 4: /merge" "200" || \
    log_warning "Endpoint 4 (/merge) not available"
else
...
# Test 2.$((i + 1)): /split
if [[ "/split" == "/"* ]]; then
    test_json_endpoint "/split" "Endpoint 5: /split" || \
    test_http_endpoint "/split" "Endpoint 5: /split" "200" || \
    log_warning "Endpoint 5 (/split) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ CSV-PROCESSOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ CSV-PROCESSOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
