#!/bin/bash
# database-analyzer.probe.sh - Runtime Validation Probe for database-analyzer
#
# Comprehensive probe script to validate database-analyzer service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/database-analyzer.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 DATABASE-ANALYZER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /report
if [[ "/report" == "/"* ]]; then
    test_json_endpoint "/report" "Endpoint 2: /report" || \
    test_http_endpoint "/report" "Endpoint 2: /report" "200" || \
    log_warning "Endpoint 2 (/report) not available"
else
    log_info "Command check: /report"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /optimize
if [[ "/optimize" == "/"* ]]; then
    test_json_endpoint "/optimize" "Endpoint 3: /optimize" || \
    test_http_endpoint "/optimize" "Endpoint 3: /optimize" "200" || \
    log_warning "Endpoint 3 (/optimize) not available"
else
    log_info "Command check: /optimize"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /monitor
if [[ "/monitor" == "/"* ]]; then
    test_json_endpoint "/monitor" "Endpoint 4: /monitor" || \
    test_http_endpoint "/monitor" "Endpoint 4: /monitor" "200" || \
    log_warning "Endpoint 4 (/monitor) not available"
else
...
# Test 2.$((i + 1)): /schema
if [[ "/schema" == "/"* ]]; then
    test_json_endpoint "/schema" "Endpoint 5: /schema" || \
    test_http_endpoint "/schema" "Endpoint 5: /schema" "200" || \
    log_warning "Endpoint 5 (/schema) not available"
else
    log_info "Command check: /schema"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ DATABASE-ANALYZER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ DATABASE-ANALYZER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
