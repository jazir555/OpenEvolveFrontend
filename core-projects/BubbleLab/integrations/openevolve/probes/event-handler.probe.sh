#!/bin/bash
# event-handler.probe.sh - Runtime Validation Probe for event-handler
#
# Comprehensive probe script to validate event-handler service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/event-handler.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 EVENT-HANDLER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /handle
if [[ "/handle" == "/"* ]]; then
    test_json_endpoint "/handle" "Endpoint 1: /handle" || \
    test_http_endpoint "/handle" "Endpoint 1: /handle" "200" || \
    log_warning "Endpoint 1 (/handle) not available"
else
    log_info "Command check: /handle"
...
# Test 2.$((i + 1)): /register
if [[ "/register" == "/"* ]]; then
    test_json_endpoint "/register" "Endpoint 2: /register" || \
    test_http_endpoint "/register" "Endpoint 2: /register" "200" || \
    log_warning "Endpoint 2 (/register) not available"
else
    log_info "Command check: /register"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /trigger
if [[ "/trigger" == "/"* ]]; then
    test_json_endpoint "/trigger" "Endpoint 3: /trigger" || \
    test_http_endpoint "/trigger" "Endpoint 3: /trigger" "200" || \
    log_warning "Endpoint 3 (/trigger) not available"
else
    log_info "Command check: /trigger"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /list
if [[ "/list" == "/"* ]]; then
    test_json_endpoint "/list" "Endpoint 4: /list" || \
    test_http_endpoint "/list" "Endpoint 4: /list" "200" || \
    log_warning "Endpoint 4 (/list) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ EVENT-HANDLER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ EVENT-HANDLER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
