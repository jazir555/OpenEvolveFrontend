#!/bin/bash
# github.probe.sh - Runtime Validation Probe for github
#
# Comprehensive probe script to validate github service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/github.probe.sh
...
# Test 2.$((i + 1)): /user
if [[ "/user" == "/"* ]]; then
    test_json_endpoint "/user" "Endpoint 1: /user" || \
    test_http_endpoint "/user" "Endpoint 1: /user" "200" || \
    log_warning "Endpoint 1 (/user) not available"
else
    log_info "Command check: /user"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /user/repos
if [[ "/user/repos" == "/"* ]]; then
    test_json_endpoint "/user/repos" "Endpoint 2: /user/repos" || \
    test_http_endpoint "/user/repos" "Endpoint 2: /user/repos" "200" || \
    log_warning "Endpoint 2 (/user/repos) not available"
else
    log_info "Command check: /user/repos"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /rate_limit
if [[ "/rate_limit" == "/"* ]]; then
    test_json_endpoint "/rate_limit" "Endpoint 3: /rate_limit" || \
    test_http_endpoint "/rate_limit" "Endpoint 3: /rate_limit" "200" || \
    log_warning "Endpoint 3 (/rate_limit) not available"
else
    log_info "Command check: /rate_limit"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /user/orgs
if [[ "/user/orgs" == "/"* ]]; then
    test_json_endpoint "/user/orgs" "Endpoint 4: /user/orgs" || \
    test_http_endpoint "/user/orgs" "Endpoint 4: /user/orgs" "200" || \
    log_warning "Endpoint 4 (/user/orgs) not available"
else
    log_info "Command check: /user/orgs"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /user/starred
if [[ "/user/starred" == "/"* ]]; then
    test_json_endpoint "/user/starred" "Endpoint 5: /user/starred" || \
    test_http_endpoint "/user/starred" "Endpoint 5: /user/starred" "200" || \
    log_warning "Endpoint 5 (/user/starred) not available"
else
    log_info "Command check: /user/starred"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ GITHUB PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ GITHUB PROBE FAILED${NC}"
    echo ""
    exit 1
fi
