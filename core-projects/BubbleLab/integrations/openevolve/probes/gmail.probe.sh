#!/bin/bash
# gmail.probe.sh - Runtime Validation Probe for gmail
#
# Comprehensive probe script to validate gmail service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/gmail.probe.sh
...
# Test 2.$((i + 1)): /gmail/v1/users/me/profile
if [[ "/gmail/v1/users/me/profile" == "/"* ]]; then
    test_json_endpoint "/gmail/v1/users/me/profile" "Endpoint 1: /gmail/v1/users/me/profile" || \
    test_http_endpoint "/gmail/v1/users/me/profile" "Endpoint 1: /gmail/v1/users/me/profile" "200" || \
    log_warning "Endpoint 1 (/gmail/v1/users/me/profile) not available"
else
    log_info "Command check: /gmail/v1/users/me/profile"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /gmail/v1/users/me/messages
if [[ "/gmail/v1/users/me/messages" == "/"* ]]; then
    test_json_endpoint "/gmail/v1/users/me/messages" "Endpoint 2: /gmail/v1/users/me/messages" || \
    test_http_endpoint "/gmail/v1/users/me/messages" "Endpoint 2: /gmail/v1/users/me/messages" "200" || \
    log_warning "Endpoint 2 (/gmail/v1/users/me/messages) not available"
else
    log_info "Command check: /gmail/v1/users/me/messages"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /gmail/v1/users/me/labels
if [[ "/gmail/v1/users/me/labels" == "/"* ]]; then
    test_json_endpoint "/gmail/v1/users/me/labels" "Endpoint 3: /gmail/v1/users/me/labels" || \
    test_http_endpoint "/gmail/v1/users/me/labels" "Endpoint 3: /gmail/v1/users/me/labels" "200" || \
    log_warning "Endpoint 3 (/gmail/v1/users/me/labels) not available"
else
    log_info "Command check: /gmail/v1/users/me/labels"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /gmail/v1/users/me/threads
if [[ "/gmail/v1/users/me/threads" == "/"* ]]; then
    test_json_endpoint "/gmail/v1/users/me/threads" "Endpoint 4: /gmail/v1/users/me/threads" || \
    test_http_endpoint "/gmail/v1/users/me/threads" "Endpoint 4: /gmail/v1/users/me/threads" "200" || \
    log_warning "Endpoint 4 (/gmail/v1/users/me/threads) not available"
else
    log_info "Command check: /gmail/v1/users/me/threads"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ GMAIL PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ GMAIL PROBE FAILED${NC}"
    echo ""
    exit 1
fi
