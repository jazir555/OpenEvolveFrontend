#!/bin/bash
# google-sheets.probe.sh - Runtime Validation Probe for google-sheets
#
# Comprehensive probe script to validate google-sheets service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/google-sheets.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 GOOGLE-SHEETS SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /sheets/v4/spreadsheets
if [[ "/sheets/v4/spreadsheets" == "/"* ]]; then
    test_json_endpoint "/sheets/v4/spreadsheets" "Endpoint 1: /sheets/v4/spreadsheets" || \
    test_http_endpoint "/sheets/v4/spreadsheets" "Endpoint 1: /sheets/v4/spreadsheets" "200" || \
    log_warning "Endpoint 1 (/sheets/v4/spreadsheets) not available"
else
    log_info "Command check: /sheets/v4/spreadsheets"
...
if [[ "/drive/v3/about" == "/"* ]]; then
    test_json_endpoint "/drive/v3/about" "Endpoint 2: /drive/v3/about" || \
    test_http_endpoint "/drive/v3/about" "Endpoint 2: /drive/v3/about" "200" || \
    log_warning "Endpoint 2 (/drive/v3/about) not available"
else
...
# Test 2.$((i + 1)): /sheets/v4/spreadsheets/empty
if [[ "/sheets/v4/spreadsheets/empty" == "/"* ]]; then
    test_json_endpoint "/sheets/v4/spreadsheets/empty" "Endpoint 3: /sheets/v4/spreadsheets/empty" || \
    test_http_endpoint "/sheets/v4/spreadsheets/empty" "Endpoint 3: /sheets/v4/spreadsheets/empty" "200" || \
    log_warning "Endpoint 3 (/sheets/v4/spreadsheets/empty) not available"
else
    log_info "Command check: /sheets/v4/spreadsheets/empty"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ GOOGLE-SHEETS PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ GOOGLE-SHEETS PROBE FAILED${NC}"
    echo ""
    exit 1
fi
