#!/bin/bash
# http.probe.sh - Runtime Validation Probe for http
#
# Comprehensive probe script to validate http service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/http.probe.sh
...
# Test 2.$((i + 1)): /api/status
if [[ "/api/status" == "/"* ]]; then
    test_json_endpoint "/api/status" "Endpoint 3: /api/status" || \
    test_http_endpoint "/api/status" "Endpoint 3: /api/status" "200" || \
    log_warning "Endpoint 3 (/api/status) not available"
else
    log_info "Command check: /api/status"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /api/health
if [[ "/api/health" == "/"* ]]; then
    test_json_endpoint "/api/health" "Endpoint 4: /api/health" || \
    test_http_endpoint "/api/health" "Endpoint 4: /api/health" "200" || \
    log_warning "Endpoint 4 (/api/health) not available"
else
    log_info "Command check: /api/health"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /api/ping
if [[ "/api/ping" == "/"* ]]; then
    test_json_endpoint "/api/ping" "Endpoint 5: /api/ping" || \
    test_http_endpoint "/api/ping" "Endpoint 5: /api/ping" "200" || \
    log_warning "Endpoint 5 (/api/ping) not available"
else
    log_info "Command check: /api/ping"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ HTTP PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ HTTP PROBE FAILED${NC}"
    echo ""
    exit 1
fi
