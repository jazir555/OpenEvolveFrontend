#!/bin/bash
# log-parser.probe.sh - Runtime Validation Probe for log-parser
#
# Comprehensive probe script to validate log-parser service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/log-parser.probe.sh
...
# Test 2.$((i + 1)): /analyze
if [[ "/analyze" == "/"* ]]; then
    test_json_endpoint "/analyze" "Endpoint 2: /analyze" || \
    test_http_endpoint "/analyze" "Endpoint 2: /analyze" "200" || \
    log_warning "Endpoint 2 (/analyze) not available"
else
...
# Test 2.$((i + 1)): /errors
if [[ "/errors" == "/"* ]]; then
    test_json_endpoint "/errors" "Endpoint 3: /errors" || \
    test_http_endpoint "/errors" "Endpoint 3: /errors" "200" || \
    log_warning "Endpoint 3 (/errors) not available"
else
    log_info "Command check: /errors"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /stats
if [[ "/stats" == "/"* ]]; then
    test_json_endpoint "/stats" "Endpoint 4: /stats" || \
    test_http_endpoint "/stats" "Endpoint 4: /stats" "200" || \
    log_warning "Endpoint 4 (/stats) not available"
else
...
# Test 2.$((i + 1)): /patterns
if [[ "/patterns" == "/"* ]]; then
    test_json_endpoint "/patterns" "Endpoint 5: /patterns" || \
    test_http_endpoint "/patterns" "Endpoint 5: /patterns" "200" || \
    log_warning "Endpoint 5 (/patterns) not available"
else
    log_info "Command check: /patterns"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ LOG-PARSER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ LOG-PARSER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
