#!/bin/bash
# pdf-generator.probe.sh - Runtime Validation Probe for pdf-generator
#
# Comprehensive probe script to validate pdf-generator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/pdf-generator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 PDF-GENERATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /generate
if [[ "/generate" == "/"* ]]; then
    test_json_endpoint "/generate" "Endpoint 1: /generate" || \
    test_http_endpoint "/generate" "Endpoint 1: /generate" "200" || \
    log_warning "Endpoint 1 (/generate) not available"
else
    log_info "Command check: /generate"
...
# Test 2.$((i + 1)): /merge
if [[ "/merge" == "/"* ]]; then
    test_json_endpoint "/merge" "Endpoint 2: /merge" || \
    test_http_endpoint "/merge" "Endpoint 2: /merge" "200" || \
    log_warning "Endpoint 2 (/merge) not available"
else
    log_info "Command check: /merge"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /split
if [[ "/split" == "/"* ]]; then
    test_json_endpoint "/split" "Endpoint 3: /split" || \
    test_http_endpoint "/split" "Endpoint 3: /split" "200" || \
    log_warning "Endpoint 3 (/split) not available"
else
    log_info "Command check: /split"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /watermark
if [[ "/watermark" == "/"* ]]; then
    test_json_endpoint "/watermark" "Endpoint 4: /watermark" || \
    test_http_endpoint "/watermark" "Endpoint 4: /watermark" "200" || \
    log_warning "Endpoint 4 (/watermark) not available"
else
    log_info "Command check: /watermark"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ PDF-GENERATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ PDF-GENERATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
