#!/bin/bash
# qdrant.probe.sh - Runtime Validation Probe for qdrant
#
# Comprehensive probe script to validate qdrant service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/qdrant.probe.sh
...
# Test 2.$((i + 1)): /collections
if [[ "/collections" == "/"* ]]; then
    test_json_endpoint "/collections" "Endpoint 3: /collections" || \
    test_http_endpoint "/collections" "Endpoint 3: /collections" "200" || \
    log_warning "Endpoint 3 (/collections) not available"
else
    log_info "Command check: /collections"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /cluster
if [[ "/cluster" == "/"* ]]; then
    test_json_endpoint "/cluster" "Endpoint 4: /cluster" || \
    test_http_endpoint "/cluster" "Endpoint 4: /cluster" "200" || \
    log_warning "Endpoint 4 (/cluster) not available"
else
    log_info "Command check: /cluster"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /metrics
if [[ "/metrics" == "/"* ]]; then
    test_json_endpoint "/metrics" "Endpoint 5: /metrics" || \
    test_http_endpoint "/metrics" "Endpoint 5: /metrics" "200" || \
    log_warning "Endpoint 5 (/metrics) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ QDRANT PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ QDRANT PROBE FAILED${NC}"
    echo ""
    exit 1
fi
