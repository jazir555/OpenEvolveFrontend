#!/bin/bash
# research-agent.probe.sh - Runtime Validation Probe for research-agent
#
# Comprehensive probe script to validate research-agent service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/research-agent.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 RESEARCH-AGENT SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /research
if [[ "/research" == "/"* ]]; then
    test_json_endpoint "/research" "Endpoint 1: /research" || \
    test_http_endpoint "/research" "Endpoint 1: /research" "200" || \
    log_warning "Endpoint 1 (/research) not available"
else
    log_info "Command check: /research"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /summarize
if [[ "/summarize" == "/"* ]]; then
    test_json_endpoint "/summarize" "Endpoint 2: /summarize" || \
    test_http_endpoint "/summarize" "Endpoint 2: /summarize" "200" || \
    log_warning "Endpoint 2 (/summarize) not available"
else
    log_info "Command check: /summarize"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /sources
if [[ "/sources" == "/"* ]]; then
    test_json_endpoint "/sources" "Endpoint 3: /sources" || \
    test_http_endpoint "/sources" "Endpoint 3: /sources" "200" || \
    log_warning "Endpoint 3 (/sources) not available"
else
    log_info "Command check: /sources"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /citations
if [[ "/citations" == "/"* ]]; then
    test_json_endpoint "/citations" "Endpoint 4: /citations" || \
    test_http_endpoint "/citations" "Endpoint 4: /citations" "200" || \
    log_warning "Endpoint 4 (/citations) not available"
else
    log_info "Command check: /citations"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /analyze
if [[ "/analyze" == "/"* ]]; then
    test_json_endpoint "/analyze" "Endpoint 5: /analyze" || \
    test_http_endpoint "/analyze" "Endpoint 5: /analyze" "200" || \
    log_warning "Endpoint 5 (/analyze) not available"
else
    log_info "Command check: /analyze"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ RESEARCH-AGENT PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ RESEARCH-AGENT PROBE FAILED${NC}"
    echo ""
    exit 1
fi
