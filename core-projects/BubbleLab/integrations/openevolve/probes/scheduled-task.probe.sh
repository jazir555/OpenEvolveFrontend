#!/bin/bash
# scheduled-task.probe.sh - Runtime Validation Probe for scheduled-task
#
# Comprehensive probe script to validate scheduled-task service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/scheduled-task.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 SCHEDULED-TASK SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /schedule
if [[ "/schedule" == "/"* ]]; then
    test_json_endpoint "/schedule" "Endpoint 1: /schedule" || \
    test_http_endpoint "/schedule" "Endpoint 1: /schedule" "200" || \
    log_warning "Endpoint 1 (/schedule) not available"
else
...
# Test 2.$((i + 1)): /execute
if [[ "/execute" == "/"* ]]; then
    test_json_endpoint "/execute" "Endpoint 2: /execute" || \
    test_http_endpoint "/execute" "Endpoint 2: /execute" "200" || \
    log_warning "Endpoint 2 (/execute) not available"
else
    log_info "Command check: /execute"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /list
if [[ "/list" == "/"* ]]; then
    test_json_endpoint "/list" "Endpoint 3: /list" || \
    test_http_endpoint "/list" "Endpoint 3: /list" "200" || \
    log_warning "Endpoint 3 (/list) not available"
else
    log_info "Command check: /list"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /cancel
if [[ "/cancel" == "/"* ]]; then
    test_json_endpoint "/cancel" "Endpoint 4: /cancel" || \
    test_http_endpoint "/cancel" "Endpoint 4: /cancel" "200" || \
    log_warning "Endpoint 4 (/cancel) not available"
else
    log_info "Command check: /cancel"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /status
if [[ "/status" == "/"* ]]; then
    test_json_endpoint "/status" "Endpoint 5: /status" || \
    test_http_endpoint "/status" "Endpoint 5: /status" "200" || \
    log_warning "Endpoint 5 (/status) not available"
else
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ SCHEDULED-TASK PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ SCHEDULED-TASK PROBE FAILED${NC}"
    echo ""
    exit 1
fi
