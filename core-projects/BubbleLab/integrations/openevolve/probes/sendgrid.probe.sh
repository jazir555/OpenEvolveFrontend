#!/bin/bash
# sendgrid.probe.sh - Runtime Validation Probe for sendgrid
#
# Comprehensive probe script to validate sendgrid service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/sendgrid.probe.sh
...
# Test 2.$((i + 1)): /v3/templates
if [[ "/v3/templates" == "/"* ]]; then
    test_json_endpoint "/v3/templates" "Endpoint 1: /v3/templates" || \
    test_http_endpoint "/v3/templates" "Endpoint 1: /v3/templates" "200" || \
    log_warning "Endpoint 1 (/v3/templates) not available"
else
    log_info "Command check: /v3/templates"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v3/user/account
if [[ "/v3/user/account" == "/"* ]]; then
    test_json_endpoint "/v3/user/account" "Endpoint 2: /v3/user/account" || \
    test_http_endpoint "/v3/user/account" "Endpoint 2: /v3/user/account" "200" || \
    log_warning "Endpoint 2 (/v3/user/account) not available"
else
    log_info "Command check: /v3/user/account"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v3/user/profile
if [[ "/v3/user/profile" == "/"* ]]; then
    test_json_endpoint "/v3/user/profile" "Endpoint 3: /v3/user/profile" || \
    test_http_endpoint "/v3/user/profile" "Endpoint 3: /v3/user/profile" "200" || \
    log_warning "Endpoint 3 (/v3/user/profile) not available"
else
    log_info "Command check: /v3/user/profile"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v3/marketing/senders
if [[ "/v3/marketing/senders" == "/"* ]]; then
    test_json_endpoint "/v3/marketing/senders" "Endpoint 4: /v3/marketing/senders" || \
    test_http_endpoint "/v3/marketing/senders" "Endpoint 4: /v3/marketing/senders" "200" || \
    log_warning "Endpoint 4 (/v3/marketing/senders) not available"
else
    log_info "Command check: /v3/marketing/senders"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /v3/marketing/lists
if [[ "/v3/marketing/lists" == "/"* ]]; then
    test_json_endpoint "/v3/marketing/lists" "Endpoint 5: /v3/marketing/lists" || \
    test_http_endpoint "/v3/marketing/lists" "Endpoint 5: /v3/marketing/lists" "200" || \
    log_warning "Endpoint 5 (/v3/marketing/lists) not available"
else
    log_info "Command check: /v3/marketing/lists"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ SENDGRID PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ SENDGRID PROBE FAILED${NC}"
    echo ""
    exit 1
fi
