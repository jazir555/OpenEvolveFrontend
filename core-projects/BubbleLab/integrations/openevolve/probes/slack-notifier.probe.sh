#!/bin/bash
# slack-notifier.probe.sh - Runtime Validation Probe for slack-notifier
#
# Comprehensive probe script to validate slack-notifier service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/slack-notifier.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 SLACK-NOTIFIER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /notify
if [[ "/notify" == "/"* ]]; then
    test_json_endpoint "/notify" "Endpoint 1: /notify" || \
    test_http_endpoint "/notify" "Endpoint 1: /notify" "200" || \
    log_warning "Endpoint 1 (/notify) not available"
else
    log_info "Command check: /notify"
...
# Test 2.$((i + 1)): /alert
if [[ "/alert" == "/"* ]]; then
    test_json_endpoint "/alert" "Endpoint 3: /alert" || \
    test_http_endpoint "/alert" "Endpoint 3: /alert" "200" || \
    log_warning "Endpoint 3 (/alert) not available"
else
...
# Test 2.$((i + 1)): /digest
if [[ "/digest" == "/"* ]]; then
    test_json_endpoint "/digest" "Endpoint 4: /digest" || \
    test_http_endpoint "/digest" "Endpoint 4: /digest" "200" || \
    log_warning "Endpoint 4 (/digest) not available"
else
    log_info "Command check: /digest"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ SLACK-NOTIFIER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ SLACK-NOTIFIER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
