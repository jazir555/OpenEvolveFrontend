#!/bin/bash
# text-analyzer.probe.sh - Runtime Validation Probe for text-analyzer
#
# Comprehensive probe script to validate text-analyzer service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/text-analyzer.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 TEXT-ANALYZER SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /analyze
if [[ "/analyze" == "/"* ]]; then
    test_json_endpoint "/analyze" "Endpoint 1: /analyze" || \
    test_http_endpoint "/analyze" "Endpoint 1: /analyze" "200" || \
    log_warning "Endpoint 1 (/analyze) not available"
else
...
# Test 2.$((i + 1)): /keywords
if [[ "/keywords" == "/"* ]]; then
    test_json_endpoint "/keywords" "Endpoint 2: /keywords" || \
    test_http_endpoint "/keywords" "Endpoint 2: /keywords" "200" || \
    log_warning "Endpoint 2 (/keywords) not available"
else
    log_info "Command check: /keywords"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /sentiment
if [[ "/sentiment" == "/"* ]]; then
    test_json_endpoint "/sentiment" "Endpoint 3: /sentiment" || \
    test_http_endpoint "/sentiment" "Endpoint 3: /sentiment" "200" || \
    log_warning "Endpoint 3 (/sentiment) not available"
else
    log_info "Command check: /sentiment"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [[ "/summarize" == "/"* ]]; then
    test_json_endpoint "/summarize" "Endpoint 4: /summarize" || \
    test_http_endpoint "/summarize" "Endpoint 4: /summarize" "200" || \
    log_warning "Endpoint 4 (/summarize) not available"
else
...
# Test 2.$((i + 1)): /language
if [[ "/language" == "/"* ]]; then
    test_json_endpoint "/language" "Endpoint 5: /language" || \
    test_http_endpoint "/language" "Endpoint 5: /language" "200" || \
    log_warning "Endpoint 5 (/language) not available"
else
    log_info "Command check: /language"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ TEXT-ANALYZER PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ TEXT-ANALYZER PROBE FAILED${NC}"
    echo ""
    exit 1
fi
