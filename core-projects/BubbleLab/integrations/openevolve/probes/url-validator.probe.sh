#!/bin/bash
# url-validator.probe.sh - Runtime Validation Probe for url-validator
#
# Comprehensive probe script to validate url-validator service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/url-validator.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 URL-VALIDATOR SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /validate
if [[ "/validate" == "/"* ]]; then
    test_json_endpoint "/validate" "Endpoint 1: /validate" || \
    test_http_endpoint "/validate" "Endpoint 1: /validate" "200" || \
    log_warning "Endpoint 1 (/validate) not available"
else
    log_info "Command check: /validate"
...
# Test 2.$((i + 1)): /check
if [[ "/check" == "/"* ]]; then
    test_json_endpoint "/check" "Endpoint 2: /check" || \
    test_http_endpoint "/check" "Endpoint 2: /check" "200" || \
    log_warning "Endpoint 2 (/check) not available"
else
    log_info "Command check: /check"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /normalize
if [[ "/normalize" == "/"* ]]; then
    test_json_endpoint "/normalize" "Endpoint 3: /normalize" || \
    test_http_endpoint "/normalize" "Endpoint 3: /normalize" "200" || \
    log_warning "Endpoint 3 (/normalize) not available"
else
    log_info "Command check: /normalize"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /info
if [[ "/info" == "/"* ]]; then
    test_json_endpoint "/info" "Endpoint 4: /info" || \
    test_http_endpoint "/info" "Endpoint 4: /info" "200" || \
    log_warning "Endpoint 4 (/info) not available"
else
    log_info "Command check: /info"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /screenshot
if [[ "/screenshot" == "/"* ]]; then
    test_json_endpoint "/screenshot" "Endpoint 5: /screenshot" || \
    test_http_endpoint "/screenshot" "Endpoint 5: /screenshot" "200" || \
    log_warning "Endpoint 5 (/screenshot) not available"
else
    log_info "Command check: /screenshot"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ URL-VALIDATOR PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ URL-VALIDATOR PROBE FAILED${NC}"
    echo ""
    exit 1
fi
