#!/bin/bash
# vector-search.probe.sh - Runtime Validation Probe for vector-search
#
# Comprehensive probe script to validate vector-search service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/vector-search.probe.sh
...
echo ""
echo "════════════════════════════════════════════════════════════════════"
echo "  🔍 VECTOR-SEARCH SERVICE PROBE"
echo "════════════════════════════════════════════════════════════════════"
echo ""
...
# Test 2.$((i + 1)): /search
if [[ "/search" == "/"* ]]; then
    test_json_endpoint "/search" "Endpoint 1: /search" || \
    test_http_endpoint "/search" "Endpoint 1: /search" "200" || \
    log_warning "Endpoint 1 (/search) not available"
else
    log_info "Command check: /search"
...
# Test 2.$((i + 1)): /index
if [[ "/index" == "/"* ]]; then
    test_json_endpoint "/index" "Endpoint 2: /index" || \
    test_http_endpoint "/index" "Endpoint 2: /index" "200" || \
    log_warning "Endpoint 2 (/index) not available"
else
    log_info "Command check: /index"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
# Test 2.$((i + 1)): /status
if [[ "/status" == "/"* ]]; then
    test_json_endpoint "/status" "Endpoint 3: /status" || \
    test_http_endpoint "/status" "Endpoint 3: /status" "200" || \
    log_warning "Endpoint 3 (/status) not available"
else
...
# Test 2.$((i + 1)): /health
if [[ "/health" == "/"* ]]; then
    test_json_endpoint "/health" "Endpoint 4: /health" || \
    test_http_endpoint "/health" "Endpoint 4: /health" "200" || \
    log_warning "Endpoint 4 (/health) not available"
else
...
# Test 2.$((i + 1)): /stats
if [[ "/stats" == "/"* ]]; then
    test_json_endpoint "/stats" "Endpoint 5: /stats" || \
    test_http_endpoint "/stats" "Endpoint 5: /stats" "200" || \
    log_warning "Endpoint 5 (/stats) not available"
else
    log_info "Command check: /stats"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ VECTOR-SEARCH PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ VECTOR-SEARCH PROBE FAILED${NC}"
    echo ""
    exit 1
fi
