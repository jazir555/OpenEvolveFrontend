#!/bin/bash
# webhook.probe.sh - Runtime Validation Probe for webhook
#
# Comprehensive probe script to validate webhook service health and functionality
# Tests connectivity, endpoints, performance, and error handling
#
# Usage: ./probes/webhook.probe.sh
...
# Test 2.$((i + 1)): /webhooks
if [[ "/webhooks" == "/"* ]]; then
    test_json_endpoint "/webhooks" "Endpoint 1: /webhooks" || \
    test_http_endpoint "/webhooks" "Endpoint 1: /webhooks" "200" || \
    log_warning "Endpoint 1 (/webhooks) not available"
else
    log_info "Command check: /webhooks"
...
# Test 2.$((i + 1)): /health
if [[ "/health" == "/"* ]]; then
    test_json_endpoint "/health" "Endpoint 2: /health" || \
    test_http_endpoint "/health" "Endpoint 2: /health" "200" || \
    log_warning "Endpoint 2 (/health) not available"
else
...
# Test 2.$((i + 1)): /events
if [[ "/events" == "/"* ]]; then
    test_json_endpoint "/events" "Endpoint 3: /events" || \
    test_http_endpoint "/events" "Endpoint 3: /events" "200" || \
    log_warning "Endpoint 3 (/events) not available"
else
    log_info "Command check: /events"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /subscriptions
if [[ "/subscriptions" == "/"* ]]; then
    test_json_endpoint "/subscriptions" "Endpoint 4: /subscriptions" || \
    test_http_endpoint "/subscriptions" "Endpoint 4: /subscriptions" "200" || \
    log_warning "Endpoint 4 (/subscriptions) not available"
else
    log_info "Command check: /subscriptions"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi

# Test 2.$((i + 1)): /deliveries
if [[ "/deliveries" == "/"* ]]; then
    test_json_endpoint "/deliveries" "Endpoint 5: /deliveries" || \
    test_http_endpoint "/deliveries" "Endpoint 5: /deliveries" "200" || \
    log_warning "Endpoint 5 (/deliveries) not available"
else
    log_info "Command check: /deliveries"
    # For non-HTTP endpoints (Redis commands, SQL queries, etc.)
    log_success "PASS (Command defined)"
    ((PASS_COUNT++))
fi
...
if [ ${FAIL_COUNT} -eq 0 ]; then
    echo -e "${GREEN}✅ WEBHOOK PROBE PASSED${NC}"
    echo ""
    exit 0
else
    echo -e "${RED}❌ WEBHOOK PROBE FAILED${NC}"
    echo ""
    exit 1
fi
