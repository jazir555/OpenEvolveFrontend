/**
 * APIAggregatorWorkflow Test Suite
 *
 * Comprehensive tests for api-aggregator workflow bubble
 *
 * Test Coverage:
...
import { APIAggregatorWorkflow } from '../workflow-bubbles/api-aggregator-workflow';

describe('APIAggregatorWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new APIAggregatorWorkflow({
        operation: 'test',
        apis: '[]'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('APIAggregatorWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(APIAggregatorWorkflow.service).toBe('openevolve');
      expect(APIAggregatorWorkflow.bubbleName).toBe('api-aggregator');
      expect(APIAggregatorWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new APIAggregatorWorkflow({
        operation: 'test1',
        apis: '[]'
      } as any);

      const bubble2 = new APIAggregatorWorkflow({
        operation: 'test2',
        apis: '[]'
      } as any);
