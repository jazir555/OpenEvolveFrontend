/**
 * BackupRestoreWorkflow Test Suite
 *
 * Comprehensive tests for backup-restore workflow bubble
 *
 * Test Coverage:
...
import { BackupRestoreWorkflow } from '../workflow-bubbles/backup-restore-workflow';

describe('BackupRestoreWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new BackupRestoreWorkflow({
        operation: 'test',
        source: '/data'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('BackupRestoreWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(BackupRestoreWorkflow.service).toBe('openevolve');
      expect(BackupRestoreWorkflow.bubbleName).toBe('backup-restore');
      expect(BackupRestoreWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new BackupRestoreWorkflow({
        operation: 'test1',
        source: '/data'
      } as any);

      const bubble2 = new BackupRestoreWorkflow({
        operation: 'test2',
        source: '/data'
      } as any);
