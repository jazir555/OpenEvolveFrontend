/**
 * CodeFormatterTool Test Suite
 *
 * Comprehensive tests for code-formatter tool bubble
 *
 * Test Coverage:
...
import { CodeFormatterTool } from '../tool-bubbles/code-formatter-tool';
...
describe('CodeFormatterTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new CodeFormatterTool({
        operation: 'test',
        code: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('CodeFormatterTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(CodeFormatterTool.service).toBe('openevolve');
      expect(CodeFormatterTool.bubbleName).toBe('code-formatter');
      expect(CodeFormatterTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new CodeFormatterTool({
        operation: 'test1',
        code: 'test'
      } as any);

      const bubble2 = new CodeFormatterTool({
        operation: 'test2',
        code: 'test'
      } as any);
