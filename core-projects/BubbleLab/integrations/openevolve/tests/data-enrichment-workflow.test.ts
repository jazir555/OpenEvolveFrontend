/**
 * DataEnrichmentWorkflow Test Suite
 *
 * Comprehensive tests for data-enrichment workflow bubble
 *
 * Test Coverage:
...
import { DataEnrichmentWorkflow } from '../workflow-bubbles/data-enrichment-workflow';

describe('DataEnrichmentWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new DataEnrichmentWorkflow({
        operation: 'test',
        data: '{}'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('DataEnrichmentWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(DataEnrichmentWorkflow.service).toBe('openevolve');
      expect(DataEnrichmentWorkflow.bubbleName).toBe('data-enrichment');
      expect(DataEnrichmentWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new DataEnrichmentWorkflow({
        operation: 'test1',
        data: '{}'
      } as any);

      const bubble2 = new DataEnrichmentWorkflow({
        operation: 'test2',
        data: '{}'
      } as any);
