/**
 * EventHandlerWorkflow Test Suite
 *
 * Comprehensive tests for event-handler workflow bubble
 *
 * Test Coverage:
...
import { EventHandlerWorkflow } from '../workflow-bubbles/event-handler-workflow';

describe('EventHandlerWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new EventHandlerWorkflow({
        operation: 'test',
        event: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('EventHandlerWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(EventHandlerWorkflow.service).toBe('openevolve');
      expect(EventHandlerWorkflow.bubbleName).toBe('event-handler');
      expect(EventHandlerWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new EventHandlerWorkflow({
        operation: 'test1',
        event: 'test'
      } as any);

      const bubble2 = new EventHandlerWorkflow({
        operation: 'test2',
        event: 'test'
      } as any);
