/**
 * FileProcessorTool Test Suite
 *
 * Comprehensive tests for file-processor tool bubble
 *
 * Test Coverage:
...
import { FileProcessorTool } from '../tool-bubbles/file-processor-tool';
...
describe('FileProcessorTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new FileProcessorTool({
        operation: 'test',
        path: '/tmp/test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('FileProcessorTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(FileProcessorTool.service).toBe('openevolve');
      expect(FileProcessorTool.bubbleName).toBe('file-processor');
      expect(FileProcessorTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new FileProcessorTool({
        operation: 'test1',
        path: '/tmp/test'
      } as any);

      const bubble2 = new FileProcessorTool({
        operation: 'test2',
        path: '/tmp/test'
      } as any);
