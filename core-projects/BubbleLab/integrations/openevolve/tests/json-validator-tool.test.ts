/**
 * JSONValidatorTool Test Suite
 *
 * Comprehensive tests for json-validator tool bubble
 *
 * Test Coverage:
...
import { JSONValidatorTool } from '../tool-bubbles/json-validator-tool';
...
describe('JSONValidatorTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new JSONValidatorTool({
        operation: 'test',
        json: '{}'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('JSONValidatorTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(JSONValidatorTool.service).toBe('openevolve');
      expect(JSONValidatorTool.bubbleName).toBe('json-validator');
      expect(JSONValidatorTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new JSONValidatorTool({
        operation: 'test1',
        json: '{}'
      } as any);

      const bubble2 = new JSONValidatorTool({
        operation: 'test2',
        json: '{}'
      } as any);
