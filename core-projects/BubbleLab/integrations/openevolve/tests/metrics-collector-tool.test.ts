/**
 * MetricsCollectorTool Test Suite
 *
 * Comprehensive tests for metrics-collector tool bubble
 *
 * Test Coverage:
...
describe('MetricsCollectorTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new MetricsCollectorTool({
        operation: 'test',
        metric: 'cpu'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('MetricsCollectorTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(MetricsCollectorTool.service).toBe('openevolve');
      expect(MetricsCollectorTool.bubbleName).toBe('metrics-collector');
      expect(MetricsCollectorTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new MetricsCollectorTool({
        operation: 'test1',
        metric: 'cpu'
      } as any);

      const bubble2 = new MetricsCollectorTool({
        operation: 'test2',
        metric: 'cpu'
      } as any);
