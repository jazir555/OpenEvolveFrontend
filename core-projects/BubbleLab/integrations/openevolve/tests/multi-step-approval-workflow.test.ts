/**
 * MultiStepApprovalWorkflow Test Suite
 *
 * Comprehensive tests for multi-step-approval workflow bubble
 *
 * Test Coverage:
...
import { MultiStepApprovalWorkflow } from '../workflow-bubbles/multi-step-approval-workflow';

describe('MultiStepApprovalWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new MultiStepApprovalWorkflow({
        operation: 'test',
        workflow: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('MultiStepApprovalWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(MultiStepApprovalWorkflow.service).toBe('openevolve');
      expect(MultiStepApprovalWorkflow.bubbleName).toBe('multi-step-approval');
      expect(MultiStepApprovalWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new MultiStepApprovalWorkflow({
        operation: 'test1',
        workflow: 'test'
      } as any);

      const bubble2 = new MultiStepApprovalWorkflow({
        operation: 'test2',
        workflow: 'test'
      } as any);
