/**
 * RedisBubble Test Suite
 *
 * Comprehensive tests for redis service bubble
 *
 * Test Coverage:
...
describe('RedisBubble', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ServiceBubble properly', () => {
      ...
      expect(bubble.constructor.name).toBe('RedisBubble');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(RedisBubble.service).toBe('openevolve');
      expect(RedisBubble.bubbleName).toBe('redis');
      expect(RedisBubble.type).toBe('service');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new RedisBubble({
        operation: 'test1',
        host: 'localhost',
        port: '6379'
      } as any);

      const bubble2 = new RedisBubble({
        operation: 'test2',
        host: 'localhost',
        port: '6379'
      } as any);
