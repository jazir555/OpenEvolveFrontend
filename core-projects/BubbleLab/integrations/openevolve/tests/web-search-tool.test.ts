 ...
 *
 * Comprehensive tests for web-search tool bubble
 *
 * Test Coverage:
...
import { WebSearchTool } from '../tool-bubbles/web-search-tool';
...
describe('WebSearchTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new WebSearchTool({
        operation: 'test',
        query: 'test'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('WebSearchTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(WebSearchTool.service).toBe('openevolve');
      expect(WebSearchTool.bubbleName).toBe('web-search');
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new WebSearchTool({
        operation: 'test1',
        query: 'test'
      } as any);

      const bubble2 = new WebSearchTool({
        operation: 'test2',
        query: 'test'
      } as any);
