/**
 * WebhookRepeaterWorkflow Test Suite
 *
 * Comprehensive tests for webhook-repeater workflow bubble
 *
 * Test Coverage:
...
import { WebhookRepeaterWorkflow } from '../workflow-bubbles/webhook-repeater-workflow';

describe('WebhookRepeaterWorkflow', () => {
  let mockBubble;
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend WorkflowBubble properly', () => {
      const bubble = new WebhookRepeaterWorkflow({
        operation: 'test',
        url: 'https://test.com'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('WebhookRepeaterWorkflow');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(WebhookRepeaterWorkflow.service).toBe('openevolve');
      expect(WebhookRepeaterWorkflow.bubbleName).toBe('webhook-repeater');
      expect(WebhookRepeaterWorkflow.type).toBe('workflow');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new WebhookRepeaterWorkflow({
        operation: 'test1',
        url: 'https://test.com'
      } as any);

      const bubble2 = new WebhookRepeaterWorkflow({
        operation: 'test2',
        url: 'https://test.com'
      } as any);
