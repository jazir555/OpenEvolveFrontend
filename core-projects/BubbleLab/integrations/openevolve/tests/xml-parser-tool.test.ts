/**
 * XMLParserTool Test Suite
 *
 * Comprehensive tests for xml-parser tool bubble
 *
 * Test Coverage:
...
import { XMLParserTool } from '../tool-bubbles/xml-parser-tool';
...
describe('XMLParserTool', () => {
  ...
  describe('1. Base Class Inheritance', () => {
    it('should extend ToolBubble properly', () => {
      const bubble = new XMLParserTool({
        operation: 'test',
        xml: '<root/>'
      } as any);

      expect(bubble).toBeDefined();
      expect(bubble.constructor.name).toBe('XMLParserTool');
      expect(typeof bubble.action).toBe('function');
    });

    it('should have correct static properties', () => {
      expect(XMLParserTool.service).toBe('openevolve');
      expect(XMLParserTool.bubbleName).toBe('xml-parser');
      expect(XMLParserTool.type).toBe('tool');
    });
  ...
  describe('7. Request Deduplication', () => {
    ...
    it('should not deduplicate different requests', async () => {
      const bubble1 = new XMLParserTool({
        operation: 'test1',
        xml: '<root/>'
      } as any);

      const bubble2 = new XMLParserTool({
        operation: 'test2',
        xml: '<root/>'
      } as any);
