...
const IcrConfigOperationSchema = z.enum([
  'config',
]);
...
const IcrConfigParamsSchema = z.object({
  operation: IcrConfigOperationSchema,
...
type IcrConfigParams = z.input<typeof IcrConfigParamsSchema> & ServiceBubbleParams;

const IcrConfigResultSchema = z.object({
  success: z.boolean(),
  operation: z.string(),
...
type IcrConfigResult = z.output<typeof IcrConfigResultSchema> & BubbleOperationResult;

export class OpenEvolveIcrConfigBubble extends ServiceBubble<
  IcrConfigParams,
  IcrConfigResult
> {
  ...
  static readonly shortDescription = 'ICR config bubble';
  static readonly longDescription = `
    Fetch the ICR configuration.
  `;
  static readonly alias = 'openevolve-icr-config';

  constructor(params: IcrConfigParams, context?: BubbleContext) {
  ...
  protected async performAction(): Promise<IcrConfigResult> {
    const startTime = Date.now();
    try {
      switch (((this.params.operation as string) as string)) {
        case 'config':
          return await this.request('GET', `/icr/config`, undefined, startTime);
        default:
          return {
            success: false,
...
export default OpenEvolveIcrConfigBubble;
