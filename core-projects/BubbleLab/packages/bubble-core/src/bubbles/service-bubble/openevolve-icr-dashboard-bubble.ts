...
const IcrDashboardOperationSchema = z.enum([
  'dashboard',
]);
...
const IcrDashboardParamsSchema = z.object({
  operation: IcrDashboardOperationSchema,
...
type IcrDashboardParams = z.input<typeof IcrDashboardParamsSchema> & ServiceBubbleParams;

const IcrDashboardResultSchema = z.object({
  success: z.boolean(),
  operation: z.string(),
...
type IcrDashboardResult = z.output<typeof IcrDashboardResultSchema> & BubbleOperationResult;

export class OpenEvolveIcrDashboardBubble extends ServiceBubble<
  IcrDashboardParams,
  IcrDashboardResult
> {
  ...
  static readonly shortDescription = 'ICR dashboard bubble';
  static readonly longDescription = `
    Fetch the ICR dashboard.
  `;
  static readonly alias = 'openevolve-icr-dashboard';

  constructor(params: IcrDashboardParams, context?: BubbleContext) {
  ...
  protected async performAction(): Promise<IcrDashboardResult> {
    const startTime = Date.now();
    try {
      switch (((this.params.operation as string) as string)) {
        case 'dashboard':
          return await this.request('GET', `/icr/dashboard`, undefined, startTime);
        default:
          return {
            success: false,
...
export default OpenEvolveIcrDashboardBubble;
