   ...
   import { TwilioBubble } from "./twilio-bubble";

   describe("TwilioBubble", () => {
     it("does not throw on invalid params (captures validationError)", () => {
       expect(() => new TwilioBubble({ notARealField: 1 } as any)).not.toThrow();
     });

     it("action() returns a controlled error for invalid params", async () => {
       const bubble = new TwilioBubble({ notARealField: 1 } as any);
     ...
     it("constructs with provided params without throwing", () => {
       const bubble = new TwilioBubble({ operation: "sendSms", to: "+15551234567", from: "+15557654321", body: "hi" } as any);
       expect(bubble).toBeDefined();
     });

     it("exposes static metadata", () => {
       expect(TwilioBubble.bubbleName).toBeDefined();
       expect(TwilioBubble.service).toBeDefined();
       expect(TwilioBubble.schema).toBeDefined();
     });
   });
