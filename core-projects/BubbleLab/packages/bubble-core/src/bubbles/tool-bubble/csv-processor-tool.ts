...
export class CSVProcessorTool extends ToolBubble<
  CSVProcessorToolParams,
  CSVProcessorToolResult
> {
  /**
   * REQUIRED STATIC METADATA
   */
  static readonly type = 'tool' as const;
  static readonly bubbleName: BubbleName = 'csv-processor-tool';
  static readonly schema = CSVProcessorToolParamsSchema;
  static readonly resultSchema = CSVProcessorToolResultSchema;
  static readonly shortDescription =
    'Process, validate, transform, and analyze CSV files';
  ...
  constructor(
    params: CSVProcessorToolParamsInput,
  ...
  async performAction(
    context?: BubbleContext
  ): Promise<CSVProcessorToolResult> {
    ...
    try {
      ...
      let result: CSVProcessorToolResult;
   ...
   */
  private parseLine(line: string, delimiter: CSVDelimiter): string[] {
  ...
  private async parseCSV(): Promise<CSVProcessorToolResult> {
    const { csvData, delimiter, hasHeader, skipEmptyLines, trimWhitespace } =
      this.params;
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      try {
        ...
        if (this.params.maxRows && data.length >= this.params.maxRows) {
          console.warn(
            `[CSVProcessorTool] Reached max rows limit (${this.params.maxRows}), stopping parse`
          );
          break;
        }
      } catch (error) {
   ...
   */
  private async validateCSV(): Promise<CSVProcessorToolResult> {
    ...
    parseResult.data.forEach((row, rowIndex) => {
      ...
      Object.entries(this.params.validateSchema!).forEach(
        ([column, expectedType]) => {
   ...
   * Transform CSV data
   */
  private async transformCSV(): Promise<CSVProcessorToolResult> {
    const parseResult = await this.parseCSV();
      ...
      this.params.transformRules!.forEach((rule) => {
        ...
        switch (operation) {
          ...
          case 'trim':
            ...
            break;
          case 'replace':
            transformedRow[column] = String(currentValue).replace(
              new RegExp(value!, 'g'),
              value!
            );
            break;
          ...
          case 'format':
            ...
            transformedRow[column] = value!.replace(
              '{value}',
              String(currentValue)
            );
            break;
   ...
   * Filter CSV data
   */
  private async filterCSV(): Promise<CSVProcessorToolResult> {
    const parseResult = await this.parseCSV();
   ...
   * Export data to CSV
   */
  private async exportCSV(): Promise<CSVProcessorToolResult> {
   ...
   * Aggregate CSV data
   */
  private async aggregateCSV(): Promise<CSVProcessorToolResult> {
    const parseResult = await this.parseCSV();
    ...
    groups.forEach((groupRows, key) => {
      ...
      this.params.aggregations!.forEach((agg) => {
        ...
        switch (operation) {
          case 'sum':
            result = values.reduce((sum: number, v) => sum + Number(v), 0);
            break;
          case 'avg':
            result =
