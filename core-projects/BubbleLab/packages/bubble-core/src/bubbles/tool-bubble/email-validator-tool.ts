...
export class EmailValidatorTool extends ToolBubble<
  EmailValidatorToolParams,
  EmailValidatorToolResult
> {
  /**
   * REQUIRED STATIC METADATA
   */
  static readonly type = 'tool' as const;
  static readonly bubbleName: BubbleName = 'email-validator-tool';
  static readonly schema = EmailValidatorToolParamsSchema;
  static readonly resultSchema = EmailValidatorToolResultSchema;
  static readonly shortDescription =
    'Validate email addresses with syntax, domain, and quality checks';
  ...
  constructor(
    params: EmailValidatorToolParamsInput,
  ...
  async performAction(
    context?: BubbleContext
  ): Promise<EmailValidatorToolResult> {
    ...
    try {
      ...
      const results = await Promise.all(
        emailsToValidate.map((email) => this.validateEmail(email))
      );
  ...
  private async validateEmail(
    ...
    let domainValid: boolean | undefined = undefined;
   ...
   * Check for common TLD typos
   */
  private checkTLDTypos(domain: string): string | null {
