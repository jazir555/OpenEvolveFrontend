...
> {
  ...
  static readonly bubbleName: BubbleName = 'vector-search-tool';
  static readonly schema = VectorSearchToolParamsSchema;
  static readonly resultSchema = VectorSearchToolResultSchema;
  static readonly shortDescription =
    'Perform vector similarity search using Qdrant or other vector databases';
  ...
  constructor(
    params: VectorSearchToolParamsInput,
  ...
  async performAction(
    context?: BubbleContext
  ): Promise<VectorSearchToolResult> {
    void context; // Context available but not currently used
    const startTime = Date.now();

    try {
      const { vector, topK, scoreThreshold, collectionName, vectorDimension } =
        this.params;

      console.log(
        `[VectorSearchTool] Starting vector similarity search in collection: ${collectionName}`
      );
      ...
      console.log(
        `[VectorSearchTool] Search completed. Found ${filteredResults.length} results above threshold`
      );
   ...
   */
  private async performQdrantSearch(): Promise<
    Array<{
      id: string;
      score: number;
  ...
  > {
    ...
    try {
      ...
      const response = await fetch(
        `${qdrantUrl}/collections/${this.params.collectionName}/points/search`,
        {
          method: 'POST',
          headers,
   ...
   */
  private performInMemorySearch(): Array<{
    id: string;
    score: number;
    payload?: Record<string, unknown>;
    vector?: number[];
    ...
    const vectors = this.params.filter?.vectors as Array<{
      id: string;
      vector: number[];
      payload?: Record<string, unknown>;
    }>;
