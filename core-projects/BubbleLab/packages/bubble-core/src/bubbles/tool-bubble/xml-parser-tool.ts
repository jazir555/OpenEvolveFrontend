 ...
 * Configure parsing behavior for xml2js library
 */
interface XML2JSParseOptions {
  ...
  attrName?: string;
  attrValueProcess?: (value: string, name: string) => string;
  tagNameProcess?: (name: string) => string;
  valueProcess?: (value: string, name: string) => string;
  textKey?: string;
  xmldec?: {
    version?: string;
    encoding?: string;
...
/**
 * XML Builder Options
 * Configure XML generation behavior
 */
interface XML2JSBuilderOptions {
  rootName?: string;
  xmldec?: {
    version?: string;
    encoding?: string;
...
class ProductionXMLParser {
  private parser: any;
  private builder: any;
  private parseOptions: XML2JSParseOptions;
  private builderOptions: XML2JSBuilderOptions;

  constructor(parseOptions: XML2JSParseOptions = {}, builderOptions: XML2JSBuilderOptions = {}) {
    this.parseOptions = {
   ...
   */
  async build(obj: Record<string, unknown>): Promise<string> {
    await this.initialize();
   ...
   */
  extractNodes(obj: Record<string, unknown>, nodeNames: string[]): Array<Record<string, unknown>> {
   ...
   */
  queryPath(obj: Record<string, unknown>, path: string): unknown[] {
    ...
    const traverse = (current: unknown, index: number): void => {
      ...
      if (typeof current === 'object' && current !== null) {
        if (Array.isArray(current)) {
          ...
          traverse((current as Record<string, unknown>)[part], index + 1);
        }
      }
    };
   ...
   */
  async formatXML(xml: string, indent: string = '  '): Promise<string> {
    try {
      // Parse and rebuild to get proper formatting
      const obj = await this.parse(xml);
      const formatted = await this.build(obj as Record<string, unknown>);
      return formatted;
    } catch (error) {
   ...
   */
  countNodes(obj: unknown): { nodeCount: number; attributeCount: number } {
    let nodeCount = 0;
...
> {
  ...
  static readonly bubbleName: BubbleName = 'xml-parser-tool';
  static readonly schema = XMLParserToolParamsSchema;
  static readonly resultSchema = XMLParserToolResultSchema;
  ...
  constructor(
    params: XMLParserToolParamsInput,
  ...
  async performAction(
    ...
    try {
      ...
      let result: XMLParserToolResult;
   ...
   */
  private async parseXML(): Promise<XMLParserToolResult> {
   ...
   * Validate XML
   */
  private async validateXML(): Promise<XMLParserToolResult> {
   ...
   */
  private async extractNodes(): Promise<XMLParserToolResult> {
    ...
    let extractedNodes: Array<Record<string, unknown>> = [];
   ...
   * Query XML
   */
  private async queryXML(): Promise<XMLParserToolResult> {
   ...
   */
  private async generateXML(): Promise<XMLParserToolResult> {
   ...
   * Format XML
   */
  private async formatXML(): Promise<XMLParserToolResult> {
