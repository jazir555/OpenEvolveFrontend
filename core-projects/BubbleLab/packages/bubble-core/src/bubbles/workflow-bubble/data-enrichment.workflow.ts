...
import { CredentialType, type BubbleName, AvailableModels } from '@bubblelab/shared-schemas';
...
> {
  ...
  static readonly bubbleName: BubbleName = 'data-enrichment-workflow';
  static readonly schema = DataEnrichmentParamsSchema;
  static readonly resultSchema = DataEnrichmentResultSchema;
  static readonly shortDescription =
    'Multi-source data enrichment with AI-powered analysis';
   ...
   */
  private async performVectorSearch(): Promise<{ success: boolean; data?: unknown[] }> {
    try {
      ...
      if (result.success && result.data.json) {
        return { success: true, data: result.data.json as unknown[] };
      }
   ...
   */
  private async performDatabaseLookup(): Promise<{
    success: boolean;
    data?: unknown;
    rowsAffected?: number;
  }> {
  ...
  private async performAIAnalysis(
    ...
    enrichmentResults: DataEnrichmentResult['enrichmentResults']
  ): Promise<{ success: boolean; insights?: string; confidence?: number }> {
    try {
...
NOTES: [Any concerns or additional context]`,
   ...
   * Generate search query from record
   */
  private generateSearchQueryFromRecord(record: Record<string, unknown>): string {
    const keys = Object.keys(record);
    const searchableKeys = keys.filter(k =>
   ...
   */
  private extractSearchResults(response: unknown, searchEngine: string): unknown[] {
    ...
    switch (searchEngine) {
      ...
      case 'bing':
        return ((resp.webPages as any)?.value as unknown[]) || [];
      case 'duckduckgo':
        return (resp.RelatedTopics as unknown[]) || [];
      default:
        return [];
    }
  }
  ...
  private calculateDataQualityScore(
    ...
    if (enrichmentResults) {
      ...
      const successfulSources = sources.filter(s =>
        enrichmentResults[s as keyof typeof enrichmentResults]?.success
      );
