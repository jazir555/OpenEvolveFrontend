import { z } from 'zod';
import type { BubbleOperationResult } from '@bubblelab/shared-schemas';
import type { ServiceBubbleParams } from '../../types/bubble.js';
import { WorkflowBubble } from '../../types/workflow-bubble-class.js';
import type { BubbleContext } from '../../types/bubble.js';
import type { BubbleName } from '@bubblelab/shared-schemas';
import { BubbleFactory } from '../../bubble-factory.js';

const resolveBaseUrl = (): string => {
  const envUrl =
    (typeof process !== 'undefined' && process.env
      ? process.env.OPENEVOLVE_API_URL || process.env.OPENEVOLVE_API_BASE_URL
      : undefined) || '';
  return (envUrl.trim().length > 0 ? envUrl : 'http://localhost:8000').replace(/\/$/, '');
};

const OpenEvolveDecompositionToEvolutionParamsSchema = z
  .object({
    problem_statement: z.string().optional(),
    plan_id: z.string().optional(),
    workflow_type: z.string().default('evolution'),
    evolved_code: z.string().optional(),
    target_system: z.string().optional(),
    target_path: z.string().optional(),
    base_url: z.string().url().default(resolveBaseUrl()),
    auth_token: z.string().optional(),
    auto_deploy: z.boolean().default(true),
  })
  .passthrough();

type OpenEvolveDecompositionToEvolutionParams = z.input<
  typeof OpenEvolveDecompositionToEvolutionParamsSchema
> &
  ServiceBubbleParams;

const OpenEvolveDecompositionToEvolutionResultSchema = z.object({
  success: z.boolean(),
  operation: z.string(),
  data: z.unknown().optional(),
  steps: z.array(z.unknown()).optional(),
  error: z.string().optional(),
  timing: z.object({ total: z.number() }),
});

type OpenEvolveDecompositionToEvolutionResult = z.output<
  typeof OpenEvolveDecompositionToEvolutionResultSchema
> &
  BubbleOperationResult;

export class OpenEvolveDecompositionToEvolutionBubble extends WorkflowBubble<
  OpenEvolveDecompositionToEvolutionParams,
  OpenEvolveDecompositionToEvolutionResult
> {
  static readonly service = 'openevolve';
  static readonly authType = 'apikey' as const;
  static readonly bubbleName: BubbleName = 'openevolve-decomposition-to-evolution' as BubbleName;
  static readonly type = 'workflow' as const;
  static readonly schema = OpenEvolveDecompositionToEvolutionParamsSchema;
  static readonly resultSchema = OpenEvolveDecompositionToEvolutionResultSchema;
  static readonly shortDescription = 'OpenEvolve decomposition -> evolution workflow';
  static readonly longDescription = `
    Chains decomposition plan retrieval with an evolution trigger, validation,
    and application to turn a decomposed plan into applied evolved code.
  `;
  static readonly alias = 'openevolve-decomposition-to-evolution';

  constructor(
    params: OpenEvolveDecompositionToEvolutionParams,
    context?: BubbleContext
  ) {
    super(params, context);
  }

  protected async performAction(): Promise<OpenEvolveDecompositionToEvolutionResult> {
    const startTime = Date.now();
    const steps: unknown[] = [];
    try {
      const factory = await BubbleFactory.getInstance();

      const planRes = await factory
        .createBubble('openevolve-decomposition-plan-get' as unknown as BubbleName, {
          plan_id: this.params.plan_id,
          problem_statement: this.params.problem_statement,
          base_url: this.params.base_url,
          auth_token: this.params.auth_token,
        })
        .action();
      steps.push({ step: 'decomposition-plan-get', result: planRes.data });
      if (!planRes.success) {
        return this.fail(`decomposition-plan-get failed: ${planRes.error}`, steps, startTime);
      }

      const triggerRes = await factory
        .createBubble('openevolve-evolution-trigger' as unknown as BubbleName, {
          operation: 'create_and_run',
          problem_statement: this.params.problem_statement,
          workflow_type: this.params.workflow_type,
          base_url: this.params.base_url,
          auth_token: this.params.auth_token,
        })
        .action();
      steps.push({ step: 'evolution-trigger', result: triggerRes.data });
      if (!triggerRes.success) {
        return this.fail(`evolution-trigger failed: ${triggerRes.error}`, steps, startTime);
      }
      const triggerData = (triggerRes.data ?? {}) as any;
      const code = this.params.evolved_code ?? JSON.stringify(triggerData.raw ?? triggerData);

      const validationRes = await factory
        .createBubble('openevolve-evolution-validation' as unknown as BubbleName, {
          operation: 'validate',
          code,
          base_url: this.params.base_url,
          auth_token: this.params.auth_token,
        })
        .action();
      steps.push({ step: 'evolution-validation', result: validationRes.data });
      if (!validationRes.success) {
        return this.fail(`evolution-validation failed: ${validationRes.error}`, steps, startTime);
      }

      const applicationRes = await factory
        .createBubble('openevolve-evolution-application' as unknown as BubbleName, {
          operation: 'apply',
          workflow_id: triggerData.workflow_id,
          evolved_code: code,
          target_system: this.params.target_system,
          target_path: this.params.target_path,
          auto_deploy: this.params.auto_deploy,
          base_url: this.params.base_url,
          auth_token: this.params.auth_token,
        })
        .action();
      steps.push({ step: 'evolution-application', result: applicationRes.data });

      return {
        success: true,
        operation: 'decomposition-to-evolution',
        data: {
          plan: planRes.data,
          trigger: triggerRes.data,
          validation: validationRes.data,
          application: applicationRes.data,
        },
        steps,
        timing: { total: Date.now() - startTime },
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      return {
        success: false,
        operation: 'decomposition-to-evolution',
        error: message,
        steps,
        timing: { total: Date.now() - startTime },
      };
    }
  }

  private fail(error: string, steps: unknown[], startTime: number): OpenEvolveDecompositionToEvolutionResult {
    return {
      success: false,
      operation: 'decomposition-to-evolution',
      error,
      steps,
      timing: { total: Date.now() - startTime },
    };
  }
}

export default OpenEvolveDecompositionToEvolutionBubble;
