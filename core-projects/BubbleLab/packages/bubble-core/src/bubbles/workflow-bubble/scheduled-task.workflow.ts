...
export class ScheduledTaskWorkflow extends WorkflowBubble<ScheduledTaskParams, z.infer<typeof ScheduledTaskResultSchema>> {
  static readonly type = 'workflow' as const;
  static readonly bubbleName: BubbleName = 'scheduled-task-workflow';
  static readonly schema = ScheduledTaskParamsSchema;
  static readonly resultSchema = ScheduledTaskResultSchema;
  ...
  protected async performAction() {
    ...
    try {
      ...
      if (schedule.type === 'once' && schedule.runAt) {
        ...
        ScheduledTaskWorkflow.scheduledTasks.set(taskId, intervalObj as unknown as NodeJS.Timeout);
  ...
  private async executeTask(taskId: string, action: z.infer<typeof ScheduledTaskParamsSchema>['action']) {
