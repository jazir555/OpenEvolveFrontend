...
export class WebhookRepeaterWorkflow extends WorkflowBubble<
  WebhookRepeaterParams,
  WebhookRepeaterResult
> {
  static readonly type = 'workflow' as const;
  static readonly bubbleName: BubbleName = 'webhook-repeater-workflow';
  static readonly schema = WebhookRepeaterParamsSchema;
  static readonly resultSchema = WebhookRepeaterResultSchema;
  static readonly shortDescription =
    'Robust webhook delivery with retries and circuit breaker';
  ...
  // Circuit breaker state
  private static circuitBreakerState: Map<string, { isOpen: boolean; failureCount: number; lastFailureTime?: Date }> = new Map();

  constructor(params: WebhookRepeaterParams, context?: BubbleContext) {
    super(params, context);
  }

  protected async performAction(): Promise<WebhookRepeaterResult> {
    ...
    const deliveryAttempts: WebhookRepeaterResult['deliveryAttempts'] = [];
  ...
  private calculateBackoffDelay(
    attempt: number,
    strategy: Partial<z.infer<typeof RetryStrategySchema>>
  ): number {
  ...
  static getCircuitBreakerState(webhookUrl: string):
    | { isOpen: boolean; failureCount: number; lastFailureTime?: Date }
    | undefined {
