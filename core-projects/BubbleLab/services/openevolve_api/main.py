...
# records (RUNNING -> COMPLETED/FAILED/...) since the engine has no callback.
try:
    from .api import workflow_status_sync
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    # Start the background engine-status sync poller (best-effort, daemon
    # thread so it never blocks shutdown). Resilient to an unreachable engine.
    try:
        workflow_status_sync.start_status_sync_loop(interval_seconds=10)
    except Exception as exc:  # pragma: no cover - defensive
        logger.error(
            "status_sync_start_failed",
            error=str(exc),
