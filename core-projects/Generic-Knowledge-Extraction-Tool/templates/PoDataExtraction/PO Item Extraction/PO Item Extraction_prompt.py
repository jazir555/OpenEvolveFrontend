...
extraction_prompt = """
TASK: PO Item Extraction

EXTRACTION TASK:
Extracts all item lines from the PO, capturing Material Number, Quantity, Description, and Delivery Date for each item.
...
class POItemExtractionModel(BaseModel):
    materialnumber: str = Field(description="Unique identifier for the item; used for linking with BOM.")
    quantity: int = Field(description="Number of units ordered.")
    description: str = Field(description="Description of the item.")
    deliverydate: str = Field(description="Required delivery date for the item.")
```
...
OUTPUT FORMAT:
Return the extracted information as a JSON object that exactly matches the POItemExtractionModel structure shown above.
...
Return the extracted information as a JSON array of objects (if multiple items) or single JSON object matching the POItemExtractionModel structure.
        """
