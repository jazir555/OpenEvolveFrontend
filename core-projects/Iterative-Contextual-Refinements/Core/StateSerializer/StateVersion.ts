...
export function convertLegacyToVersioned(legacyConfig: Record<string, unknown>): VersionedState {
    ...
    const data: ExportedConfigV1 = {
        ...
        customPrompts: {
            ...
            mathsolver: legacyConfig.customPromptsMathSolver,
            generativeui: legacyConfig.customPromptsGenerativeUI,
            react: legacyConfig.customPromptsReact,
        },
...
function extractLegacyModeState(config: Record<string, unknown>, mode: ApplicationMode): unknown {
    switch (mode) {
        ...
        case 'mathsolver':
            return config.activeMathSolverState;
        case 'generativeui':
            return config.activeGenerativeUIState;
        case 'react':
            return config.activeReactState;
        default:
            return null;
    }
}
