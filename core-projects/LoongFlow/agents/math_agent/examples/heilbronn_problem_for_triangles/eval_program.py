"""
Evaluator for the Heilbronn problem for triangles (n=11), aligned with standard specifications.
"""
...
# --- Constants ---
N_POINTS = 11
TARGET_VALUE = 0.0365  # sota of n=11
...
def evaluate(program_path):
    ...
    status = "success"
    try:
        points, min_area = run_with_timeout(program_path, n_points=N_POINTS)
        ...
        # If valid, use the returned min_area. Otherwise, it's 0.
        final_min_area = (
            (
                min_triangle_area(points)
                / triangle_area((0.0, 0.0), (1.0, 0.0), (0.5, HALF_SQRT3))
            )
            if is_valid
            else 0.0
        )
        target_ratio = final_min_area / TARGET_VALUE if is_valid else 0.0
