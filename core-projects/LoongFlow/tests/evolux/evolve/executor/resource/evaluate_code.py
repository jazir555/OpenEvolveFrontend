...
def validate_generating(step_function_heights_1):
    ...
    is_valid = (
        len(validation_details["has_negative_val"]) == 0
    )
...
def evaluate(program_path):
    ...
    try:
        ...
        if valid and target_ratio > 0.98:  # Near-optimal solutions
            artifacts["stdout"] = f"Excellent generating! Achieved {target_ratio:.1%} of target value"
            artifacts[
                "C_lower_bound_stats"
            ] = f"C_lower_bound: {C_lower_bound:.6f}"
...
def evaluate_stage1(program_path):
    ...
    try:
        ...
        try:
            start_time = time.time()
            step_function_heights_1 = run_with_timeout(program_path, timeout_seconds=1800)
            ...
            if not valid:
                ...
                if validation_details.get("has_negative_val"):
                    artifacts["has_negative_val"] = validation_details["has_negative_val"][
                        0
                    ]  # Just first issue
