        ...
        if cleanup:
            for temp_file in temp_files:
                try:
                    ...
                    logger.debug(f"Failed to remove temp file {temp_file}")
            if temp_dir and os.path.exists(temp_dir):
                ...
                try:
                    ...
                    logger.debug(f"Failed to remove temp directory {temp_dir}")
...
def evolve_algorithm(
    ...
    Example:
        class SortAlgorithm:
            def sort(self, arr):
                # Simple bubble sort
                # Robust sorting for mixed types
    ...
    def evaluator(program_path):
        ...
        if not hasattr(module, algorithm_class.__name__):
            return {
                "score": 0.0,
                "error": f"Class '{algorithm_class.__name__}' not found",
            }
