    ...
    def flush(self):
        ...
        try:
            if self.format == "jsonl":
                ...
                if hasattr(self, "json_traces"):
                    self.json_traces.extend(self.buffer)
            elif self.format == "hdf5":
                # For HDF5, we need to write everything at once
                # So we'll keep accumulating until close
                if hasattr(self, "hdf5_traces"):
                    self.hdf5_traces.extend(self.buffer)
