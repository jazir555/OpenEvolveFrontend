...
class CodeIndexer:
    ...
    def _get_default_models(self, config_path: str):
        """
        ...
        try:
            if os.path.exists(config_path):
                ...
                anthropic_model = llm_config.get(
                    "anthropic_default_model", "claude-sonnet-4-20250514"
                )
                openai_model = llm_config.get("openai_default_model", "o3-mini")
                google_model = llm_config.get("google_default_model", "gemini-2.0-flash")
    ...
    def __init__(
        ...
        self.default_models = self._get_default_models(self.indexer_config_path)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    await asyncio.sleep(
                        self.retry_delay * (attempt + 1)  # Exponential backoff
                    )
            ...
            return """
{
    ...
    "main_functions": ["main_function", "helper_function", "CodeIndexer"],
    "key_concepts": ["data_processing", "algorithm", "LLM"],
    "dependencies": ["numpy", "pandas"],
    "summary": "Mock analysis of a code file that uses an LLM."
}
"""
...
"""
    ...
    async def process_repository(self, repo_path: Path) -> RepoIndex:
        ...
        repo_index = RepoIndex(
            ...
            analysis_metadata={
                ...
                "cache_hits": len(self.content_cache) if self.enable_content_caching else 0,
            },
        )
    ...
    async def _process_files_concurrently(self, files_to_analyze: list) -> tuple:
        ...
        async def _process_with_semaphore(file_path: Path, index: int, total: int):
            async with semaphore:
                ...
                if index > 1:
                    await asyncio.sleep(
                        self.request_delay * 0.5  # Reduced delay for concurrent processing
                    )
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(_process_with_semaphore(file_path, i, len(files_to_analyze)))
                for i, file_path in enumerate(files_to_analyze, 1)
            ]
    ...
    def _extract_repository_statistics(self, repo_index: RepoIndex) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(r.confidence_score for r in repo_index.relationships) / len(repo_index.relationships)
            ...
            "performance_metrics": {
                ...
                "filtering_efficiency": {
                    "average_filtering_efficiency": round(
                        sum(s.get("filtering_efficiency", 0) for s in statistics_data) / total_repos,
                        2,
                    )
                    if total_repos
                    else 0,
...
async def main():
    ...
    code_base_path = "code_base"  # Will use config file value if None
    output_dir = "indexes"  # Will use config file value if None
