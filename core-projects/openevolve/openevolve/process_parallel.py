...
def _log_to_queue(message: str, level: str = "INFO") -> None:
    ...
    if _worker_log_queue:
        _worker_log_queue.put(f"[{level}] {message}")
...
def _run_iteration_worker(
    ...
    try:
        ...
        # Build prompt
        _log_to_queue(f"Building prompt for iteration {iteration}")
        ...
        try:
            ...
            _log_to_queue(f"LLM generation failed: {e}", level="ERROR")
        ...
        if llm_response is None:
            _log_to_queue("LLM returned None response", level="WARNING")
            return SerializableResult(
        ...
        if _worker_config.diff_based_evolution:
            ...
            if not new_code:
                _log_to_queue("No valid code found in response", level="WARNING")
                return SerializableResult(
