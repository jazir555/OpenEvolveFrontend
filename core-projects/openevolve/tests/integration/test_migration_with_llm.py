...
class TestMigrationWithLLM:
    ...
    async def test_island_migration_no_duplicates_real_evolution(
        ...
        programs_with_migration_data = [
            p
            for p in controller.database.programs.values()
        ...
        print(
            f"Last migration generation: {controller.database.last_migration_generation}"
        )
    ...
    async def test_per_island_map_elites_isolation(
        ...
        for island_idx, island_map in enumerate(
            controller.database.island_feature_maps
        ):
    ...
    async def test_migration_preserves_program_quality(
        ...
        for migrant in migrant_programs:
            ...
            assert migrant.parent_id is not None, (
                f"Migrant {migrant.id} should have parent_id"
            )
            ...
            if parent:  # Parent might have been replaced in MAP-Elites
                ...
                assert migrant.language == parent.language, (
    ...
    async def test_migration_timing_logic(
        ...
        # Basic sanity checks
        assert all(gen >= 0 for gen in final_generations), (
            "All generations should be non-negative"
        )
        assert final_migration_gen >= 0, (
            "Last migration generation should be non-negative"
        )
    ...
    async def test_single_island_no_migration(
        ...
        assert len(migrant_programs) == 0, (
            "Single island should not create migrant programs"
        )
