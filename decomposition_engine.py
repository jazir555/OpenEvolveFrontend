...
class SemanticDecomposition(DecompositionStrategyBase):
    ...
    def decompose(self, problem: ProblemDefinition) -> List[SubProblem]:
        ...
        Falls back to template-based decomposition if LLM is unavailable.
        """
        logger.info(f"Semantic decomposition for problem: {problem.id}")
        
        # Try LLM-based decomposition first (primary path)
        if self.openevolve_client:
            try:
                ...
                if sub_problems and len(sub_problems) >= 2:
                    ...
                    logger.warning("LLM decomposition returned insufficient sub-problems, using fallback")
            except Exception as e:
                logger.warning(f"LLM semantic decomposition failed: {e}, using fallback")
        else:
            logger.info("OpenEvolve client not available, using template-based decomposition")
        
        # Fallback to template-based decomposition (reliability path)
        return self._decompose_template_based(problem)
...
Begin decomposition:"""
        ...
        for section_idx, section in enumerate(sections):
            ...
            try:
                ...
                try:
                    ...
                    priority = 5
                    logger.debug(f"Using default priority {priority} for: {title}")
                ...
                try:
                    ...
                    effort = 8
                    logger.debug(f"Using default effort {effort}h for: {title}")
    ...
    def _decompose_template_based(self, problem: ProblemDefinition) -> List[SubProblem]:
        ...
        logger.info(f"Created {len(sub_problems)} sub-problems via template decomposition")
        return sub_problems
...
class DependencyDecomposition(DecompositionStrategyBase):
    ...
    PRODUCTION IMPLEMENTATION:
    - LLM-powered dependency analysis for intelligent ordering
    ...
    def decompose(self, problem: ProblemDefinition) -> List[SubProblem]:
        ...
        # Try LLM-based dependency analysis
        if self.openevolve_client:
            try:
                ...
                logger.warning(f"LLM dependency analysis failed: {e}, using fallback")
        
        # Fallback: simple sequential dependencies
        for i in range(1, len(sub_problems)):
            sub_problems[i].dependencies = [sub_problems[i-1].id]
        
        logger.info(f"Created {len(sub_problems)} sub-problems with sequential dependencies")
        return sub_problems
...
class ComplexityDecomposition(DecompositionStrategyBase):
    ...
    def decompose(self, problem: ProblemDefinition) -> List[SubProblem]:
        ...
        for sp in sub_problems:
            if sp.complexity_score.overall_complexity > max_complexity:
                ...
                # Try LLM-based intelligent splitting
                if self.openevolve_client:
                    try:
                        split_sps = self._split_with_llm(sp, problem)
                        if split_sps and len(split_sps) >= 2:
                            refined_sub_problems.extend(split_sps)
                            continue
                    except Exception as e:
                        logger.warning(f"LLM splitting failed: {e}, using heuristic")
                
                # Fallback to heuristic splitting
    ...
    def _split_with_llm(self, sub_problem: SubProblem, parent_problem: ProblemDefinition) -> List[SubProblem]:
...
Provide 2-3 splits:"""
            ...
            try:
                ...
                split_sp = SubProblem(
                    ...
                    complexity_score=ComplexityScore(
                        ...
                        explanation=f"Split from complex sub-problem (LLM-guided)"
                    ),
    ...
    def _split_complex_subproblem(self, sub_problem: SubProblem) -> List[SubProblem]:
        """Split a complex sub-problem into simpler ones (heuristic fallback)."""
...
class DecompositionEngine:
    ...
    def select_strategy(self, problem: ProblemDefinition) -> str:
        """
        Chooses optimal decomposition strategy using intelligent analysis.
        ...
        Returns:
            Strategy name
        """
        # Try LLM-based strategy selection for sophisticated analysis
        try:
            from openevolve_client import OpenEvolveClient, OPENEVOLVE_AVAILABLE
            if OPENEVOLVE_AVAILABLE:
            ...
            self.logger.warning(f"LLM strategy selection failed: {e}, using heuristic")
        
        # Fallback to heuristic selection
        return self._select_strategy_heuristic(problem)
...
Your selection:"""
    ...
    def _select_strategy_heuristic(self, problem: ProblemDefinition) -> str:
        """Heuristic-based strategy selection."""
        complexity = problem.complexity_score.overall_complexity
        
        # Very high complexity problems benefit from hybrid approach
        if complexity > 8.0:
            return 'hybrid'
