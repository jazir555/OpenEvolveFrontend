...
class WorkflowOrchestrator {
    constructor(registry) {
        ...
        structuredLogger_1.apiLogger.info('Workflow Orchestrator initialized', this.correlationContext);
    }
    /**
     * Execute a workflow
     */
    async executeWorkflow(workflow, input = {}) {
        ...
        structuredLogger_1.apiLogger.info('Executing workflow', {
            ...this.correlationContext,
        ...
        try {
            ...
            const workflowStatus = stepExecution.errors.length > 0 && stepResults.size === 0 ? 'failed' : 'completed';
            context.status = workflowStatus === 'failed' ? 'failed' : 'completed';
            const duration = Date.now() - startTime;
            if (workflowStatus === 'failed') {
                structuredLogger_1.apiLogger.warn('Workflow completed with no successful steps', {
                    ...this.correlationContext,
                    workflow_id: workflow.id,
                    execution_id: executionId,
                ...
                structuredLogger_1.apiLogger.info('Workflow completed successfully', {
                    ...this.correlationContext,
                    workflow_id: workflow.id,
                    execution_id: executionId,
                    duration_ms: duration
            ...
            structuredLogger_1.apiLogger.error('Workflow execution failed', error, {
                ...this.correlationContext,
                workflow_id: workflow.id,
                execution_id: executionId,
                duration_ms: duration
     ...
     */
    async executeSteps(workflow, context, executionOrder) {
        const results = new Map();
        const errors = [];
        for (const step of executionOrder) {
            // Check if step should be executed
            if (step.condition && !step.condition(context)) {
                structuredLogger_1.apiLogger.info('Skipping step due to condition', {
                    ...this.correlationContext,
                    step_id: step.id,
                    step_name: step.name
                });
            ...
            if (step.dependsOn) {
                ...
                if (!dependenciesMet) {
                    structuredLogger_1.apiLogger.warn('Step dependencies not met, skipping', {
                        ...this.correlationContext,
                        step_id: step.id,
            ...
            try {
                ...
                structuredLogger_1.apiLogger.error('Step execution failed', error, {
                    ...this.correlationContext,
                    step_id: step.id,
                    step_name: step.name
                });
    ...
    async executeStep(step, context, workflow) {
        const startTime = Date.now();
        let retryCount = 0;
        structuredLogger_1.apiLogger.info('Executing step', {
            ...this.correlationContext,
            step_id: step.id,
            step_name: step.name,
            plugin: step.plugin,
        ...
        try {
            ...
            structuredLogger_1.apiLogger.info('Step completed', {
                ...this.correlationContext,
                step_id: step.id,
                duration_ms: duration
            });
    ...
    async cancelWorkflow(executionId) {
        ...
        this.activeWorkflows.delete(executionId);
        structuredLogger_1.apiLogger.info('Workflow cancelled', {
            ...this.correlationContext,
            execution_id: executionId
        });
        return true;
