...
exports.DatapizzaClient = void 0;
...
class DatapizzaClient {
    constructor(config) {
        ...
        if (!config.baseUrl) {
            throw new Error('DatapizzaClient: baseUrl is REQUIRED. ' +
                'Set DATAPIZZA_BASE_URL environment variable.');
        }
        if (!config.timeout || config.timeout <= 0) {
            throw new Error('DatapizzaClient: timeout is REQUIRED and must be > 0. ' +
                'Set DATAPIZZA_TIMEOUT_MS environment variable.');
        }
        this.config = config;
        this.correlationId = `datapizza-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        structuredLogger_1.logger.info('DatapizzaClient initialized', {
    ...
    configure(config) {
        ...
        structuredLogger_1.logger.info('DatapizzaClient configuration updated', {
            correlation_id: this.correlationId,
            source_service: 'datapizza-client',
        });
    }
    ...
    async testConnection() {
        ...
        try {
            structuredLogger_1.logger.info('Testing Datapizza connection', context);
            const response = await this.fetchWithTimeout('/health', {
                method: 'GET',
            });
            if (response.ok) {
                structuredLogger_1.logger.info('Datapizza connection successful', {
                    ...context,
                ...
                structuredLogger_1.logger.warn('Datapizza connection failed', {
                    ...context,
                    status: response.status,
            ...
            structuredLogger_1.logger.error('Datapizza connection test failed', error, context);
            throw error;
        }
    }
    /**
    ...
    async runPipeline(request) {
        ...
        try {
            structuredLogger_1.logger.info('Running Datapizza pipeline', context);
            ...
            if (!response.ok) {
                ...
                structuredLogger_1.logger.error('Pipeline run failed', new Error(errorText), {
                    ...context,
                    status: response.status,
                    status_text: response.statusText
                });
            ...
            structuredLogger_1.logger.info('Pipeline run successful', {
                ...context,
            ...
            structuredLogger_1.logger.error('Pipeline run error', error, context);
            throw error;
        }
    }
    /**
    ...
    async processData(request) {
        ...
        try {
            structuredLogger_1.logger.info('Processing data with Datapizza', context);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Data processing failed', new Error(errorText), {
                    ...context,
                    status: response.status
                });
            ...
            const result = await response.json();
            structuredLogger_1.logger.info('Data processing successful', {
                ...context,
            ...
            structuredLogger_1.logger.error('Data processing error', error, context);
            throw error;
        }
    }
    /**
    ...
    async queryData(request) {
        ...
        try {
            structuredLogger_1.logger.info('Querying Datapizza', context);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Data query failed', new Error(errorText), {
                    ...context,
                    status: response.status
                });
            ...
            const result = await response.json();
            structuredLogger_1.logger.info('Data query successful', {
                ...context,
            ...
            structuredLogger_1.logger.error('Data query error', error, context);
            throw error;
        }
    }
    /**
    ...
    async getPipelineRecommendation(dataSource, context) {
        ...
        try {
            structuredLogger_1.logger.info('Getting pipeline recommendation', logContext);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Failed to get recommendation', new Error(errorText), {
                    ...logContext,
                    status: response.status
                });
            ...
            const result = await response.json();
            structuredLogger_1.logger.info('Pipeline recommendation received', {
                ...logContext,
            ...
            structuredLogger_1.logger.error('Pipeline recommendation error', error, logContext);
            throw error;
        }
    }
    /**
    ...
    async detectDataDomain(data) {
        ...
        try {
            structuredLogger_1.logger.info('Detecting data domain', context);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Domain detection failed', new Error(errorText), {
                    ...context,
                    status: response.status
                });
            ...
            const result = await response.json();
            structuredLogger_1.logger.info('Domain detection successful', {
                ...context,
            ...
            structuredLogger_1.logger.error('Domain detection error', error, context);
            throw error;
        }
    }
    /**
    ...
    async isProcessableData(data) {
        ...
        try {
            structuredLogger_1.logger.info('Checking if data is processable', context);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Processable check failed', new Error(errorText), {
                    ...context,
                    status: response.status
                });
                return false;
            ...
            const result = await response.json();
            structuredLogger_1.logger.info('Processable check successful', {
                ...context,
            ...
            structuredLogger_1.logger.error('Processable check error', error, context);
            return false;
        }
    }
    /**
    ...
    async clearCache() {
        ...
        try {
            structuredLogger_1.logger.info('Clearing Datapizza cache', context);
            const response = await this.fetchWithTimeout('/cache/clear', {
                method: 'POST',
            });
            if (!response.ok) {
                const errorText = await response.text();
                structuredLogger_1.logger.error('Cache clear failed', new Error(errorText), {
                    ...context,
                    status: response.status
                });
                throw new Error(`Cache clear failed: ${response.statusText}`);
            }
            structuredLogger_1.logger.info('Cache cleared successfully', context);
        }
        catch (error) {
            structuredLogger_1.logger.error('Cache clear error', error, context);
            throw error;
        }
    }
    /**
...
exports.DatapizzaClient = DatapizzaClient;
//# sourceMappingURL=DatapizzaClient.js.map