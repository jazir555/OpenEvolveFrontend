...
exports.createDatapizzaPlugin = createDatapizzaPlugin;
exports.useDatapizzaPlugin = useDatapizzaPlugin;
const react_1 = require("react");
const react_toastify_1 = require("react-toastify");
const plugin_types_1 = require("../types/plugin-types");
const DatapizzaClient_1 = require("../services/DatapizzaClient");
const DatapizzaService_1 = require("../services/DatapizzaService");
// Global plugin state management
let globalPluginState = { ...plugin_types_1.DEFAULT_DATAPIZZA_CONFIG, ...{
        status: 'idle',
        operationHistory: [],
        statistics: {
            totalOperations: 0,
 ...
 */
function createDatapizzaPlugin(initialConfig) {
    ...
    const client = new DatapizzaClient_1.DatapizzaClient({
        baseUrl: config.serverUrl,
        apiKey: config.apiKey,
        timeout: config.timeout ?? plugin_types_1.DEFAULT_DATAPIZZA_CONFIG.timeout
    });
    const service = new DatapizzaService_1.DatapizzaService(client);
    ...
    const pluginMethods = {
        ...
        async initialize(configUpdate) {
            try {
                ...
                state.status = 'ready';
                react_toastify_1.toast.success('Datapizza plugin initialized successfully');
            }
            catch (error) {
                state.status = 'error';
                react_toastify_1.toast.error(`Failed to initialize Datapizza plugin: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        },
        async updateConfig(configUpdate) {
            try {
                ...
                react_toastify_1.toast.success('Datapizza configuration updated successfully');
            }
            catch (error) {
        ...
        async resetConfig() {
            Object.assign(config, plugin_types_1.DEFAULT_DATAPIZZA_CONFIG);
            Object.assign(state, plugin_types_1.DEFAULT_DATAPIZZA_CONFIG);
            client.configure({
                baseUrl: config.serverUrl,
                apiKey: config.apiKey,
                timeout: config.timeout ?? plugin_types_1.DEFAULT_DATAPIZZA_CONFIG.timeout
            });
            react_toastify_1.toast.success('Datapizza configuration reset to defaults');
        },
        async runPipeline(dataSource, pipelineType) {
            ...
            try {
                ...
                react_toastify_1.toast.error(`Pipeline failed: ${errorMessage}`);
        ...
        async processData(data, processingType) {
            ...
            try {
                ...
                state.currentOperation = undefined;
                const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                react_toastify_1.toast.error(`Data processing failed: ${errorMessage}`);
                // Add error to operation history
                state.operationHistory.unshift({
                    id: Date.now().toString(),
                    type: 'processing',
        ...
        async queryData(query, dataSource) {
            ...
            try {
                ...
                state.currentOperation = undefined;
                const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                react_toastify_1.toast.error(`Query failed: ${errorMessage}`);
                // Add error to operation history
                state.operationHistory.unshift({
                    id: Date.now().toString(),
                    type: 'query',
        ...
        async getPipelineRecommendation(dataSource, context) {
            ...
            try {
                ...
                react_toastify_1.toast.error(`Failed to get pipeline recommendation: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        },
        async detectDataDomain(data) {
            ...
            try {
                ...
                react_toastify_1.toast.error(`Failed to detect data domain: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        },
        async isProcessableData(data) {
            ...
            try {
                ...
                react_toastify_1.toast.error(`Failed to check data processability: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        },
        async clearCache() {
            try {
                ...
                react_toastify_1.toast.success('Datapizza cache cleared successfully');
            }
            catch (error) {
        ...
        getContext() {
            return {
                config: { ...config },
                state: { ...state },
                availablePipelineTypes: [...plugin_types_1.DATAPIZZA_PIPELINE_TYPES],
                dataDomains: [...plugin_types_1.DATAPIZZA_DATA_DOMAINS],
                capabilities: {
...
function useDatapizzaPlugin() {
    const [plugin] = (0, react_1.useState)(() => createDatapizzaPlugin());
    (0, react_1.useEffect)(() => {
