...
exports.GraphitiMemoryManager = void 0;
...
class GraphitiMemoryManager {
    constructor(config) {
        ...
        if (!this.config.graphitiAdapter.isInitialized()) {
            throw new Error('GraphitiMemoryManager: Graphiti adapter is not initialized. ' +
    ...
    async storeRefinementInsights(insights, sessionId, correlationId) {
        ...
        try {
            ...
            const validation = (0, canonical_1.validateMemorySchema)(require('./canonical').RefinementInsightsSchema, insights);
            ...
            const result = await this.config.graphitiAdapter.addEpisode(episodeOperation, cid);
            return {
    ...
    async storeContextualSession(session, correlationId) {
        ...
        try {
            // Validate session
            const validation = (0, canonical_1.validateMemorySchema)(require('./canonical').ContextualSessionSchema, session);
            if (!validation.success) {
                return {
                    success: false,
                    processing_time_ms: Date.now() - startTime,
     ...
     */
    async retrieveHistoricalKnowledge(query, contextWindow, correlationId) {
        const cid = correlationId || (0, uuid_1.v4)();
        try {
            // Build search query for Graphiti
            const searchQuery = this.buildSearchQuery(query);
            // Search Graphiti
            const searchResult = await this.config.graphitiAdapter.search(searchQuery, {
     ...
     */
    async retrieveContextualMemory(sessionId, correlationId) {
        const cid = correlationId || (0, uuid_1.v4)();
        try {
            // Search for session by group_id
            const searchResult = await this.config.graphitiAdapter.search(`session_id:${sessionId}`, {
                max_results: 1,
                group_ids: [sessionId]
            });
     ...
     */
    async buildContextualGraph(sessions, correlationId) {
        const cid = correlationId || (0, uuid_1.v4)();
        try {
            const nodes = [];
            const edges = [];
        ...
        try {
            ...
            for (const outcome of outcomes) {
                ...
                for (const pattern of outcome.successful_patterns) {
                    ...
                    await this.storePatternInsight(session.session_id, pattern, outcome.outcome, outcome.quality_score, cid);
                    patternsLearned++;
                }
                ...
                for (const lesson of outcome.lessons_learned) {
                    await this.storeLessonLearned(session.session_id, lesson, outcome, cid);
                    insightsExtracted++;
                }
     ...
     */
    formatInsightsAsEpisode(insights) {
        const sections = [
        ...
        for (const iteration of insights.iterations) {
            sections.push('', `### Iteration ${iteration.iteration_number}`, `- Type: ${iteration.refinement_type}`, `- Outcome: ${iteration.outcome}`, `- Execution Time: ${iteration.execution_time_ms}ms`, iteration.insights.length > 0 ? `- Insights: ${iteration.insights.join('; ')}` : '', iteration.suggested_features ? `- Suggested Features: ${iteration.suggested_features}` : '', iteration.bug_fixes ? `- Bug Fixes: ${iteration.bug_fixes}` : '');
        }
     ...
     */
    formatSessionAsEpisode(session) {
        const sections = [
        ...
        for (const interaction of session.interactions) {
            sections.push('', `### ${interaction.agent_type}`, `- Timestamp: ${interaction.timestamp_utc}`, interaction.execution_time_ms ? `- Execution Time: ${interaction.execution_time_ms}ms` : '', `- Content: ${interaction.content.substring(0, 200)}${interaction.content.length > 200 ? '...' : ''}`);
        }
        if (session.memory_compression_events && session.memory_compression_events.length > 0) {
            ...
            for (const event of session.memory_compression_events) {
                sections.push(`- ${event.timestamp_utc}: Compressed ${event.compressed_message_count} messages`, `  Ratio: ${event.compression_ratio}, Saved: ${event.bytes_saved} bytes`);
            }
        }
        return sections.filter(s => s !== '').join('\n');
    }
     ...
     */
    transformEdgeToKnowledge(edge, query) {
        try {
            // Extract knowledge from edge attributes
            const attrs = edge.attributes;
            return {
                session_id: attrs.session_id || (0, uuid_1.v4)(),
                prompt: edge.fact,
     ...
     */
    filterKnowledge(knowledge, query) {
     ...
     */
    buildPatternRelationships(sessions) {
     ...
     */
    async storePatternInsight(sessionId, pattern, outcome, qualityScore, correlationId) {
        const episodeOperation = {
     ...
     */
    async storeLessonLearned(sessionId, lesson, outcome, correlationId) {
        const episodeOperation = {
...
exports.GraphitiMemoryManager = GraphitiMemoryManager;
//# sourceMappingURL=graphiti-memory.js.map