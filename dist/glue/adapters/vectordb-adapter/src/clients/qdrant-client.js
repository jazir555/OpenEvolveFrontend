...
class QdrantClient {
    constructor(config) {
        this.logger = new logger_1.Logger('vectordb-adapter:qdrant-client');
        this.config = {
            timeout: 5000,
            maxRetries: 3,
            ...config,
    ...
    async upsert(request) {
        return this.circuitBreaker.execute(async () => {
            const points = request.entries.map(vectordb_canonical_1.transformCanonicalToQdrant);
            await (0, retry_1.retryWithJitter)(async () => {
                const response = await fetch(`${this.baseUrl}/collections/${request.collection_name}/points`, {
                    method: 'PUT',
                    headers: this.headers,
    ...
    async search(collectionName, query) {
        return this.circuitBreaker.execute(async () => {
            ...
            const response = await fetch(`${this.baseUrl}/collections/${collectionName}/points/query`, {
                method: 'POST',
                headers: this.headers,
            ...
            const results = data.result.map((point) => ({
                entry: (0, vectordb_canonical_1.transformQdrantToCanonical)(point),
                score: point.score || 0,
    ...
    async delete(request) {
        return this.circuitBreaker.execute(async () => {
            if (request.delete_all) {
                ...
                await (0, retry_1.retryWithJitter)(async () => {
                    const response = await fetch(`${this.baseUrl}/collections/${request.collection_name}`, {
                        method: 'DELETE',
                        headers: this.headers,
                        signal: AbortSignal.timeout(this.config.timeout),
                    });
