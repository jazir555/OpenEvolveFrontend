...
describe('Idempotency Contract Tests', () => {
    describe('idempotentCreate', () => {
        it('should return existing resource if already exists', async () => {
            ...
            let checkCalled = 0;
            let createCalled = 0;
            const result = await (0, idempotency_1.idempotentCreate)(async () => {
                checkCalled++;
    ...
    describe('upsert', () => {
        it('should update if resource exists', async () => {
            ...
            const result = await (0, idempotency_1.upsert)(async () => {
        ...
        it('should be idempotent across multiple calls', async () => {
            let callCount = 0;
            for (let i = 0; i < 3; i++) {
                await (0, idempotency_1.upsert)(async () => {
                    callCount++;
                    return { exists: false };
                }, async () => {
                    callCount++;
    ...
    describe('deduplicate', () => {
        it('should remove duplicates based on id', () => {
            ...
            const deduplicated = (0, idempotency_1.deduplicate)(items);
        ...
        it('should use custom getId function', () => {
            ...
            const deduplicated = (0, idempotency_1.deduplicate)(items, (item) => item.name);
            expect(deduplicated.length).toBe(2);
            expect(deduplicated.map(d => d.name)).toEqual(['Alice', 'Bob']);
        });
        it('should handle empty array', () => {
            const result = (0, idempotency_1.deduplicate)([]);
    ...
    describe('idempotentBatch', () => {
        it('should process unique items', async () => {
            ...
            const results = await (0, idempotency_1.idempotentBatch)(items, async (item) => {
                processCallCount++;
        ...
        it('should report success and failure counts', async () => {
            ...
            await (0, idempotency_1.idempotentBatch)(items, async (item) => {
    ...
    describe('idempotentWrite', () => {
        it('should not write if content unchanged', async () => {
            ...
            const result = await (0, idempotency_1.idempotentWrite)('/test/path', async () => {
        ...
        it('should write if content changed', async () => {
            let writeCallCount = 0;
            let writtenContent = '';
            await (0, idempotency_1.idempotentWrite)('/test/path', async () => {
                return 'new content';
            }, async (content) => {
                writeCallCount++;
    ...
    describe('idempotentRetry', () => {
        it('should retry with exponential backoff', async () => {
            ...
            const result = await (0, idempotency_1.idempotentRetry)(async () => {
                const now = Date.now();
                if (delays.length > 0) {
        ...
        it('should exhaust retries and throw', async () => {
            let attempts = 0;
            try {
                await (0, idempotency_1.idempotentRetry)(async () => {
                    attempts++;
        ...
        it('should be safe to retry idempotent operations', async () => {
            ...
            const result1 = await (0, idempotency_1.idempotentRetry)(operation, 3, 100, {});
            executionCount = 0; // Reset
            const result2 = await (0, idempotency_1.idempotentRetry)(operation, 3, 100, {});
    ...
    describe('Idempotency Law Compliance', () => {
        ...
        it('should handle concurrent idempotent operations', async () => {
            ...
            const results = await Promise.all([
                (0, idempotency_1.idempotentCreate)(async () => ({ exists: true, resource: existingResource, id: 'shared' }), async () => ({ id: 'new-1' }), {}),
                (0, idempotency_1.idempotentCreate)(async () => ({ exists: true, resource: existingResource, id: 'shared' }), async () => ({ id: 'new-2' }), {}),
                (0, idempotency_1.idempotentCreate)(async () => ({ exists: true, resource: existingResource, id: 'shared' }), async () => ({ id: 'new-3' }), {})
            ]);
...
//# sourceMappingURL=idempotency.contract.test.js.map