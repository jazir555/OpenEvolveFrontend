...
exports.VectorDBClient = exports.GraphitiClient = exports.RAGBitsClient = void 0;
...
class RAGBitsClient {
    constructor(config) {
        ...
        this.circuitBreaker = new glue_lib_1.CircuitBreaker({
            threshold: 5,
            timeout_ms: 60000,
        ...
        this.logger = new glue_lib_1.Logger('ragbits-client');
    }
    /**
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            this.logger.error('RAGBits search failed', error, {
                correlation_id: correlationId,
                query: query.substring(0, 100),
                source_service: 'unified-knowledge-query',
                target_service: 'ragbits',
    ...
    async ingest(document) {
        ...
        try {
            ...
            this.logger.error('RAGBits ingest failed', error, {
                correlation_id: correlationId,
                source_service: 'unified-knowledge-query',
                target_service: 'ragbits',
            });
    ...
    async getStats() {
        try {
            ...
            this.logger.error('RAGBits stats failed', error, {
                source_service: 'unified-knowledge-query',
                target_service: 'ragbits',
            });
            throw error;
    ...
    async healthCheck() {
        try {
            ...
            this.logger.warn('RAGBits health check failed', error);
            return false;
        }
    }
...
class GraphitiClient {
    constructor(config) {
        ...
        this.logger = new glue_lib_1.Logger('graphiti-client');
    }
    /**
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            // Add entities
            if (results.entities) {
                items.push(...results.entities.map((entity) => ({
            ...
            if (results.relationships) {
                items.push(...results.relationships.map((rel) => ({
    ...
    async temporalQuery(query, startDate, endDate, options = {}) {
        ...
        try {
            ...
            this.logger.error('Graphiti temporal query failed', error, {
                correlation_id: correlationId,
                source_service: 'unified-knowledge-query',
                target_service: 'graphiti',
            });
    ...
    async getEntities(options = {}) {
        try {
            ...
            this.logger.error('Graphiti getEntities failed', error);
            throw error;
        }
    }
    /**
    ...
    async getRelationships(options = {}) {
        try {
            ...
            this.logger.error('Graphiti getRelationships failed', error);
            throw error;
        }
    }
    /**
    ...
    async getStats() {
        try {
            ...
            this.logger.error('Graphiti stats failed', error);
            throw error;
        }
    }
    /**
    ...
    async healthCheck() {
        try {
            ...
            this.logger.warn('Graphiti health check failed', error);
            return false;
        }
    }
    generateCorrelationId() {
...
class VectorDBClient {
    constructor(config) {
        ...
        this.logger = new glue_lib_1.Logger('vectordb-client');
    }
    /**
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            // Normalize to canonical format
            const items = (results.results || []).map((doc) => ({
            ...
            this.logger.error('VectorDB search failed', error, {
                correlation_id: correlationId,
                query: query.substring(0, 100),
                source_service: 'unified-knowledge-query',
                target_service: 'vectordb',
    ...
    async upsert(collection, vectors) {
        ...
        try {
            ...
            this.logger.error('VectorDB upsert failed', error, {
                correlation_id: correlationId,
                source_service: 'unified-knowledge-query',
                target_service: 'vectordb',
            });
    ...
    async getCollectionInfo(collection) {
        try {
            ...
            this.logger.error('VectorDB getCollectionInfo failed', error);
            throw error;
        }
    }
    /**
    ...
    async getStats() {
        try {
            ...
            this.logger.error('VectorDB stats failed', error);
            throw error;
        }
    }
    /**
    ...
    async healthCheck() {
        try {
            ...
            this.logger.warn('VectorDB health check failed', error);
            return false;
        }
    }
    generateCorrelationId() {
...
exports.VectorDBClient = VectorDBClient;
