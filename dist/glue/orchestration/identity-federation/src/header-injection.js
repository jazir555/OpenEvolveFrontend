...
exports.HeaderInjectionAuth = void 0;
const logger_1 = require("../../lib/logger");
class HeaderInjectionAuth {
    constructor(config = {}) {
        this.config = {
        ...
        logger_1.logger.info('Header Injection Auth initialized', {
            ...this.loggerContext,
    ...
    extractUserFromHeaders(headers) {
        ...
        logger_1.logger.debug('Extracted user from headers', {
            ...this.loggerContext,
            user,
            email,
            has_groups: !!groups,
    ...
    validateHeaders(headers) {
        ...
        const isValid = !!user;
        if (!isValid) {
            logger_1.logger.warn('Missing required auth headers', {
                ...this.loggerContext,
    ...
    parseGroups(headers) {
        ...
        logger_1.logger.debug('Parsed groups from headers', {
            ...this.loggerContext,
     ...
     */
    hasAnyGroup(headers, requiredGroups) {
        const groups = this.parseGroups(headers);
        const hasAny = requiredGroups.some(g => groups.includes(g));
        if (!hasAny) {
            logger_1.logger.debug('User missing any required groups', {
                ...this.loggerContext,
     ...
     */
    createMiddleware(requiredGroups) {
        return async (req, res, next) => {
            const headers = req.headers || {};
            // Validate authentication
            if (!this.validateHeaders(headers)) {
                logger_1.logger.warn('Unauthorized request - missing auth headers', {
                    ...this.loggerContext,
                    path: req.path,
                    method: req.method,
                });
            ...
            if (requiredGroups && requiredGroups.length > 0) {
                if (!this.hasAnyGroup(headers, requiredGroups)) {
                    logger_1.logger.warn('Forbidden request - missing required groups', {
                        ...this.loggerContext,
                        required_groups: requiredGroups,
            ...
            logger_1.logger.debug('Request authenticated via header injection', {
                ...this.loggerContext,
     ...
     */
    createProxyHeaders(accessToken) {
        return {
    ...
    validateCookie(cookie) {
        if (!this.config.cookieSecret) {
            logger_1.logger.warn('Cannot validate cookie - no secret configured', this.loggerContext);
        ...
        try {
            ...
            if (!isValid) {
                logger_1.logger.warn('Cookie signature validation failed', {
                    ...this.loggerContext,
                    timestamp,
                });
            }
            ...
            logger_1.logger.error('Cookie validation error', error, this.loggerContext);
            return false;
        }
    }
    /**
     ...
     */
    computeCookieSignature(value, timestamp) {
    ...
    isWhitelistedDomain(origin) {
        ...
        if (!isWhitelisted) {
            logger_1.logger.warn('Origin not in whitelist', {
                ...this.loggerContext,
                origin,
...
exports.HeaderInjectionAuth = HeaderInjectionAuth;
/**
 * Example usage:
 *
 * ```typescript
