...
const circuit_breaker_1 = require("../../../glue/lib/circuit-breaker");
class PluginRegistry {
    constructor(config = {}) {
        ...
        structured_logger_1.apiLogger.info('Plugin Registry initialized', {
            ...this.correlationContext,
            config: this.config
        });
    }
    ...
    async registerPlugin(plugin) {
        ...
        structured_logger_1.apiLogger.info('Registering plugin', {
            ...this.correlationContext,
            plugin: pluginName,
        ...
        const circuitBreaker = new circuit_breaker_1.CircuitBreaker({
            ...
            onStateChange: (old, newState) => {
                structured_logger_1.apiLogger.warn('Plugin circuit breaker state changed', {
                    ...this.correlationContext,
                    plugin: pluginName,
                    old_state: old,
                    new_state: newState
        ...
        if (this.config.autoInitialize && plugin.metadata.enabled) {
            try {
                ...
                structured_logger_1.apiLogger.error('Failed to auto-initialize plugin', error, {
                    ...this.correlationContext,
                    plugin: pluginName
                });
            }
    ...
    async unregisterPlugin(pluginName) {
        ...
        structured_logger_1.apiLogger.info('Unregistering plugin', {
            ...this.correlationContext,
            plugin: pluginName
        });
        // Destroy plugin
    ...
    async initializePlugin(pluginName) {
        ...
        return entry.circuitBreaker.execute(async () => {
            structured_logger_1.apiLogger.info('Initializing plugin', {
                ...this.correlationContext,
                plugin: pluginName
            });
            ...
            structured_logger_1.apiLogger.info('Plugin initialized successfully', {
                ...this.correlationContext,
                plugin: pluginName
            });
        });
    ...
    async executePlugin(pluginName, fn) {
        ...
        return entry.circuitBreaker.execute(async () => {
            ...
            try {
                ...
                structured_logger_1.apiLogger.info('Plugin execution succeeded', {
                    ...this.correlationContext,
                    plugin: pluginName,
                ...
                const duration = Date.now() - startTime;
                structured_logger_1.apiLogger.error('Plugin execution failed', error, {
                    ...this.correlationContext,
                    plugin: pluginName,
                    duration_ms: duration
                });
    ...
    startHealthChecks() {
        ...
        structured_logger_1.apiLogger.info('Started periodic health checks', {
            ...this.correlationContext,
    ...
    stopHealthChecks() {
        if (this.healthCheckTimer) {
            ...
            structured_logger_1.apiLogger.info('Stopped periodic health checks', this.correlationContext);
        }
    }
    /**
    ...
    async destroy() {
        ...
        const destroyPromises = Array.from(this.plugins.values()).map(async (entry) => {
            try {
                ...
                structured_logger_1.apiLogger.error('Failed to destroy plugin', error, {
                    ...this.correlationContext,
        ...
        this.plugins.clear();
        structured_logger_1.apiLogger.info('Plugin Registry destroyed', this.correlationContext);
    }
}
