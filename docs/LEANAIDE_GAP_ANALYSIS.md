...
The LeanAide integration has a **different architecture** than expected. Instead of being integrated into the BubbleLab API, LeanAide runs as a **standalone server** (port 7654) with its own Streamlit UI. The frontend has excellent integration code, but there's a **critical architectural gap** between the BubbleLab API and the LeanAide server.
...
- **Protocol:** HTTP API
- **UI:** Streamlit interface (port 8501 via LEANAIDE_STREAMLIT_PORT)
