...
class QueryOptimizer:
    def __init__(self, config):
        ...
        self.query_rewriter = QueryRewriter(config.rewrite_config)
    
    async def optimize_query(self, query_ast):
        # Collect query statistics
        query_stats = await self.statistics_collector.analyze_query(query_ast)
        
        # Suggest indexes based on query patterns
        index_suggestions = await self.suggest_indexes(query_ast, query_stats)
        
        # Rewrite query for performance
        optimized_ast = await self.query_rewriter.rewrite(query_ast, query_stats)
        
        # Apply cost-based optimization
        final_ast = await self.apply_cost_based_optimization(optimized_ast, query_stats)
        
        return {
            "optimized_ast": final_ast,
            "index_suggestions": index_suggestions,
            "estimated_cost_reduction": self.calculate_cost_reduction(query_ast, final_ast),
            "optimization_steps": self.get_optimization_steps()
        }
    
    async def suggest_indexes(self, query_ast, query_stats):
        # Analyze query patterns and suggest indexes
        suggestions = []
        
        # Look for frequently accessed properties
        for prop in query_stats.accessed_properties:
            if query_stats.access_frequency[prop] > config.index_threshold:
                suggestions.append({
                    "property": prop,
                    "type": "property_index",
                    "estimated_benefit": self.estimate_index_benefit(prop, query_stats)
                })
        
        # Look for frequently joined properties
        for join_pattern in query_stats.join_patterns:
            suggestions.append({
                "properties": join_pattern.properties,
                "type": "composite_index",
                "estimated_benefit": self.estimate_composite_index_benefit(join_pattern, query_stats)
            })
        
        return suggestions
    
    def calculate_cost_reduction(self, original_ast, optimized_ast):
        # Calculate estimated cost reduction
        original_cost = self.estimate_query_cost(original_ast)
        optimized_cost = self.estimate_query_cost(optimized_ast)
    ...
    def estimate_query_cost(self, ast):
        # Estimate query execution cost based on AST
        cost = 0
        
        # Add cost for pattern matching
        cost += len(ast.patterns) * 100
        
        # Add cost for where clauses
        cost += len(ast.where_clauses) * 50
        
        # Add cost for aggregations
        cost += len(ast.aggregations) * 200
        
        # Add cost for joins
        cost += len(ast.joins) * 300
        
        # Add cost for sorting
        if ast.order_by:
            cost += 150
        
        # Add cost for limiting
        if ast.limit:
            cost += 25
...
```json
{
  "query_alerts": [
    ...
    },
    {
      ...
      "description": "Detect high error rate in queries",
      "severity": "high",
      "condition": {
        "metric": "error_rate",
        "operator": ">",
      ...
      "actions": [
        {
          "type": "alert",
          "destination": "on_call_pager"
        },
        {
          "type": "ticket",
          "destination": "jira_ops"
        }
      ]
    }
  ]
