...
### ACE + Steer Active Reliability Pipeline

The SOP generator can run through a dedicated ACE + Steer pipeline to harden
generation and refinement:

1. ACE injects skills into the task prompt.
2. MAKER/MDAP generates candidate SOPs.
3. Steer verifies the output (markdown, slop, optional json checks).
4. ACE learns from any verification failure and the SOP is regenerated with
   structured feedback appended to the task.

This is enabled by default in `SOPGenerator` when ACE + Steer is available.

```python
from sop_generator import SOPGenerator

generator = SOPGenerator(
    ace_steer_enabled=True,
    ace_agent_id="sop_generator",
    ace_verifications=["slop", "markdown"],
    ace_max_attempts=2
)

sop = await generator.generate_sop(
    requirement_description="Protocol for measuring liquid volume",
    domain="chemistry"
)
```
