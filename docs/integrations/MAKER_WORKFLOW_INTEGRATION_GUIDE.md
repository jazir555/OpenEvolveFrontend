...
### 1b. Stage 0/1 Reliability (Content Analysis + Decomposition)

When `workflow_state.maker_enabled` is set, Stage 0 and Stage 1 route through the
generic MAKER pipeline to enforce JSON structure and improve reliability. The
workflow engine uses `generic_maker_integration.run_generic_maker` with
stage-specific evaluators and falls back to the standard path if MAKER fails to
converge.

Stage 1 also follows the recursive MDAP/MAKER decomposition methodology from the
paper (Algorithm 4). Each decomposition step proposes two subproblems plus a
composition instruction, and the workflow recurses until atomic, producing a
dependency-aware plan with explicit composition nodes.
