# Roadmap: Complete CrewAI Removal

## **Objective**
To systematically remove all remaining string references, documentation notes, and backward-compatibility aliases for the deprecated **CrewAI** system. CrewAI has been fully replaced by the **MIT-licensed CrewAI** orchestration layer.

---

## **Phase 1: Codebase Cleanup (String References & Aliases)**
*Goal: Remove all non-functional mentions of CrewAI in the source code.*

### 1.1. Alias Removal
*   **Target:** `ace_crewai_bridge.py`
    *   Remove `ACECrewAIWorkflowBridge = ACECrewAIWorkflowBridge`.
    *   Remove `"ACECrewAIWorkflowBridge"` from `__all__`.
*   **Target:** `bubblelabs_crewai_bridge.py`
    *   Remove any legacy aliasing for `BubbleLabsCrewAIBridge`.
...
*   **Details:** Remove notes like *"It replaces the AGPL-licensed CrewAI integration"* and replace with purely functional descriptions.

### 1.3. Logic Pruning
*   **Target:** `crewai_unified_bridge.py`
    *   Remove mentions of *"backward compatible with CrewAI"*.
*   **Target:** `crewai_state_management.py`
    *   Remove descriptions comparing local JSON state to the old *"CrewAI ticket-based system"*.
...
*   **Target:** `COMPREHENSIVE_CODEBASE_SECURITY_ANALYSIS.md`
    *   Remove sections referencing vulnerabilities in `CrewAI/src/validation/check_executors.py` (as these files no longer exist).
*   **Target:** `COMPREHENSIVE_SECURITY_AND_QUALITY_ANALYSIS.md`
    *   Remove references to the tight coupling between CrewAI and CrewAI.
...
*   **Target:** `verify_core_crewai_files.py`
    *   This script currently scans for CrewAI imports. 
...
*   **Action:** Manually clear all `__pycache__` directories containing `.pyc` files with the name `crewai`.
*   **Files identified:**
    *   `integrations/bug_fixes/__pycache__/crewai_config_fix...`
    *   `__pycache__/bubblelabs_crewai_bridge...`
    *   `__pycache__/crewai_integration...`

---

## **Phase 5: Hardware Backend Cleanup (High Priority)**
*Goal: Resolve legacy hardware acceleration hooks originally tied to the CrewAI ecosystem.*
...
### 5.2. Logic Re-wiring
*   **Action:** Systematically remove hardware-related placeholder logic that was waiting for CrewAI integration.
...
Once this roadmap is complete, a search for the case-insensitive string `"crewai"` across the entire project directory should yield **zero results**. The system will be 100% described and implemented as a **CrewAI-native** architecture with direct, high-performance hardware backend utilization.
