...
class KnowledgeSourceConnector(ABC):
    ...
    def __init__(self, config: KnowledgeSourceConfig):
        ...
        self._min_query_interval = self._calculate_min_interval(config.rate_limit_per_minute)
    ...
    def query(self, context: Dict[str, Any]) -> List[KnowledgeItem]:
        ...
        """
        raise NotImplementedError("KnowledgeSourceConnector.query must be implemented by subclasses.")
    ...
    @staticmethod
    def _calculate_min_interval(rate_limit_per_minute: Optional[int]) -> float:
        if not rate_limit_per_minute or rate_limit_per_minute <= 0:
            return 0.0
        return 60.0 / float(rate_limit_per_minute)

    def _apply_rate_limit(self) -> None:
        if self._min_query_interval <= 0:
            return
        now = time.time()
        elapsed = now - self._last_query_time
        if elapsed < self._min_query_interval:
            sleep_time = self._min_query_interval - elapsed
            logger.debug(
                "Rate limiting %s for %.2fs",
                self.config.name,
                sleep_time
            )
