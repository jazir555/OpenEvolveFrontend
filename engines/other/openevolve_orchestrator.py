...
class WorkflowTemplate:
    """Represents a workflow template with name, description, and parameters."""
    name: str
    description: str
...
def render_create_workflow_tab(orchestrator: OpenEvolveOrchestrator):
    ...
    **Why This Matters**: Different evolution tasks require different configurations. The orchestrator allows you to fine-tune every aspect of the evolution process to match your specific needs.
    ...
    if workflow_type == "sovereign_decomposition":
        ...
        with st.expander("MDAP / MAKER Configuration", expanded=False):
            mdap_enabled = st.checkbox("Enable MDAP", value=False, key="sg_mdap_enabled")
            maker_enabled = st.checkbox("Enable MAKER", value=False, key="sg_maker_enabled")
            mdap_config_str = st.text_area("MDAP Config (JSON)", value="{}", key="sg_mdap_config")
            maker_config_str = st.text_area("MAKER Config (JSON)", value="{}", key="sg_maker_config")
            try:
                mdap_config = json.loads(mdap_config_str) if mdap_config_str.strip() else {}
            except json.JSONDecodeError:
                st.warning("Invalid MDAP config JSON. Using defaults.")
                mdap_config = {}
            try:
                maker_config = json.loads(maker_config_str) if maker_config_str.strip() else {}
            except json.JSONDecodeError:
    ...
    with start_col:
        if st.button("🚀 Start Workflow", type="primary", use_container_width=True):
            ...
            if workflow_type == "sovereign_decomposition":
                ...
                workflow_state = SovereignWorkflowState(
                    workflow_id=workflow_id,
                    ...
                    mdap_enabled=sg_config.get("mdap_enabled", False),
                    mdap_config=sg_config.get("mdap_config", {}),
                    maker_enabled=sg_config.get("maker_enabled", False),
                    maker_config=sg_config.get("maker_config", {}),
...
def render_monitoring_tab(orchestrator: OpenEvolveOrchestrator):
    ...
    **Why This Matters**: Real-time monitoring allows you to track the evolution performance, identify bottlenecks, and make decisions about resource allocation and workflow continuation.
    ...
    if "active_sovereign_workflow" in st.session_state:
        workflow_state: SovereignWorkflowState = st.session_state.active_sovereign_workflow

        # Always run the workflow engine if the status is 'running'
        if workflow_state.status == "running":
            try:
                asyncio.run(run_sovereign_workflow(
        ...
        if workflow_state.current_stage == "Monitoring" and workflow_state.crewai_workflow_id:
            ...
            try:
                ...
                if tickets:
                    ...
                    col2.metric("Completed", completed_tickets)
                    col3.metric("In Progress", in_progress_tickets)
                    col4.metric("Failed", failed_tickets)
                    
                    # Progress bar
                    if total_tickets > 0:
                        progress_pct = (completed_tickets / total_tickets) * 100
                        st.progress(completed_tickets / total_tickets)
                        st.caption(f"Overall Progress: {progress_pct:.1f}%")
                    ...
                    for ticket in tickets:
                        # Map ticket ID to sub-problem ID if available
                        ticket_id = ticket.get("id", "Unknown")
                        sub_problem_id = workflow_state.ticket_id_to_subproblem_id_map.get(ticket_id, "N/A")
                        
                        ticket_data.append({
                            "Ticket ID": ticket_id[:8] + "...",
                            "Sub-Problem": sub_problem_id,
                            "Title": ticket.get("title", "Untitled")[:50] + "..." if len(ticket.get("title", "")) > 50 else ticket.get("title", ""),
                            "Status": ticket.get("status", "Unknown").title(),
                            "Assignee": ticket.get("assigned_agent_id", "Unassigned")[:8] + "..." if ticket.get("assigned_agent_id") and len(ticket.get("assigned_agent_id")) > 8 else ticket.get("assigned_agent_id", "Unassigned"),
                            "Created": ticket.get("created_at", "")[:19] if ticket.get("created_at") else "N/A"
                        })
                    ...
                    # Add refresh button
                    if st.button("🔄 Refresh Ticket Status"):
                        st.rerun()
        ...
        if workflow_state.status == "running" and workflow_state.current_stage != "Monitoring":
            time.sleep(1)
            st.rerun()
        return
    ...
    try:
        ...
        while retry_count < max_retries and not backend_started:
            try:
                ...
                logging.debug("OpenEvolve backend not reachable yet.")
