...
class FallbackHandler:
    """
    Fallback handler for when OpenEvolve is unavailable or operations fail.

    Provides graceful degradation when the primary OpenEvolve backend
    is not available or encounters errors.
    """

    def __init__(self):
        self.logger = logging.getLogger("OpenEvolve.FallbackHandler")
    ...
    def get_fallback_result(self, operation: str, context: Dict[str, Any]) -> Any:
        ...
        Args:
            operation: The operation that failed (e.g., "evolution", "assessment")
            context: Context information about the operation

        Returns:
            Appropriate fallback result based on operation type
        """
        self.fallback_count += 1
        self.last_fallback_time = time.time()

        self.logger.warning(f"Fallback activated for operation: {operation}")

        # Import here to avoid circular dependency
        try:
            from openevolve_client import EvolutionResult
        except ImportError:
            # If we can't import EvolutionResult, return a dict
            return self._get_dict_fallback(operation, context)
        ...
        if operation == "evolution":
            ...
            return EvolutionResult(
                success=False,
                best_code=content,  # Return original content unchanged
                best_score=0.0,
                iterations_completed=0,
                metrics={
                    "fallback_used": True,
                    "fallback_reason": context.get("error", "OpenEvolve unavailable"),
                    "operation": operation
                },
                error=context.get("error", "OpenEvolve not available - using fallback")
            )

        elif operation == "assessment":
            from red_team import RedTeamAssessment, IssueFinding
            from quality_assessment import SeverityLevel
            from datetime import datetime

            return RedTeamAssessment(
                findings=[],
                assessment_summary="Fallback assessment - OpenEvolve unavailable",
            ...
            return {
                ...
                "fallback_used": True,
                "operation": operation,
                "error": "OpenEvolve not available - fallback mode"
            }

    def _get_dict_fallback(self, operation: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Get dict-based fallback when EvolutionResult is not available"""
        return {
            "success": False,
            "fallback_used": True,
            "operation": operation,
            "error": "OpenEvolve not available - using fallback mode",
            "metrics": {
    ...
    def get_fallback_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_fallback_time": self.last_fallback_time,
            "fallback_active": self.fallback_count > 0
        }
