        ...
        if api_key:
            ...
            if llm_response_content:
                try:
                    ...
                    logger.warning(f"LLM response for {metric.value} not valid JSON. Falling back to heuristics.")
            else:
                logger.warning(f"LLM call failed for {metric.value} evaluation. Falling back to heuristics.")
        ...
        if api_key:
            ...
            if llm_response_content:
                try:
                    ...
                    logger.warning("LLM response for improvement gain not valid JSON. Falling back to heuristics.")
            else:
                logger.warning("LLM call failed for improvement gain evaluation. Falling back to heuristics.")
...
class EvaluatorTeam:
    ...
    def _evaluate_with_openevolve_backend(self, content: str, content_type: str,
        ...
        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if llm_response_content:
                                        ...
                                        logger.warning("LLM call failed for evaluator assessment. Falling back to default score.")
                                    ...
                                    logger.error(f"Error in LLM call for evaluator assessment: {e}", exc_info=True)
                                    return {
                                        "score": 0.75,
                                        "timestamp": datetime.now().timestamp(),
                                        "error": str(e)
                                ...
                                logger.error(f"Error in evaluator assessment: {e}", exc_info=True)
                                return {
                                    "score": 0.0,
                                    "timestamp": datetime.now().timestamp(),
                                    "error": str(e)
            ...
            logger.error(f"Error using OpenEvolve backend for evaluation: {e}", exc_info=True)
            # Fallback to custom implementation
        ...
        try:
            ...
            if orchestration_result and orchestration_result.success:
                try:
                    ...
                    self.logger.error(f"Error parsing orchestration result: {e}. Falling back to internal evaluation.", exc_info=True)
    ...
    def evaluate_with_ensemble(
        ...
        try:
            ...
            logger.error(f"Error using ensemble evaluation: {e}", exc_info=True)
            return self.evaluate_content(content, content_type)
