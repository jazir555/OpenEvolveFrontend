...
try:
    ...
    print("OpenEvolve backend not available - evolutionary_optimization.py cannot function properly")
...
from quality_assessment import QualityAssessmentEngine, QualityIssue, QualityDimension, SeverityLevel
from red_team import RedTeam, RedTeamAssessment, IssueFinding, IssueCategory
from blue_team import BlueTeam, BlueTeamAssessment, FixSuggestion, FixType, FixPriority
from evaluator_team import EvaluatorTeam, IntegratedEvaluation, EvaluatorAssessment, EvaluationMetric, EvaluationScore
...
def run_evolution(initial_content: str, 
    ...
    try:
        ...
        config.evolution.num_islands = 1  # Add island model for better exploration
        config.evolution.elite_ratio = 0.2  # Keep top 20% of individuals
        config.evolution.mutation_rate = 0.1  # 10% mutation rate
        config.evolution.crossover_rate = 0.8  # 80% crossover rate
        config.evolution.archive_size = 100  # Maintain an archive of best solutions
        config.evolution.checkpoint_interval = 5  # Save checkpoint every 5 iterations
