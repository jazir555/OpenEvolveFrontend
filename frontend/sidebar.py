...
from sessionstate import reset_defaults, save_user_preferences, toggle_theme
...
def render_sidebar():
    with st.sidebar:
        ...
        st.selectbox("Provider", list(providers.keys()), key="provider", on_change=reset_defaults)
        ...
        st.selectbox("Reasoning Effort", ["low", "medium", "high"], index=1, key="reasoning_effort")
        ...
        st.selectbox("Language", ["python", "javascript", "java", "cpp", "csharp", "go", "rust", "swift", "kotlin", "typescript", "document"], key="language")
        st.text_input("File Suffix", value=".py", key="file_suffix")
        st.multiselect("Feature Dimensions", ["complexity", "diversity", "readability", "performance"], default=["complexity", "diversity"], key="feature_dimensions")
        st.number_input("Feature Bins", 1, 100, 10, key="feature_bins")
        st.selectbox("Diversity Metric", ["edit_distance", "cosine_similarity", "levenshtein_distance"], key="diversity_metric")
        ...
        if checkpoints:
            selected_checkpoint = st.selectbox("Load Checkpoint", options=checkpoints, key="selected_checkpoint")
        ...
        st.text_area("Evaluator System Prompt", key="evaluator_system_prompt", height=200)

        st.markdown("---")
        st.subheader("Integrations")
        st.text_input("Discord Webhook URL", key="discord_webhook_url", help="Enter your Discord webhook URL to receive notifications.")
        st.text_input("Microsoft Teams Webhook URL", key="msteams_webhook_url", help="Enter your Microsoft Teams webhook URL to receive notifications.")
        st.text_input("Generic Webhook URL", key="generic_webhook_url", help="Enter a generic webhook URL to receive notifications.")

        st.markdown("---")
        st.subheader("Project Settings")
        st.text_input("Project Name", key="project_name")
        st.text_area("Project Description", key="project_description")
        st.checkbox("Public Project", key="project_public", help="Make your project publicly accessible.")
        st.text_input("Project Password", key="project_password", type="password", help="Password-protect your public project.")
        if st.button("Share Project"):
            if st.session_state.project_public:
                ...
                st.warning("Project is not public. Please make the project public to share.")
        ...
        selected_theme = st.selectbox("Select Theme", theme_options, index=theme_options.index(current_theme) if current_theme in theme_options else 0, label_visibility="collapsed")
        ...
        auto_save = st.checkbox("Auto-save preferences", value=st.session_state.user_preferences.get("auto_save", True))
        ...
        st.caption(f"Content Type: Auto-detected")
