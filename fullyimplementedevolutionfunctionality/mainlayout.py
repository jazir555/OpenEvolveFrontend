...
from adversarial import (
    ...
    MODEL_META_LOCK, MODEL_META_BY_ID, run_integrated_adversarial_evolution
)
from evolution import (
    render_evolution_settings, 
    _run_evolution_with_openevolve_backend_refactored,
...
from integrations import (
    ...
    authenticate_github, list_github_repositories, link_github_repository, unlink_github_repository
)
    ...
    if st.session_state.get("page") == "evaluator_uploader":
        ...
        with tabs[0]: # Evolution tab
            ...
            system_prompt = st.text_area(
                ...
                value="You are an expert content generator. Create high-quality, optimized content based on the user's requirements.",
                height=150
            )
            
            evaluator_system_prompt = st.text_area(
                "Evaluator Prompt (for evaluating content quality)",
                value="Evaluate the quality, clarity, and effectiveness of this content. Provide a score from 0 to 100.",
                height=150
            )
                ...
                with st.spinner("Running evolution..."):
                    try:
                        ...
                        if st.session_state.get("evolution_current_best"):
                            st.success(f"✅ Evolution completed successfully! Best content updated.")
                        else:
            ...
            if "evolution_log" in st.session_state and st.session_state.evolution_log:
                ...
                log_text = "\n".join(st.session_state.evolution_log[-20:])  # Show last 20 entries
                st.text_area("Log", value=log_text, height=200, key="evolution_log_display")
        ...
        with st.container(border=True):
            ...
            if integrated_button:
                ...
                if not st.session_state.get("openrouter_key", "").strip():
                    ...
                    st.info("🔄 Starting integrated adversarial testing and evolution...")
                    st.session_state.adversarial_running = True
                    # Run integrated function in a thread
                    def run_integrated():
                        try:
                            result = run_integrated_adversarial_evolution(
                                current_content=st.session_state.protocol_text,
                                content_type="general",  # Default content type, can be enhanced based on review type
                                ...
                                system_prompt="You are an expert content generator. Create high-quality, optimized content based on the user's requirements.", # Use default prompt
                                ...
                                enable_data_augmentation=False, # Default to False
                                augmentation_model_id=None, # Default to None
                                enable_human_feedback=False, # Default to False
                            )
                            if result.get("success"):
                                ...
                                st.success("✅ Integrated adversarial testing and evolution completed successfully!")
                            else:
                                st.error(f"❌ Integrated process failed: {result.get('error', 'Unknown error')}")
                        except Exception as e:
                            st.error(f"❌ Error running integrated process: {e}")
                            import traceback
                            traceback.print_exc()
                        finally:
                            st.session_state.adversarial_running = False
                            
                    threading.Thread(target=run_integrated).start()
                    st.success("✅ Integrated process started successfully!")
                    st.rerun()
