...
class ContentAnalyzer:
    ...
    def _calculate_pattern_score(self, structural_issues: List, logical_anomalies: List, compliance_violations: List) -> float:
        ...
        total_issues = len(structural_issues) + len(logical_anomalies) + len(compliance_violations)
        
        # Base score is 100, subtract points for issues found
        # Each issue type has different weight
        score = 100
        score -= len(structural_issues) * 2  # Structural issues: -2 each
        score -= len(logical_anomalies) * 5  # Logical anomalies: -5 each
        score -= len(compliance_violations) * 10  # Compliance: -10 each
