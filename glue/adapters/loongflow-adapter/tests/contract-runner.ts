...
import { validateAllContracts } from './contract.test';
...
async function testHealthCheck(): Promise<TestResult> {
  ...
  try {
    const response = await axios.get(`${API_URL}/health`, { timeout: TIMEOUT_MS });
 ...
 */
function testFixtureContracts(): TestResult {
  const start = Date.now();
 ...
 */
export async function runContractTests(): Promise<ContractTestReport> {
