...
export class RAGClient {
   ...
   */
  async search(request: RAGSearchRequest, correlationId?: string): Promise<any> {
    const startTime = Date.now();
    const cid = correlationId || randomUUID();
   ...
   * Ingest a document
   */
  async ingest(request: RAGIngestRequest, correlationId?: string): Promise<any> {
    const startTime = Date.now();
    const cid = correlationId || randomUUID();
   ...
   */
  async batchIngest(requests: RAGIngestRequest[], correlationId?: string): Promise<any> {
    const startTime = Date.now();
    const cid = correlationId || randomUUID();
