#!/usr/bin/env python3
"""
Probe script to verify Research Quest - Curie-GlobalChem integration works correctly

This script tests that the integration can access all three systems and perform basic operations.
"""
...
def test_adapter_creation():
    ...
    try:
        sys.path.insert(0, os.path.join(cur_dir, 'src'))
        from research_quest_curie_globalchem_adapter import ResearchQuestCurieGlobalChemAdapter
        adapter = ResearchQuestCurieGlobalChemAdapter()
        print("[SUCCESS] Successfully created Research Quest - Curie-GlobalChem adapter")
        return True
    except Exception as e:
...
async def test_basic_integration():
    ...
    try:
        ...
        # Create adapter
        adapter = ResearchQuestCurieGlobalChemAdapter()
        interface = create_research_interface(adapter)
        
        # Test a basic operation (this might return results or empty lists,
        # but shouldn't throw an exception)
        result = await interface('search', research_topic='water properties')
...
async def main():
    """Main probe function"""
    print("=== Research Quest - Curie-GlobalChem Integration Probe ===")
