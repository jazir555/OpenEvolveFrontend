...
exports.OpenEvolveIntegration = exports.OpenEvolveClient = void 0;
...
class OpenEvolveClient {
    constructor(apiUrl, apiKey, logger, timeout = 30000) {
        this.apiUrl = apiUrl;
        this.apiKey = apiKey;
        this.logger = logger || new logger_1.Logger('openevolve-client');
        this.timeout = timeout;
    }
    /**
     * Get evolution result by ID
     */
    async getEvolutionResult(evolutionId) {
        ...
        try {
            ...
            const response = await fetch(`${this.apiUrl}/evolutions/${evolutionId}`, {
                method: 'GET',
                headers,
            ...
            this.logger.error('Failed to fetch evolution result', error, {
                correlation_id: correlationId,
                evolution_id: evolutionId,
            });
            throw error;
    ...
    async listEvolutions(limit = 10) {
        ...
        try {
            ...
            const response = await fetch(`${this.apiUrl}/evolutions?limit=${limit}`, {
                method: 'GET',
                headers,
                signal: AbortSignal.timeout(this.timeout),
            });
            ...
            this.logger.error('Failed to list evolutions', error, {
                correlation_id: correlationId,
            });
            throw error;
        }
...
class OpenEvolveIntegration {
    constructor(config) {
        ...
        this.logger = this.config.logger || new logger_1.Logger('openevolve-integration');
        // Initialize OpenEvolve client
        this.client = new OpenEvolveClient(this.config.openevolve_api_url, this.config.openevolve_api_key, this.logger, this.config.timeout_ms);
        // Initialize capturer
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.error('Failed to initialize OpenEvolveIntegration', error, {
                correlation_id: correlationId,
            });
            throw new Error(`OpenEvolveIntegration initialization failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // ========================================================================
    // CAPTURE OPERATIONS
     ...
     */
    async captureEvolution(evolutionId, correlationId) {
        ...
        try {
            ...
            for (const solution of topNSolutions) {
                const result = await this.capturer.captureEvolution(problem, solution, solution.metrics, cid);
     ...
     */
    convertToProblem(evolutionResult) {
     ...
     */
    convertToEvolvedCodes(evolutionResult) {
        const evolvedCodes = [];
     ...
     */
    convertSolutionToEvolvedCode(solution, evolutionResult) {
        return {
            id: solution.id || (0, uuid_1.v4)(),
     ...
     */
    convertToMetrics(solution, evolutionResult) {
        return {
    ...
    async close() {
        ...
        try {
            ...
            this.logger.error('Error closing OpenEvolveIntegration', error, {
                correlation_id: 'integration-close',
            });
        }
    }
}
exports.OpenEvolveIntegration = OpenEvolveIntegration;
//# sourceMappingURL=openevolve-integration.js.map