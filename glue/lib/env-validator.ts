 ...
 * @throws Error if validation fails
 */
export function validateEnvWithTypes(vars: EnvVar[]): Record<string, any> {
  const errors: string[] = [];
  ...
  for (const envVar of vars) {
    ...
    try {
      ...
      errors.push(
        `${envVar.name}: Validation error - ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
 ...
 */
export function getEnv(name: string, type: EnvType = 'string'): any {
