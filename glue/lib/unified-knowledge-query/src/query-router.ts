...
export class QueryRouter {
  ...
  constructor() {
    this.logger = new Logger('query-router');
    this.routingContext = {
   ...
   */
  private determineStrategy(query: UnifiedKnowledgeQuery): QueryStrategy {
    // Explicit query type
   ...
   */
  private getStrategyForType(type: QueryType): QueryStrategy {
    switch (type) {
   ...
   */
  private isSystemHealthy(system: SystemSource): boolean {
   ...
   */
  private domainToSystemName(domain: KnowledgeDomain): SystemSource {
    const mapping: Record<KnowledgeDomain, SystemSource> = {
      ragbits: 'ragbits',
      graphiti: 'graphiti',
      vectordb: 'vectordb',
  ...
  private estimateCost(plan: QueryPlan): number {
    ...
    const strategyOverhead: Record<QueryType, number> = {
      'semantic-search': 1.0,
      'temporal-query': 1.5,
      'graph-traversal': 2.0,
      'hybrid': 1.2,
   ...
   */
  private getSystemCost(system: SystemSource): number {
    const costs: Record<SystemSource, number> = {
      ragbits: 2000,
      graphiti: 3000,
      vectordb: 1500,
      fused: 4000,
