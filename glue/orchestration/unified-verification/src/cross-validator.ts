...
export class CrossValidator {
  ...
  private systems: Map<'z3' | 'leanaide', SystemEndpoint> = new Map();
  ...
  constructor(
    ...
    this.logger = logger || new Logger('CrossValidator');
   ...
   */
  async validate(request: VerificationRequest): Promise<CrossValidationResult> {
   ...
   */
  private async executeParallel(request: VerificationRequest): Promise<SystemResult[]> {
    this.logger.info({
   ...
   */
  private async executeSequential(request: VerificationRequest): Promise<SystemResult[]> {
    this.logger.info({
   ...
   */
  private async executeHybrid(request: VerificationRequest): Promise<SystemResult[]> {
    this.logger.info({
   ...
   */
  private compareResults(results: SystemResult[]): ComparisonReport {
    if (results.length < 2) {
      return {
        agreement: true,
    ...
    let agreementType: ComparisonReport['agreementType'];
   ...
   */
  private calculateAggregateConfidence(results: SystemResult[]): number {
   ...
   */
  private inferStrategy(request: VerificationRequest): VerificationStrategy {
   ...
   */
  private toVerificationResults(results: SystemResult[]): VerificationResult[] {
  ...
  private generateComparisonDetails(
    results: SystemResult[],
    agreementType: ComparisonReport['agreementType']
  ): string {
    const [r1, r2] = results;
    ...
    const details: Record<ComparisonReport['agreementType'], string> = {
   ...
   */
  private async checkSystemHealth(system: SystemEndpoint): Promise<boolean> {
    try {
      const response = await axios.get(
        `${system.url}${system.healthCheckPath}`,
        { timeout: 5000 }
      );
