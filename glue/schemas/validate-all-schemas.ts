...
function runTests(): void {
  ...
  const z3Result = validateSolverRequest(Z3Examples.validSolverRequest);
  if (z3Result.success) {
  ...
  console.log('\nTest 2: LeanAide Proof Verification Request');
  const leanAideResult = validateProofVerificationRequest({
  ...
  console.log('\nTest 3: RAGBits RAG Request');
  const ragResult = validateRAGRequest(RAGExamples.validRAGRequest);
  ...
  console.log('\nTest 4: RAGBits Document Chunk');
  const chunkResult = validateDocumentChunk({
  ...
  console.log('\nTest 5: BubbleLab Bubble Request');
  const bubbleResult = validateBubbleRequest(BubbleLabExamples.validBubbleRequest);
  ...
  console.log('\nTest 6: BubbleLab Workflow Request');
  const workflowResult = validateWorkflowRequest(BubbleLabExamples.validWorkflowRequest);
  ...
  console.log('\nTest 7: VectorDB Search Request');
  const vectorSearchResult = validateVectorSearchRequest(VectorDBExamples.validVectorSearchRequest);
  ...
  console.log('\nTest 8: VectorDB Collection Info');
  const collectionInfoResult = validateCollectionInfo(VectorDBExamples.validCollectionInfo);
  ...
  console.log('\nTest 9: Graphiti Entity');
  const entityResult = validateCanonical(CanonicalEntitySchema, {
    id: createCorrelationId(),
    name: 'Test Entity',
  ...
  console.log('\nTest 10: Graphiti Episode');
  const episodeResult = validateCanonical(CanonicalEpisodeSchema, {
    id: createCorrelationId(),
    name: 'Test Episode',
  ...
  console.log('\nTest 11: KarateClub Node Embedding Request');
  const nodeEmbResult = validateNodeEmbeddingRequest({
    algorithm: 'node2vec',
    graph: {
      nodes: [
  ...
  console.log('\nTest 12: KarateClub Community Detection Request');
  const communityResult = validateCommunityDetectionRequest({
