...
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def handle_prometheus_metrics(self):
        ...
        try:
            ...
            payload = observability.export_prometheus_metrics()
    ...
    def handle_readiness(self):
        """Readiness check for worker nodes."""
        try:
            monitor = get_health_monitor()
            results = monitor.run_health_checks()
            ...
            if results.get('overall_healthy'):
                ...
                self.send_json_response(503, {
                    'status': 'not_ready',
                    'failed_checks': [
                        name for name, result in results.get('checks', {}).items()
                        if not result.get('healthy', False)
                    ],
                    'timestamp': results.get('timestamp')
                })
...
def start_health_server(port=8000):
    ...
    logger.info(f"  GET /metrics/prometheus - Prometheus metrics")
