...
def run_evaluator_loop(
    ...
    def _extract_feedback(evaluation_json: Dict[str, Any], raw_text: str) -> str:
        feedback_parts = []
        if isinstance(evaluation_json, dict):
            for key in ["feedback", "summary", "recommendations", "notes"]:
                value = evaluation_json.get(key)
                if isinstance(value, str) and value.strip():
                    feedback_parts.append(value.strip())
            issues = evaluation_json.get("issues")
        ...
        if raw_text:
            feedback_parts.append(raw_text.strip())
    ...
    for iteration in range(max_iterations):
        ...
        for model_id in selected_evaluators:
            try:
                ...
                if result.get("ok"):
                    ...
                    evaluation_feedback.append(_extract_feedback(evaluation_json, result.get("text", "")))
        ...
        if iteration + 1 < max_iterations:
            feedback_blob = "\n\n".join([fb for fb in evaluation_feedback if fb])
            if feedback_blob:
                improver_model = selected_evaluators[0]
                improver_config = model_configs.get(improver_model, {})
                improvement_prompt = (
                    "You are an expert reviewer and editor. Revise the content to address the "
                    "feedback below. Preserve intent, improve clarity, and fix all noted issues. "
                    "Return only the revised content."
                )
                improvement_input = f"Content:\n\n{current_content_text}\n\nFeedback:\n{feedback_blob}"
                improvement_result = analyze_with_model(
                    api_key,
                    improver_model,
                    improvement_input,
                    improver_config,
                ...
                if improvement_result.get("ok") and improvement_result.get("text"):
                    current_content_text = improvement_result["text"].strip()
                    total_prompt_tokens += improvement_result.get("ptoks", 0)
                    total_completion_tokens += improvement_result.get("ctoks", 0)
...
def analyze_with_model(
    ...
    base_url = model_config.get("base_url") or st.session_state.get("base_url") or st.session_state.get(
        "openrouter_base_url", "https://api.openai.com/v1"
    )
    try:
        result_text = _request_openai_compatible_chat(
            api_key=api_key,
            base_url=base_url,
            model=model_id,
            messages=messages,
        ...
        if not result_text:
            return {"ok": False, "text": "", "error": "Empty response from model", "ptoks": 0, "ctoks": 0, "cost": 0.0}
        
        if force_json:
            import json
            try:
                parsed = json.loads(result_text)
                return {"ok": True, "json": parsed, "text": result_text, "ptoks": 0, "ctoks": 0, "cost": 0.0}
        ...
        return {"ok": True, "text": result_text, "ptoks": 0, "ctoks": 0, "cost": 0.0}
    except Exception as e:
        return {"ok": False, "text": "", "error": str(e), "ptoks": 0, "ctoks": 0, "cost": 0.0}
