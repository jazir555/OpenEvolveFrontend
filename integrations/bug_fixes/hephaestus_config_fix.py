<<<<<<< HEAD
"""
Hephaestus Configuration Override Adapter

Fixes invalid paths in Hephaestus config without modifying core files.
...
Usage:
    ...
    original_config = load_hephaestus_config()
...
class HephaestusConfigOverride:
    """
    Provides corrected Hephaestus configuration without modifying core files.

    Uses override pattern to fix path issues in hephaestus_config.yaml.
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        Args:
            config_path: Path to hephaestus_config.yaml (auto-detected if None)
        """
        if config_path is None:
            # Auto-detect Hephaestus config
            possible_paths = [
                './Hephaestus/hephaestus_config.yaml',
                '../Hephaestus/hephaestus_config.yaml',
                '../../Hephaestus/hephaestus_config.yaml',
            ]
    ...
    def load_original_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Loaded Hephaestus config from {self.config_path}")
        except Exception as e:
            logger.error(f"Failed to load Hephaestus config: {e}")
            self._original_config = {}
    ...
    def apply_fixes(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Apply path fixes to Hephaestus config.
    ...
    def get_fixed_config(self) -> Dict[str, Any]:
        """
        Get the fixed Hephaestus config.
...
def get_hephaestus_config() -> Dict[str, Any]:
    """
    Quick access to fixed Hephaestus config.

    Usage:
        from integrations.bug_fixes.hephaestus_config_fix import get_hephaestus_config
        config = get_hephaestus_config()
    """
