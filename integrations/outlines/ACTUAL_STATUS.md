# ACTUAL_STATUS: integrations/outlines

**Verdict: ADAPTER SCAFFOLD - imports cleanly, one optional backend missing**

_Verified 2026-08-20 on Python 3.11.0 (Windows), run from the repo root. No tests were written for this audit; only pre-existing tests were executed._

## What it is

Outlines structured-generation adapter, KG constraints and prompt templates.

## Measured facts

- Python files: **4**
- `python -m py_compile` on every file: **all 4 compile** (no syntax errors anywhere in integrations/)
- Importable as a package: **OK - `import integrations.outlines` succeeds**
- `__init__.py` present: **yes**
- Test files: **NONE**
- Docs: **NONE**
- Files using flat sibling imports: **0**
- Referenced modules that are NOT importable here: **1** - `llama_cpp`

## Honest notes

- Imports fine; `llama_cpp` (optional local-model backend) is NOT installed.
- No tests, no README. No constrained/structured generation verified.

## Bottom line

ADAPTER SCAFFOLD. Compiling is not working code: outside of `oneke` (real test suite) and `bug_fixes` (partial self-tests), **nothing in this package is covered by any executable test**, so no functional claim about it is substantiated.
