...
async def demo():
    ...
    print(f"   [OK] Connected to {len(health)} backend(s): {list(health.keys())}")
    ...
    try:
        ...
        print(f"   [OK] Added 4 knowledge entries")

        print("\n3. Searching knowledge...")
        results = await kg.search("graph database")
        print(f"   [OK] Found {results.total_count} results in {results.search_time_ms:.2f}ms")
        ...
        print(f"   [OK] Backend used: {analysis.backend_used}")
        print(f"   [OK] Analysis time: {analysis.analysis_time_ms:.2f}ms")
        ...
        print(f"   [OK] Generated JSON visualization ({len(viz)} characters)")
        ...
        print(f"   [OK] Batch added {len(ids)} entries")
        ...
        print("  [OK] Backend initialization and connection")
        print("  [OK] Adding knowledge with metadata")
        print("  [OK] Searching with relevance ranking")
        print("  [OK] Graph analysis and statistics")
        print("  [OK] Visualization generation")
        ...
        print("   [OK] Disconnected from all backends")
