...
class DeepKEEnhancedExtractor:
    """
    Enhanced extractor class for DeepKE integration that matches the expected interface
    for the integrations package.
    """

    def __init__(self):
        """Initialize the enhanced extractor."""
        self.deepke_integration = DeepKEIntegration()
        logger.info({
            "msg": "DeepKEEnhancedExtractor initialized",
            "timestamp": datetime.now(timezone.utc).isoformat()
        })

    async def extract_with_deepke(self, text: str, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Extract knowledge using DeepKE with the enhanced interface.
        ...
        """
        try:
            # Use the triple extraction method as the primary extraction
            result = await self.deepke_integration.extract_triples(
                text=text,
    ...
    def is_available(self) -> bool:
        ...
        Returns:
            True if DeepKE is available, False otherwise
        """
        try:
            import deepke
            return True
