...
export function LeanAidePanel({
  ...
  const [mode, setMode] = useState<'theorem' | 'definition' | 'verification' | 'query' | 'elaboration'>('verification');

  if (!isOpen) return null;

  return (
    <div className={`flex flex-col h-full bg-background border-l overflow-hidden ${className}`}>
      <div className="flex items-center justify-between p-3 border-b">
        ...
        <button
          ...
          aria-label="Close LeanAIDE panel"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {[
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md text-sm font-medium transition-colors flex items-center gap-1 whitespace-nowrap ${
                mode === item.value
                  ? 'bg-primary text-primary-foreground'
                  : 'bg-muted text-muted-foreground hover:bg-accent hover:text-accent-foreground'
              }`}
            >
              <MathFunction className="h-3 w-3" />
              {item.label}
            </button>
          ))}
        </div>
