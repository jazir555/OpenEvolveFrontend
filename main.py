...
if sys.platform == "win32":
    try:
        ...
        # Silently ignore as this is not critical for the application
...
try:
    from bubblelabs_ui_component import render_bubblelabs_workflow_ui
    logging.info("Successfully imported bubblelabs UI component")
except ImportError as e:
    logging.error(f"Failed to import bubblelabs UI component: {e}")
    st.error(f"Failed to load bubblelabs UI component: {e}")
    def render_bubblelabs_workflow_ui():
        st.error("BubbleLabs workflow UI component not available")
...
def main():
    ...
    try:
        ...
        try:
            ...
            with tabs[2]:
                try:
                    from n8n_workflow_integration import render_n8n_workflow_integration
                    render_n8n_workflow_integration()
                except ImportError as e:
                    st.error(f"Failed to load n8n workflow integration: {e}")
                    st.info("Please ensure n8n_workflow_integration.py is available and ClaraVerse n8n components are accessible.")
