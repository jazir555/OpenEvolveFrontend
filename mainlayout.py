...
from log_streaming import LogStreaming
from ui_components import render_monitoring_tab
                ...
                with st.spinner(st.session_state.evolution_status_message):
                    try:
                        ...
                        if selected_mode == "multi_objective":
                            ...
                            if not objectives:
                                st.warning("No objectives selected; using default objectives.")
                                st.session_state.objectives = ["performance", "readability"]
                        if selected_mode == "adversarial":
                            # adversarial_attack_model and adversarial_defense_model are handled by widget keys
                            if not st.session_state.get("adversarial_attack_model"):
                                st.session_state.adversarial_attack_model = st.session_state.model
                            if not st.session_state.get("adversarial_defense_model"):
                                st.session_state.adversarial_defense_model = st.session_state.model
...
def render_main_layout():
    ...
    tab_titles = [
        ...
        "Analytics Dashboard", "OpenEvolve Dashboard", "Orchestrator", "Monitoring"
    ]
    ...
    with tabs[11]:
        render_monitoring_tab(st.session_state.get("workflow_state"))
