...
def _should_update_log_display(log_key, current_log):
    """Check if log display should be updated to prevent unnecessary reruns."""
    # Get the previous log state from session
    prev_log_key = f"_prev_{log_key}"
    if prev_log_key not in st.session_state:
        st.session_state[prev_log_key] = []
    
    prev_log = st.session_state[prev_log_key]
    ...
    # Check if last few entries are different
                    ...
                    def run_enhanced_integrated():
                        try:
                            ...
                            if result.get("success"):
                                ...
                                from integrated_reporting import generate_integrated_report, export_integrated_report, calculate_detailed_metrics
