...
def get_integration_info() -> str:
    ...
    lines = [
        ...
        "=" * 50,
        "",
        f"Matryoshka Available: {'✓' if health['matryoshka']['available'] else '✗'}",
        f"  - Unified Client: {'✓' if health['matryoshka']['components']['unified_client'] else '✗'}",
        f"  - Enhanced Client: {'✓' if health['matryoshka']['components']['enhanced_client'] else '✗'}",
        f"  - Memory Bridge: {'✓' if health['matryoshka']['components']['memory_bridge'] else '✗'}",
        "",
        f"MDAP/MAKER Available: {'✓' if health['mdap_maker']['available'] else '✗'}",
        f"  - MDAP Engine: {'✓' if health['mdap_maker']['components']['mdap_engine'] else '✗'}",
        f"  - MAKER Engine: {'✓' if health['mdap_maker']['components']['maker_engine'] else '✗'}",
        "",
        f"Unified Memory Available: {'✓' if health['unified_memory']['available'] else '✗'}",
        f"CrewAI Available: {'✓' if health['crewai']['available'] else '✗'}",
        f"Decomposition Available: {'✓' if health['decomposition']['available'] else '✗'}",
        f"Team System Available: {'✓' if health['team']['available'] else '✗'}",
        "",
        "Usage:",
