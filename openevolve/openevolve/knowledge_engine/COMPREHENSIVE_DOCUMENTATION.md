...
│                         STORAGE LAYER                                       │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│  │ Memgraph        │  │ Qdrant (Vector) │  │  PostgreSQL     │             │
│  │ (Graph)         │  │ Similarity      │  │  JSONB Document │             │
...
**Memgraph (Graph Database) - RECOMMENDED**
- Apache 2.0 licensed (permissive, commercially friendly)
- Compatible with Neo4j's Bolt protocol and Cypher
...
**PostgreSQL with JSONB (Document Store)**
- PostgreSQL License (permissive, MIT-like)
- Raw document storage with JSONB
- ACID-compliant document operations
...
The OpenEvolve Knowledge Engine and its core components are licensed under permissive open-source licenses to ensure maximum flexibility for users.
...
### Storage Layer Licenses

| Database | License | Type | Notes |
|----------|---------|------|-------|
| **Memgraph** | Apache 2.0 | Permissive | ✅ Recommended for graph storage (Neo4j replacement) |
| **PostgreSQL** | PostgreSQL License | Permissive (MIT-like) | ✅ Document storage (MongoDB replacement) |
| **Qdrant** | Apache 2.0 | Permissive | ✅ Vector search |
| ~~Neo4j~~ | ~~GPL/Commercial~~ | ~~Copyleft~~ | ❌ **Deprecated** - replaced with Memgraph |
| ~~MongoDB~~ | ~~SSPL~~ | ~~Copyleft~~ | ❌ **Deprecated** - replaced with PostgreSQL |

### Component Licenses

All 21+ integrated components use permissive licenses:
- **Apache 2.0**: Qdrant, NeuralKG, Causal-Learn, KarateClub, GlobalChem, DSPy, etc.
- **MIT**: CrewAI, Ragbits, Research-Quest, AgentJSON, etc.
- **PostgreSQL License**: PostgreSQL backend
- **BSD**: Various scientific computing libraries

### License Migration Notes

**Neo4j → Memgraph Migration (Completed)**
- Neo4j Community Edition is GPL (copyleft)
- Migrated to Memgraph with Cypher compatibility
- Memgraph is Apache 2.0 licensed (fully permissive)
- Drop-in replacement - same Bolt protocol and Cypher
- In-memory performance with durability

**MongoDB → PostgreSQL Migration (Completed)**
- MongoDB's SSPL license is not permissive (copyleft)
- Migrated to PostgreSQL with JSONB support
- PostgreSQL License is permissive (similar to MIT)
- Full feature parity maintained
- Better ACID compliance with JSONB

### Commercial Use

All components are safe for commercial use:
- No copyleft requirements
- No forced open-sourcing of derivative works
