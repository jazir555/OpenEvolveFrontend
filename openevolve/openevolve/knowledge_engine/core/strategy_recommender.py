...
class EnsembleStrategySelector(StrategyRecommender):
    """
    Enhanced ensemble strategy selector with real-time learning
    ...
    async def recommend_with_ensemble(
        ...
        # Step 3: Get predictions from all methods
        ...
        reasoning = self._generate_ensemble_reasoning(
            individual_predictions, final_strategy, agreement
        )
    ...
    async def handle_cold_start(
        ...
        prediction = EnsemblePrediction(
            ...
            reasoning=f"Cold start: Using rule-based defaults for {domain} domain. "
