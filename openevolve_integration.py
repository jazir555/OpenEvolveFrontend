...
try:
    from openevolve.api import run_evolution as openevolve_run_evolution, EvolutionResult
    from openevolve.config import Config, LLMModelConfig, DatabaseConfig, EvaluatorConfig
...
class OpenEvolveAPI:
    ...
    def start_evolution(self, config: Dict, checkpoint_path: Optional[str] = None) -> Optional[str]:
        try:
            payload = {"config": config}
            if checkpoint_path:
                payload["checkpoint_path"] = checkpoint_path
            response = requests.post(f"{self.base_url}/evolutions", json=payload, headers=self.headers)
    ...
    def get_checkpoints(self) -> Optional[List[str]]:
        try:
            response = requests.get(f"{self.base_url}/checkpoints", headers=self.headers)
            response.raise_for_status()
    ...
    def get_evolution_status(self, evolution_id: str) -> Optional[Dict]:
        try:
            response = requests.get(f"{self.base_url}/evolutions/{evolution_id}", headers=self.headers)
            response.raise_for_status()
    ...
    def get_best_solution(self, evolution_id: str) -> Optional[Dict]:
        try:
            response = requests.get(f"{self.base_url}/evolutions/{evolution_id}/best", headers=self.headers)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def get_evolution_history(self, evolution_id: str) -> Optional[List[Dict]]:
        try:
            response = requests.get(f"{self.base_url}/evolutions/{evolution_id}/history", headers=self.headers)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def stream_evolution_logs(self, evolution_id: str) -> Iterator[str]:
        try:
            with requests.get(f"{self.base_url}/evolutions/{evolution_id}/logs", headers=self.headers, stream=True) as response:
                response.raise_for_status()
    ...
    def get_artifacts(self, evolution_id: str) -> Optional[List[Dict]]:
        try:
            response = requests.get(f"{self.base_url}/evolutions/{evolution_id}/artifacts", headers=self.headers)
            response.raise_for_status()
            return response.json().get("artifacts")
        except requests.exceptions.RequestException as e:
            st.error(f"Error getting artifacts: {e}")
            return None
    ...
    def upload_evaluator(self, evaluator_code: str) -> Optional[str]:
        try:
            response = requests.post(f"{self.base_url}/evaluators", json={"code": evaluator_code}, headers=self.headers)
            response.raise_for_status()
    ...
    def save_custom_prompt(self, prompt_name: str, prompt_content: str) -> bool:
        try:
            response = requests.post(f"{self.base_url}/prompts", json={"name": prompt_name, "content": prompt_content}, headers=self.headers)
            response.raise_for_status()
            return True
        except requests.exceptions.RequestException as e:
    ...
    def delete_evaluator(self, evaluator_id: str) -> bool:
        try:
            response = requests.delete(f"{self.base_url}/evaluators/{evaluator_id}", headers=self.headers)
            response.raise_for_status()
            return True
        except requests.exceptions.RequestException as e:
    ...
    def save_checkpoint(self, evolution_id: str) -> bool:
        try:
            response = requests.post(f"{self.base_url}/evolutions/{evolution_id}/checkpoint", headers=self.headers)
            response.raise_for_status()
            return True
        except requests.exceptions.RequestException as e:
...
def create_advanced_openevolve_config(
    ...
    try:
        ...
        config.database = DatabaseConfig(
            ...
            feature_dimensions=feature_dimensions if feature_dimensions is not None else ["complexity", "diversity"],
...
def create_language_specific_evaluator(content_type: str, custom_requirements: str = "", compliance_rules: Optional[List[str]] = None) -> Callable:
    """
        ...
        try:
            ...
            if compliance_rules and len(compliance_rules) > 0:
                for rule in compliance_rules:
                    if rule.lower() not in content.lower(): # Simple keyword check for now
                        metrics["compliance_score"] -= (1.0 / len(compliance_rules))
                        metrics["compliance_violations"].append(f"Missing compliance rule: {rule}")
            ...
            if content_type == "code_python":
                ...
                metrics.update({
                    "has_contract_terms": any(term in content.lower() for term in ["contract", "agreement", "clause", "party", "jurisdiction"]),
                    "has_legal_citations": any(citation in content for citation in ["§", "et al.", "v.", "supra"]),
                    "document_length_words": len(content.split()),
                })
            elif content_type == "document_medical":
                # Medical-specific checks
                metrics.update({
                    "has_medical_terms": any(term in content.lower() for term in ["patient", "diagnosis", "treatment", "symptom", "medication", "hospital", "clinic"]),
                    "has_patient_id_format": "PID-" in content, # Simple check for a common pattern
                    "document_length_words": len(content.split()),
                })
            elif content_type == "document_technical":
                # Technical-specific checks
                metrics.update({
                    "has_code_snippets": any(keyword in content for keyword in ["def ", "class ", "import ", "function ", "{", "}", ";"]),
                    "has_technical_terms": any(term in content.lower() for term in ["api", "database", "server", "client", "algorithm", "framework", "library"]),
                    "document_length_words": len(content.split()),
                })
            ...
            if custom_requirements:
                ...
                req_matches = sum(1 for req in custom_requirements.split(",") 
                                if req.strip().lower() in content.lower())
                req_score = min(1.0, req_matches / max(1, len(custom_requirements.split(","))))
...
def create_specialized_evaluator(content_type: str, custom_requirements: str = "", compliance_rules: Optional[List[str]] = None) -> Callable:
    """
    ...
    def code_evaluator(program_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_type == "code_python":
                ...
                try:
                    import subprocess
                    result = subprocess.run(["eslint", "-f", "json", program_path], capture_output=True, text=True)
...
def run_specialized_code_evolution(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix=get_file_suffix_from_content_type(content_type), 
