...
# **ACTUAL INTEGRATION**: Adaptive MDAP for Dataframe Util
try:
    ...
    if is_polars_lazyframe(data):
        ...
        if data.shape[0] == max_unevaluated_rows:
            _show_data_information(
                f"[WARN] Showing only {string_util.simplify_number(max_unevaluated_rows)} "
...
def _maybe_truncate_table(
    ...
    if config.get_option("server.enableArrowTruncation"):
        ...
        if truncated_rows:
            ...
            _show_data_information(
                f"[WARN] Showing {displayed_rows} out of {total_rows} "
