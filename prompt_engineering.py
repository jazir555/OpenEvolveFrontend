...
class PromptOptimizer:
    ...
    def get_best_prompts(self, prompt_type: PromptType = None, limit: int = 5) -> List[str]:
        ...
        if prompt_type:
            filtered = {}
            for prompt_id, avg_score in avg_scores.items():
                metadata = self.optimization_history.get(prompt_id, {})
                stored_type = metadata.get("prompt_type")
                if stored_type and stored_type == prompt_type:
                    filtered[prompt_id] = avg_score
            if filtered:
                avg_scores = filtered
            else:
                logger.debug("No prompt type metadata available; returning unfiltered results.")
