...
import { useRomaExecution } from '../hooks/useRomaExecution';
import { RomaExecutionResult, RomaExecutionStatus } from '../types/plugin-types';
import { Play, Square, X, RefreshCw, Clock, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
 ...
 * ```
 */
export const RomaExecutionPanel: React.FC<RomaExecutionPanelProps> = ({
  executionId,
  onClose,
  showFullHistory = false
}) => {
  const { executeTask, cancelExecution, currentExecution, isExecuting, isReady, error: pluginError } = useRomaExecution();
   ...
   * Get status icon
   */
  const getStatusIcon = (status: RomaExecutionStatus) => {
    switch (status) {
      case 'completed':
   ...
   * Get status color
   */
  const getStatusColor = (status: RomaExecutionStatus) => {
    switch (status) {
      case 'completed':
  ...
  return (
    <div className="roma-execution-panel bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-6xl mx-auto">
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
          <Play className="mr-2 w-6 h-6" /> ROMA Execution Panel
        </h2>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={!isReady || isExecuting || !goal.trim()}
          ...
          {isExecuting && (
            <button
              ...
              className="w-full bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-md flex items-center justify-center transition-colors"
            >
            ...
            {isReady && currentExecution && (
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                  ...
                  {currentExecution && (
                    <tr 
                      className={`hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer ${selectedExecutionId === currentExecution.executionId ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
                      onClick={() => setSelectedExecutionId(currentExecution.executionId)}
                    >
