...
def test_error_handling_system():
    ...
    try:
        ...
        print("[OK] Error handling system imported successfully!")
        
        # Test basic error handling
        try:
            error_handler = get_global_error_handler()
            print(f"[OK] Global error handler created: {type(error_handler).__name__}")
            ...
            print(f"[FAIL] Basic error handling failed: {e}")
            return False
        ...
        try:
            ...
            print("[OK] Error handling decorator works!")
            print(f"   - Fallback result: {result}")
            
        except Exception as e:
            print(f"[FAIL] Error handling decorator failed: {e}")
            return False
        
        # Test error summary
        try:
            summary = error_handler.get_error_summary()
            print("[OK] Error summary generation works!")
            print(f"   - Total errors: {summary['total_errors']}")
            print(f"   - Recent errors: {summary['recent_errors']}")
            print(f"   - Categories: {list(summary['category_breakdown'].keys())}")
            
        except Exception as e:
            print(f"[FAIL] Error summary failed: {e}")
            return False
        ...
        try:
            ...
            print("[OK] Evolution error handling integration works!")
            ...
            print(f"[WARN] Evolution integration test failed (expected): {e}")
        ...
        try:
            ...
            print("[OK] OpenEvolve client error handling works!")
            if result.error:
                print(f"   - Error handled gracefully: {result.error[:50]}...")
            
        except Exception as e:
            print(f"[WARN] OpenEvolve client test failed (expected): {e}")
            # This is expected to fail, but should fail gracefully
        
        print("[OK] Error handling system is comprehensive and working!")
        return True
        
    except Exception as e:
        print(f"[FAIL] Error handling system test failed: {e}")
        return False
