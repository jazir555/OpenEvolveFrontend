#!/usr/bin/env python3
"""
Final Robust Import Testing Script for RESE Framework
Handles all exceptions including pytest.skip
"""
...
def test_import_safe(file_path, base_path):
    """Test importing a single file, handling ALL exceptions including pytest.skip"""
    result = {
        'file': str(file_path),
    ...
    # Try import - catch BaseException to handle pytest.skip
    try:
    ...
    except BaseException as e:  # BaseException catches Skipped exception
        error_msg = str(e)
        error_type = type(e).__name__

        # Handle pytest skip and other special cases
        if 'Skipped' in error_type or 'skip' in error_msg.lower():
            result['skipped'] = True
            result['success'] = True  # Skipped is considered OK
            result['error'] = f"Skipped: {error_msg}"
        elif error_type in ['KeyboardInterrupt', 'SystemExit']:
            # Don't catch these
            raise
        else:
...
def main():
    ...
    if failed_results > 0:
        print(f"\nFIXES NEEDED:")
        print(f"  1. Circular imports in phase4/aci_reduction_validator.py")
        print(f"  2. Missing 'Domain' in gamma1/core/csp_models.py")
        print(f"  3. Missing 'core.constraint' module path in psi3")
