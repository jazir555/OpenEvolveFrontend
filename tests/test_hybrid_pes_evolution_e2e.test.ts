...
import { PESEvolutionWorkflow } from '../glue/orchestration/workflows/pes-evolution-workflow';
import { KnowledgeExtractionWorkflow } from '../glue/orchestration/workflows/knowledge-extraction-workflow';
import { AdaptiveExecutionWorkflow } from '../glue/orchestration/workflows/adaptive-execution-workflow';
import { MultiStageReasoningWorkflow } from '../glue/orchestration/workflows/multi-stage-reasoning-workflow';
import { InMemoryEventBus } from '../glue/orchestration/event-bus';
import { DeadLetterQueue } from '../glue/orchestration/dead-letter-queue';
...
describe('Hybrid PES Evolution System', () => {
  ...
  describe('TestHybridWorkflows', () => {
    it('should execute PES evolution workflow', async () => {
      ...
      const workflow = new PESEvolutionWorkflow({
        loongFlowAdapter: loongflow as any,
        openEvolveAdapter: openevolve as any,
        eventBus,
      });
    ...
    it('should extract and store knowledge', async () => {
      ...
      const workflow = new KnowledgeExtractionWorkflow({
        loongflowAdapter: loongflow as any,
        eventBus,
    ...
    it('should execute adaptive workflow', async () => {
      ...
      const workflow = new AdaptiveExecutionWorkflow({
        loongFlowAdapter: loongflow as any,
        openEvolveAdapter: openevolve as any,
        eventBus,
      });
  ...
  describe('TestErrorHandlingAndRecovery', () => {
    ...
    it('should retry with backoff', async () => {
      ...
      const flakyOperation = async (): Promise<{ success: boolean }> => {
        callCount++;
        if (callCount < 3) {
  ...
  describe('TestPerformanceAndScalability', () => {
    it('should handle concurrent execution', async () => {
      ...
      const results = await Promise.all(
        problems.map(p => loongflow.submitProblem(p))
      );
